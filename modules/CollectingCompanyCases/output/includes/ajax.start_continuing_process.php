...
$s_sql = "SELECT * FROM collecting_company_cases_continuing_process_steps WHERE id = ?";
$o_query = $o_main->db->query($s_sql, array($projectData['continuing_process_step_id']));
$continuing_process_step = ($o_query ? $o_query->row_array() : array());
...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($caseId) {            
            ...
            if($first_step){
                //process 
                $result = process_continuing_step($projectData['id'], $first_step['id']);
                if($result['success']){
                    $fw_redirect_url = $_POST['redirect_url'];
                } else {
                    if($result['due_date_not_reached']){
                        $fw_error_msg[] = $formText_DueDateNotReached_output;
                    } else {
                        $fw_error_msg[] = $formText_ErrorProcessingCase_output;
                    }
                }
            } else {
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=start_continuing_process";?>" method="post">

...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($caseId) {
			...
			currency_explanation_text='".$o_main->db->escape_str($_POST['currency_explanation_text'])."'
            WHERE id = $caseId";
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=".$s_inc_obj."&inc_act=".$s_inc_act;?>" method="post">
		...
		<div class="inner">
    		<div class="line">
        		<div class="lineTitle"><?php echo $formText_CurrencyExplanationText_Output; ?></div>
        		<div class="lineInput">
                    <textarea class="popupforminput botspace" name="currency_explanation_text"><?php echo htmlentities($projectData['currency_explanation_text']);?></textarea>
                </div>
        		<div class="clear"></div>
    		</div>
		</div>

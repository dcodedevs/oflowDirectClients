...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($cid) {
            $sql = "UPDATE collecting_cases_other_parts SET
            updated = now(),
            updatedBy='".$variables->loggID."',
            customer_id='".$o_main->db->escape_str($_POST['customer_id'])."'
            WHERE id = $cid";

			$o_query = $o_main->db->query($sql);
            $fw_redirect_url = $_POST['redirect_url'];
        } else {
            $sql = "INSERT INTO collecting_cases_other_parts SET
            created = now(),
            createdBy='".$variables->loggID."',
            customer_id='".$o_main->db->escape_str($_POST['customer_id'])."',
            collecting_company_case_id='".$o_main->db->escape_str($caseId)."'";

			$o_query = $o_main->db->query($sql);
            $insert_id = $o_main->db->insert_id();
            $fw_redirect_url = $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=details&cid=".$caseId;
        }

	}
}
if($action == "delete" && $cid) {
    $sql = "DELETE FROM collecting_cases_other_parts
    WHERE id = $cid";
    $o_query = $o_main->db->query($sql);
}
if($cid) {
    $sql = "SELECT * FROM collecting_cases_other_parts WHERE id = $cid";
	$o_query = $o_main->db->query($sql);
    $otherPartData = $o_query ? $o_query->row_array() : array();

    $s_sql = "SELECT * FROM customer  WHERE customer.id = ?";
    $o_query = $o_main->db->query($s_sql, array($otherPartData['customer_id']));
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_other_part";?>" method="post">
		...
		<div class="inner">
			<div class="line customerInputWrapper">
				<div class="lineTitle"><?php echo $formText_OtherPart_Output; ?></div>
				<div class="lineInput">
					<?php if($customer) { ?>
					...
					<?php } ?>
					<input type="hidden" name="customer_id" id="customerId" value="<?php print $customer['id'];?>" required>
				</div>
				<div class="clear"></div>
			</div>
		</div>
...
$(document).ready(function() {
    $(".popupform-<?php echo $caseId;?> form.output-form").validate({
        ...
        messages: {
            customer_id: "<?php echo $formText_SelectTheCreditor_output;?>",
        }
    });
    ...
    $(".popupform-<?php echo $caseId;?> .selectCustomer").unbind("click").bind("click", function(){
        fw_loading_start();

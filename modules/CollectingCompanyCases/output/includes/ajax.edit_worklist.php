<?php
$worklistId = $_POST['worklistId'] ? $o_main->db->escape_str($_POST['worklistId']) : 0;
$action = $_POST['action'] ? $_POST['action'] : '';
$status = $_POST['status'] ? $_POST['status'] : 0;

$sql = "SELECT * FROM case_worklist WHERE id = $worklistId";
...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($worklistId) {
            $sql = "UPDATE case_worklist SET
            updated = now(),
            updatedBy='".$variables->loggID."',
			name='".$o_main->db->escape_str($_POST['name'])."'
            WHERE id = '".$o_main->db->escape_str($worklistId)."'";

			$o_query = $o_main->db->query($sql);
            $fw_redirect_url = $_POST['redirect_url'];
        } else {
			$sql = "INSERT INTO case_worklist SET
			created = now(),
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_worklist";?>" method="post">
		<input type="hidden" name="fwajax" value="1">
		<input type="hidden" name="fw_nocss" value="1">
		<input type="hidden" name="output_form_submit" value="1">
		<input type="hidden" name="worklistId" value="<?php echo $worklistId;?>">
		...
		<div class="inner">
    		...
    		<div class="line">
        		...
        		<div class="lineInput">
                    <input type="text" class="popupforminput botspace" autocomplete="off" name="name" value="<?php if($projectData) echo $projectData['name']; ?>" >
                </div>
        		<div class="clear"></div>
    		</div>

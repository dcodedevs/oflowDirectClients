...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($caseId) {
			...
			forgivenAmountOnMainClaim='".$o_main->db->escape_str(str_replace(",",".",$_POST['forgivenAmountOnMainClaim']))."',
			forgivenAmountExceptMainClaim='".$o_main->db->escape_str(str_replace(",",".",$_POST['forgivenAmountExceptMainClaim']))."',
			overpaidAmount='".$o_main->db->escape_str(str_replace(",",".",$_POST['overpaidAmount']))."'
            WHERE id = $caseId";
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_forgiven";?>" method="post">
		...
		<div class="inner">
    		...
    		<div class="line">
        		<div class="lineTitle"><?php echo $formText_ForgivenAmountOnMainClaim_Output; ?></div>
        		<div class="lineInput">
                    <input type="text" class="popupforminput botspace" autocomplete="off" name="forgivenAmountOnMainClaim" value="<?php echo number_format($projectData['forgivenAmountOnMainClaim'], 2, ",", ""); ?>" >
                </div>
        		<div class="clear"></div>
    		</div>
			<div class="line">
        		<div class="lineTitle"><?php echo $formText_ForgivenAmountExceptMainClaim_Output; ?></div>
        		<div class="lineInput">
                    <input type="text" class="popupforminput botspace" autocomplete="off" name="forgivenAmountExceptMainClaim" value="<?php echo number_format($projectData['forgivenAmountExceptMainClaim'], 2, ",", ""); ?>" >
                </div>
        		<div class="clear"></div>
    		</div>
			<div class="line">
        		<div class="lineTitle"><?php echo $formText_OverpaidAmount_output; ?></div>
        		<div class="lineInput">
                    <input type="text" class="popupforminput botspace" autocomplete="off" name="overpaidAmount" value="<?php echo number_format($projectData['overpaidAmount'], 2, ",", ""); ?>" >
                </div>
        		<div class="clear"></div>
    		</div>

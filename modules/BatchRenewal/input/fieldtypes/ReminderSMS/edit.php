...
if(!is_file(__DIR__.'/../../../output_ReminderSMSFieldtype/'.trim($field[11]).'.php'))
{
	print ("Configuration error: Sms template is missing");
	return;
}
...
#items_<?php echo $field_ui_id;?> div.item.users { margin-bottom:5px; }
#items_<?php echo $field_ui_id;?> .input { float:left; width:100%; line-height:normal; }
#items_<?php echo $field_ui_id;?> .checkbox { width:auto; }
...
#<?php echo $field_ui_id;?>_times .data .date { width:75%; display:inline-block; }
#<?php echo $field_ui_id;?>_times .data .showreport { width:25%; display:inline-block; text-align:right; }
...
.<?php echo $field_ui_id;?>_report { min-width:700px; padding:2px 5px; }
...
.<?php echo $field_ui_id;?>_report span.name { width:30%; display:inline-block; }
.<?php echo $field_ui_id;?>_report span.mobile { width:30%; display:inline-block; }
.<?php echo $field_ui_id;?>_report span.performed { width:20%; display:inline-block; }
</style>
<div id="selections_<?php echo $field_ui_id;?>" class="group_<?php echo $field_ui_id.($field[9]==1?" hide":"");?>"><?php
	...
	$counter = 0;
	$o_query = $o_main->db->query("select distinct sst.receiver_mobile, sst.receiver from sys_smssendto sst join sys_smssend ss on sst.smssend_id = ss.id where ss.content_id = ? and ss.content_table = ? and ss.content_module_id = ? and ss.type = 1", array($ID, $field[3], $moduleID));
	if($o_query)
	{
		$counter = $o_query->num_rows();
		foreach($o_query->result_array() as $row)
		$o_main->db->query("insert into sys_smssend_userlist(id, session, source, mobile, name, text, origID) values (NULL, ?, ?, ?, ?, ?, ?)", array($sendSms_session, 'preload', $row['receiver_mobile'], $row['receiver'], '', 0));
	}
	?>
	<input type="hidden" value="<?php echo $myself['usersName'];?>" name="<?php echo $field[1].$ending;?>_myselfname">
	<input type="hidden" value="<?php echo $sendSms_session;?>" name="<?php echo $field[1].$ending;?>_session">
	<div id="items_<?php echo $field_ui_id;?>">
		<div class="item users">
			<div class="input" id="<?php echo $field_ui_id;?>_users">
				...
				$(function() {
					...
					$('#<?php echo $field_ui_id;?>_upopup input.pop-close').on('click', function() {
						if(<?php echo $field_ui_id;?>_instance) return;
						$.fancybox.close();
					});
					$('#<?php echo $field_ui_id;?>_mpopup input.pop-ok').unbind('click').on('click', function() {
						// Userlist OK button
						if(<?php echo $field_ui_id;?>_instance) return;
					...
					$('#<?php echo $field_ui_id;?>_addpopup input.pop-ok').unbind('click').on('click', function() {
					...
					$('.<?php echo $field_ui_id;?>_contactset input').on('change',function() {
						...
						if($(_this).is('.import'))
						{
							...
							$.ajax({
								...
								cache: false,
								data: { action: 'import', source: $(this).val(), session: '<?php echo $sendSms_session;?>', field_ui_id: '<?php echo $field_ui_id;?>', companyID : '<?php echo $_GET['companyID'];?>', removeunsubscribers: 1, choosenAdminLang: '<?php echo $choosenAdminLang;?>', caID: '<?php echo $_GET['caID'];?>' },
					...
					$('.<?php echo $field_ui_id;?>_contactset .<?php echo $field_ui_id;?>_contactsetedit').unbind('click').on('click',function() {
						if(<?php echo $field_ui_id;?>_instance) return;
					...
					$('.<?php echo $field_ui_id;?>_contactset .<?php echo $field_ui_id;?>_contactsetadd').unbind('click').on('click',function() {
						if(<?php echo $field_ui_id;?>_instance) return;
					...
					$('#<?php echo $field_ui_id;?>_total_users').unbind('click').on('click',function() {
						if(<?php echo $field_ui_id;?>_instance) return;
				...
				function <?php echo $field_ui_id;?>_show_userlist_page(page)
				{
					if(!<?php echo $field_ui_id;?>_instance)
					{
						...
						$.ajax({
							...
							url: '<?php echo $extradir;?>/input/fieldtypes/<?php echo $field[4];?>/ajax_users.php',
							cache: false,
							data: { action: 'list', source: <?php echo $field_ui_id;?>_manual_source, session: '<?php echo $sendSms_session;?>', field: '<?php echo $field[1].$ending;?>', field_ui_id: '<?php echo $field_ui_id;?>', sourceconfig: <?php echo $field_ui_id;?>_manual_source_config, page: <?php echo $field_ui_id;?>_manual_page, caID: '<?php echo $_GET['caID'];?>' },
							success: function(data) {
				...
				function <?php echo $field_ui_id;?>_save_manual_add()
				{
					if(!<?php echo $field_ui_id;?>_instance)
					{
						...
						$.ajax({
							...
							url: '<?php echo $extradir;?>/input/fieldtypes/<?php echo $field[4];?>/ajax_users.php',
							cache: false,
							data: { action: 'manual_add', session: '<?php echo $sendSms_session;?>', caID: '<?php echo $_GET['caID'];?>', name: $('#<?php echo $field_ui_id;?>_addpopup .pop-data input.name').val(), mobile: $('#<?php echo $field_ui_id;?>_addpopup .pop-data input.mobile').val(), field_ui_id: '<?php echo $field_ui_id;?>' },
							success: function(data) {
								if(data.id)
								{
		...
		<div class="item times">
			<div class="input" id="<?php echo $field_ui_id;?>_times">
				...
				<div class="data">
					<?php
					$o_query = $o_main->db->query("select distinct ss.send_on from sys_smssendto sst join sys_smssend ss on sst.smssend_id = ss.id LEFT OUTER JOIN sys_smssendto sst_check ON sst.smssend_id = sst_check.smssend_id AND sst_check.status = 0 where ss.content_id = ? and ss.content_table = ? and ss.content_module_id = ? and ss.type = 1 and sst.status > 0 AND sst_check.id IS NULL ORDER BY ss.send_on ASC", array($ID, $field[3], $moduleID));
					if($o_query && $o_query->num_rows()>0)
					foreach($o_query->result_array() as $row)
					{
						$row['send_on'] = date('d-m-Y H:i',strtotime($row['send_on']));
						?><div class="done"><span class="date"><?php echo $row['send_on'];?></span><span class="showreport"><a class="script" href="javascript:;" onclick="show_report_<?php echo $field_ui_id;?>('<?php echo $ID;?>','<?php echo $field[3];?>','<?php echo $moduleID;?>','<?php echo $row['send_on'];?>');"><?php echo $formText_showReport_fieldtype;?></a></span></div><?php
					}
					
					$o_query = $o_main->db->query("select distinct ss.send_on from sys_smssendto sst join sys_smssend ss on sst.smssend_id = ss.id LEFT OUTER JOIN sys_smssendto sst_check ON sst.smssend_id = sst_check.smssend_id AND sst_check.status > 0 where ss.content_id = ? and ss.content_table = ? and ss.content_module_id = ? and ss.type = 1 and sst.status = 0 AND sst_check.id IS NOT NULL ORDER BY ss.send_on ASC", array($ID, $field[3], $moduleID));
					if($o_query && $o_query->num_rows()>0)
					foreach($o_query->result_array() as $row)
					{
						$row['send_on'] = date('d-m-Y H:i',strtotime($row['send_on']));
						?><div class="do"><span class="date"><?php echo $row['send_on'];?></span><span class="showreport<?php echo ($field[10]==1?" hide":"");?>"><a class="script" href="javascript:;" onclick="abort_time_<?php echo $field_ui_id;?>('<?php echo $ID;?>','<?php echo $field[3];?>','<?php echo $moduleID;?>','<?php echo $row['send_on'];?>',this);"><?php echo $formText_AbortSending_fieldtype;?></a></span></div><?php
					}
			...
			function show_report_<?php echo $field_ui_id;?>(id, table, moduleid, time)
			{
				$.fancybox.showLoading();
				$.ajax({
					type: 'POST',
			...
			function abort_time_<?php echo $field_ui_id;?>(id, table, moduleid, time, _this)
			{
				...
				$.ajax({
					...
					cache: false,
					data: { id : id, table: table, moduleid: moduleid, time: time, caID: '<?php echo $_GET['caID'];?>' },
					success: function(data) {
						if(data=='OK') {
			...
			<?php /*?>Date.prototype.ddmmyyyyhhss = function() {
			...
			d.setDate(d.getDate()+1)
			d.setHours(12,0,0,0);
			console.log(d.ddmmyyyyhhss());<?php */?>
			<?php if(isset($ob_javascript)) { $ob_javascript .= " ".ob_get_clean(); } ?>
			</script>
		</div>

...
<script type="text/javascript">
	$(".employeeSearchSuggestions .tablescript").unbind("click").bind("click", function(){
		...
		if(employeeID > 0){
			fw_load_ajax("<?=$_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folder=output&folderfile=output&ownercompany=".$_POST['ownercompany']."&date=".$_POST['date']."&department_filter=".$_POST['department_filter']."&customerselfdefinedlist_filter=".$_POST['customerselfdefinedlist_filter']."&project_filter=".$_POST['project_filter']."&otherCustomerFilter=".$_POST['otherCustomerFilter'];?>&search="+employeeID, '', true);
		}
	})
</script>

...
<div class="contentMenuConnections" id="<?=$field_ui_id?>menuconnection" data-fielduniqueid="<?=$field_ui_id?>" >
	<?php foreach($parents as $parent){
		...
		<div class="menuConnectionEntry" data-menuid="<?=$parent;?>" data-menumoduleid="<?=$moduleID?>">
			...
			<input type="hidden" id="<?=$field_ui_id?>level<?=$parentLevel?>" name="<?=$field[1].$ending."level".$parentLevel."[]";?>" value="<?=$parentLevel;?>_<?=$parent;?>"/>
		</div>
		<?php } ?>
	<?php } ?>
</div>
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content allowScrollWrapper">
			<div class="modal-header">
				<div class="menuchooseTitle"><?php echo $formText_EditMenuconnections_fieldtype;?></div>
...
<script type="text/javascript">
	...
	$(".menuchooseBottom .close-btn").unbind("click").bind("click", function(){
		$(this).parents(".modal-content").find(".modal-header .close span").click();
	})	

	$(".menuchooseContent .menuItem").unbind("click").bind("click", function(ev){
		if($(ev.target).hasClass("close")){
			var parent =$(ev.target).parent();
			var menuID = parent.data("menuid");
			var fieldUniqueID = parent.parents(".popupmenucontent").data("fielduniqueid");
			var menuItemInList = $("#tab_"+fieldUniqueID + " .menuConnectionEntry[data-menuid="+menuID+"]");

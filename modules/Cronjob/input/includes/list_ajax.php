...
if($access >= 100 /*all permissions*/ or ($access > 0 and in_array($includefile, $permissionsRead)) or ($access >=10 and in_array($includefile, $permissionsWrite)) )
{
	if(isset($_POST['soker']) && $_POST['soker'] != "" && $access > 0)
	{
		...
		if($_SESSION['filter1'] > 0 || $_SESSION['filter2'] > 0 || $_SESSION['filter3'] > 0)
		{
		...
		if($module_multi_table)
		{
			...
			$listOut = "SELECT ".$soker->mainTable.".*, ".$soker->mainTable.".id as sideID ".$sqlSelect." FROM ".$soker->mainTable." LEFT OUTER JOIN ".$soker->mainTable."content ON ".$soker->mainTable."content.".$soker->mainTable."ID = ".$soker->mainTable.".id AND ".$soker->mainTable."content.languageID = ".$o_main->db->escape($s_default_output_language)." ".$s_sql_filter_join." ".$sqlList.$s_sql_order." LIMIT $startList, $perPage";
		...
		foreach($linkers as $key => $outlink)
		{
			foreach($outlink as $key=>$outlink){
				...
				{
					$outfield =  (array) $outfield;
					$outfield = array_values($outfield);

...
if(!function_exists("ftp_file_put_content")) include_once (__DIR__.'/../../input/includes/ftp_commands.php');
// function for geting list of files by extension
include_once(__DIR__.'/../../input/includes/fnctn_get_files.php');
// function for geting list of files by extension
include_once(__DIR__.'/../../input/includes/fnctn_get_dirs.php');
// function for getting list of all languages form database
if(!function_exists("getOutputLanguages")) include_once (__DIR__.'/../../input/includes/fnctn_get_all_languages.php');
// function for geting current script GET params as string
include_once(__DIR__.'/../../input/includes/fnctn_get_curent_GET_params.php');
// function for geting list of language variables used in scripts
include_once(__DIR__.'/../../input/includes/fnctn_get_language_variables.php');
// function for geting list of accesselement variables used in scripts
include_once(__DIR__.'/../../input/includes/fnctn_get_accesselement_variables.php');
// function for geting list of variables from file
include_once(__DIR__.'/../../input/includes/fnctn_get_form_text_variables.php');
include_once(__DIR__.'/../../input/includes/fnctn_devide_by_upercase.php');
if(!function_exists("include_local")) include_once(__DIR__.'/../../input/includes/fn_include_local.php');

$v_translate_modules = array();
$v_translate_exception = array('Accountinfo', 'CssEditor', 'FrameworkSettings', 'Languages', 'Layoutmanager', 'LibraryLoad', 'Modulemanager', 'SEO');
$s_sql_where = '';
if($variables->developeraccess == 0) $s_sql_where = " WHERE modulemode = 'C'";
$o_query = $o_main->db->query("SELECT name FROM moduledata".$s_sql_where." ORDER BY name");
if($o_query && $o_query->num_rows()>0)
foreach($o_query->result() as $o_row)
{
	if(in_array($o_row->name, $v_translate_exception)) continue;
	$v_translate_modules[] = $o_row->name;
}
if(sizeof($v_translate_modules) == 0)
{
	echo $formText_NoModulesFoundForTranslation_Output;
	return;
}
$s_module = $v_translate_modules[0];
if(isset($_GET['output_module']) && in_array($_GET['output_module'], $v_translate_modules)) $s_module = $_GET['output_module'];
?><ul class="nav nav-tabs"><?php
foreach($v_translate_modules as $s_item)
{
	?><li role="presentation"<?php echo ($s_item==$s_module?' class="active"':'');?>><a href="?<?php echo get_curent_GET_params(array("output_module","actionType","autoTranslate"))."&output_module=".$s_item;?>&actionType=changeOutputLanguage"><?php echo $s_item;?></a></li><?php
}
?>
</ul><br/>
<div class="translate-container">
<?php
$s_module_folder = __DIR__.'/../../../'.$s_module;
$addonFolders = array();
$scan = scandir($s_module_folder);
...
$dirs=array(realpath($s_module_folder));
//define extension of the files
$extensions=array('php');
//directory exceptions
$except_dirs=array(realpath($s_module_folder.'/input'), realpath($s_module_folder.'/properties'));
foreach($addonFolders as $addonFolder) $except_dirs[] = realpath($s_module_folder.'/'.$addonFolder);
//should check subdirs
$check_subdirs=1;
//gets files

$output_folders=array_merge($output_folders, get_dirs($dirs, $except_dirs,0));
...
if(count($db_languages_titles)>0)
{
	...
	if ($_GET['selectedTranslateFrom']!="0" and strlen($_GET['selectedTranslateFrom'])>0)
	{
		$translate_from=$_GET['selectedTranslateFrom'];
	}
	$db_languages=array($db_languages_titles['default'][0],$translate_from);
	if ($_GET['selectedLanguage']!="0" and strlen($_GET['selectedLanguage'])>0)
	{
		...
		foreach($output_folders as $output_dir)
		{
			...
			foreach($db_languages as $db_language)
			{
				...
				if(is_file("$output_dir/languagesOutput/accesselements_$db_language.php"))
				{
					...
					{
						${$directory_id.'_'.$variable_identificator.'_name_'.$db_language}= $language_values[$variable_identificator."_name"];
						${$directory_id.'_'.$variable_identificator.'_description'.'_'.$db_language} = $language_values[$variable_identificator."_description"];
					}
				}
			}
		}
	...
	{
		?>
		<form name="updateLanguages" id="updateLanguages" action="?<?php echo get_curent_GET_params(array("actionType","autoTranslate"));?>&actionType=saveOutputLanguageChanges&selectedLanguage=<?php echo $_GET['selectedLanguage'];?>" method="POST">
		<input type="hidden" value="<?php echo (isset($_GET['choosenInputLang']) ? $_GET['choosenInputLang'] : '');?>" name="updateLang" />
		<input type="hidden" value="1" name="output_form_submit" />
		<input type="hidden" value="1" name="fw_nocss" />
		<input type="hidden" value="1" name="fwajax" />
		
		<div style="margin:10px;"><input type="button" onClick="javascript: document.location.href='?<?php echo get_curent_GET_params(array("autoTranslate","actionType"));?>&actionType=changeOutputLanguage&autoTranslate=1';" value="<?php echo $formText_automaticallyTranslateEmpty_input;?>" /></div>
		...
		?>
		<tr>
			<td colspan="5" class="content_buttons">
				<input type="submit" class="content_button ui-corner-all" value="<?php echo $formText_save_input;?>" name="summitButton"/>
			</td>
		</tr>
		</table>
		</form>
		...
		<style>
		.translate-container { padding:10px 20px; max-width:1024px; margin:0 auto; }
		...
		.inputFromField { width:100%; }
		.translate-container .table td { border:none; }
		.translate-container .item-info > td { padding:0; }

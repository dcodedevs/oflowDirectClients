<?php 
$l_customer_id = isset($_POST['customer_id']) ? $_POST['customer_id'] : 0;
if($_POST['output_form_submit']){
	if($l_customer_id > 0){
		...
		$o_main->db->query("UPDATE customer SET customer_ceases_to_exist_handled = ?, customer_marked_ceases_to_exist_note = ?, customer_ceases_to_exist_handled_date = NOW(), customer_ceases_to_exist_handled_by = ? WHERE id = ?", array($handled, $_POST['note'],$variables->loggID, $l_customer_id));
	}
...
<div class="popupform">
	...
	<form class="output-form" action="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=mark_as_handled";?>" method="post">
		...
		<input type="hidden" name="customer_id" value="<?php print $l_customer_id;?>">

		<div class="inner">
			<div class="line">
				<div class="lineTitle"><?php echo $formText_CustomerName_output; ?></div>
				<div class="lineInput">
					<?php echo $v_creditor['name'];?>
				</div>
				<div class="clear"></div>
			</div>
			<div class="line">
				<div class="lineTitle"><?php echo $formText_CustomerMarkedCeasesToExistDate_output; ?></div>
				<div class="lineInput">
					<?php echo date("d.m.Y", strtotime($v_creditor['customer_marked_ceases_to_exist_date']));?>
				</div>
				<div class="clear"></div>
			</div>
			<div class="line">
				...
				<div class="lineInput">
					<?php echo $v_creditor['customer_marked_ceases_to_exist_reason'];?>
				</div>
				<div class="clear"></div>
			</div>
			<div class="line">
				...
				<div class="lineInput">
					<input type="checkbox" name="handled" value="1" <?php if($v_creditor['customer_ceases_to_exist_handled']) echo 'checked';?>/>
				</div>
				<div class="clear"></div>
			</div>
			<div class="line">
				...
				<div class="lineInput">
					<textarea name="note" class="popupforminput botspace"><?php echo $v_creditor['customer_marked_ceases_to_exist_note'];?></textarea>
				</div>
				<div class="clear"></div>
			</div>
		</div>

...
if($v_row){
		            ...
		            if($creditor['sync_status'] != 1){
		                ...
		                if($noFeeError3){
		                    ...
		                    if(is_file(__DIR__."/../../output/includes/import_scripts/import_cases2.php")){
		                        ...
		                        $s_sql = "UPDATE collecting_cases SET collecting_cases_process_step_id = 0, due_date = ? WHERE id = ?";
		                        $o_query = $o_main->db->query($s_sql, array($v_row['due_date'], $caseData['id']));
	...
	if($v_return['status']) {
	    ...
	    <div class="popupform popupform-<?php echo $eventId;?>">
	        ...
	        <form class="output-form main" action="<?php if(isset($formActionUrl)) { echo $formActionUrl; } else { print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=full_reset_case"; }?>" method="post">

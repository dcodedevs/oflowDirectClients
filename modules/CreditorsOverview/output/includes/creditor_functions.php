...
function get_transaction_list($o_main, $cid, $filter, $filters, $page=1, $perPage=100, $show_all = false, $b_countonly = FALSE) {
    ...
    if($filter == "reminderLevel"){
       ...
       foreach($filters as $filterName=>$filterValue) {
               ...
               case "list_filter": {
                   ...
                   if($filterValue == "canSendReminderNow") {
                        if($creditor['show_transfer_to_collecting_company_in_ready_to_send']) {
                            $sql_where .= " AND (DATE_ADD(IFNULL(cc.due_date, ct.due_date), INTERVAL IF(next_crcpv.days_after_due_date = '' OR next_crcpv.days_after_due_date IS null, IFNULL(nextStep.days_after_due_date, 0), next_crcpv.days_after_due_date) DAY) <= NOW())";
                        } else {
                            if($creditor['skip_reminder_go_directly_to_collecting']) {
                                $sql_where .= " AND ((cc.id is not null AND DATE_ADD(IFNULL(cc.due_date, ct.due_date), INTERVAL IF(next_crcpv.days_after_due_date = '' OR next_crcpv.days_after_due_date IS null, IFNULL(cccps.days_after_due_date, 0), next_crcpv.days_after_due_date) DAY) <= NOW() AND (cc.onhold_by_creditor is null OR  cc.onhold_by_creditor = 0) AND nextStep.id is not null AND (obj.id is null)))".$doNotSendFilter;
                            } else {
                                $sql_where .= " AND ((cc.id is null) OR profile.id is null OR
                                (cc.id is not null AND DATE_ADD(IFNULL(cc.due_date, ct.due_date), INTERVAL IF(next_crcpv.days_after_due_date = '' OR next_crcpv.days_after_due_date IS null, IFNULL(nextStep.days_after_due_date, 0), next_crcpv.days_after_due_date) DAY) <= NOW()
                                AND (cc.onhold_by_creditor is null OR  cc.onhold_by_creditor = 0) AND nextStep.id is not null AND (obj.id is null)))".$doNotSendFilter;
                            }
                        }
				   } else if($filterValue == "dueDateNotExpired"){
                       ...
                       $sql_where .= " AND (obj.id is NOT NULL) AND  (obj.objection_closed_date = '0000-00-00' OR obj.objection_closed_date is null)";
                        ...
                        if($creditor['skip_reminder_go_directly_to_collecting']) {
                            $sql_where .= " AND ((cc.id is not null AND nextStep.id is null) OR cc.id is null) AND DATE_ADD(IFNULL(cc.due_date, ct.due_date), INTERVAL IFNULL(cccps.days_after_due_date, 0) DAY) < NOW() AND (cc.onhold_by_creditor is null OR  cc.onhold_by_creditor = 0) AND (obj.id is null)";
                        } else {
                            $sql_where .= " AND (cc.id is not null AND nextStep.id is null AND profile.id IS NOT NULL) AND DATE_ADD(IFNULL(cc.due_date, ct.due_date), INTERVAL IF(profile.specify_days_here, IFNULL(profile.days_after_due_date_move_to_collecting, 0), IFNULL(stepProcess.days_after_due_date_move_to_collecting, 0)) DAY) < NOW() AND (cc.onhold_by_creditor is null OR  cc.onhold_by_creditor = 0) AND (obj.id is null)";
                        }
        ...
        $sql_select.=', IFNULL(cc.choose_move_to_collecting_process, ct.choose_move_to_collecting_process) as choose_move_to_collecting_process';
      ...
      if($creditor['id'] == 1041 || $creditor['id'] == 1031 ){
       ...
       if($list_filter != "finishedOnReminderLevel" && $list_filter != "movedToCollectingLevel" && $list_filter != "allTransactionsWithoutCases") {
            if($creditor['id'] == 1041 || $creditor['id'] == 1031 ){
                $sql_where .= " AND filter.id is NULL   AND (
            ...
            $sql_where .= " AND DATE_ADD(ct.due_date, INTERVAL IF(crcpv.days_after_due_date = '' OR crcpv.days_after_due_date IS null, IFNULL(ccps.days_after_due_date, 0), crcpv.days_after_due_date) DAY) <= NOW()";
	  ...
	  foreach($filters as $filterName=>$filterValue) {
		  switch($filterName){
            case "search_filter":
                if($filterValue != ""){
                    $sql_join .= "";
                    $sql_where .= " AND (ct.invoice_nr LIKE '%".$o_main->db->escape_like_str($filterValue)."%')";
                }
            break;
            case "customer_filter": 
                if($filterValue > 0){
                    $sql_where .=" AND ct.external_customer_id = '".$o_main->db->escape_str($filterValue)."'";
                }
            break;
            case "transaction_status": 
                if(intval($filterValue) == 0){
                    $status_sql = " AND ct.open = 1";
                } else if($filterValue == 1){
                    $status_sql = " AND ct.open = 0";
                }
            break;

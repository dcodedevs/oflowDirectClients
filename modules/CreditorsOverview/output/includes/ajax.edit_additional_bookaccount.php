...
$sql = "SELECT * FROM creditor_additional_bookaccounts WHERE id = $bookaccount_id";
$o_query = $o_main->db->query($sql);
...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        ...
        if ($bookaccount) {
            ...
            $o_query = $o_main->db->query($sql, array($creditorData['id'], $_POST['bookaccount'], $variables->loggID,$bookaccount['id']));
            $fw_redirect_url = $_POST['redirect_url'];
        } else {
            $sql = "INSERT INTO creditor_additional_bookaccounts SET creditor_id = ?,  bookaccount = ?, createdBy = ?, created=NOW()";
            $o_query = $o_main->db->query($sql, array($creditorData['id'],  $_POST['bookaccount'], $variables->loggID));
...
if($action == "delete" && $bookaccount_id) {
	$sql = "DELETE FROM creditor_additional_bookaccounts
	WHERE id = $bookaccount_id";
	$o_query = $o_main->db->query($sql);
	return;
}
...
<div class="popupform popupform-<?php echo $creditor_id;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_additional_bookaccount";?>" method="post">
		...
		<div class="inner">
            <div class="line">
				...
				<div class="lineInput">
					<input type="text" class="popupforminput botspace" autocomplete="off" name="bookaccount" value="<?php echo $bookaccount['bookaccount']; ?>" required/>
				</div>
				<div class="clear"></div>
			</div>
		</div>

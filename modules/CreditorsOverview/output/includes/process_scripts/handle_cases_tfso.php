...
if($system_settings['locked']) {
	echo $system_settings['locked_message'];
} else if($type_no == "") {
	echo $formText_FailedToRetreiveCustomerInvoiceTypeNo_output." ".$formText_PleaseContactSupport_output;
} else {
	$cases = array();
		            ...
		            if($case['onhold_by_creditor'] != 1 || $skip_to_step > 0){
						...
						if($invoice['currency'] != ""){
							...
							if($currencyName != "") {
				                    ...
				                    foreach($steps as $step) {
				                        ...
				                        if(count($activeObjections) == 0) {
											if($case['due_date'] != "0000-00-00" && $case['due_date'] != "") {
					                            if($case['collecting_cases_process_step_id'] == $previous_step['id'] || $lastStep || intval($case['collecting_cases_process_step_id']) == 0 || $forced) {
					                                ...
					                                if($correctDueDateTime < time() || $forced)
					                                {
														...
														if(!$error_with_currency) {
															...
															echo $formText_ErroConvertingCurrency_output." ".$case['id']."<br/>";
															break;
														}
					                                } else {
												...
												echo $formText_DueDateIsMissingForCase_output." ".$case['id']."<br/>";
												break;
											}
				                        }
				                    }
								...
								echo $formText_MissingCurrency_output." ".$case['id']."<br/>";
							}
						} else {
							echo $formText_MissingCurrency_output." ".$case['id']."<br/>";
						}
	...
	if(count($total_hook_params) > 0) {
	    ...
	    if (file_exists($hook_file)) {
	        ...
	        if (is_callable($run_hook)) {
				...
				$o_query = $o_main->db->query($s_sql, array($creditor['id'], 'total transaction started', $creditor_syncing_id));
	         	$connect_tries = 0;
				...
				foreach($total_hook_params_chunked as $total_hook_params_to_pass){
					...
					if(!$hook_result['result']){
						...
						if($hook_result['error'] != "" && $launchedFromProcessAllScript){
							echo $formText_ErrorCreatingTransactions_output." - ". $hook_result['error']."<br/>";
						}
						...
						if($hook_result['error'] == -100){
							echo $formText_FiscalYearIsNotSet_output."<br/>";
						}
					}
				}
	            ...
	            if($hook_result_no_error) {
					...
					$o_query = $o_main->db->query($s_sql, array($creditor['id'], 'total transaction ended', $creditor_syncing_id, $connect_tries));
	            } else {
	...
	if(!$transaction_errors || count($collectingCasesWithoutError) > 0) {
		if(count($casesToContinueProcessing) > 0){
			...
			if($failedMsg == "") {
				if(!$syncIssues) {
					...
					echo $formText_IssueWithSyncingPleaseTryLater_output."<br/>";
				}
			} else {
				echo $formText_IssueWithSyncingPleaseTryLater_output."<br/>";
			}
		...
		if(!$launchedFromProcessAllScript){
			echo $formText_ErrorCreatingTransactions_output."<br/>";
		}
	}

...
$objection_id = $_POST['cid'];
$objection_closed_handling_description = $_POST['objection_closed_handling_description'];
		...
		if($_POST['objection_closed_handling_description'] != "") {
			if(isset($_POST['cid']) && $_POST['cid'] > 0)
			{
                ...
                if($o_query && $o_query->num_rows() == 1) {
                    ...
                    WHERE id = ?";
                    $o_query = $o_main->db->query($s_sql, array($variables->loggID, $variables->loggID, $objection_closed_handling_description, $objection_id));
                    if($o_query) {
    			        ...
    			        $fw_error_msg = $formText_Error_output;
    			    }
                } else {
                    $fw_error_msg = $formText_NoObjectionFound_output;
                }
			} else {
	...
	<form class="output-form" action="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=close_objection";?>" method="post">
		...
		<div class="inner">
			<div class="line">
				...
				<div class="lineInput"><textarea class="popupforminput botspace" name="objection_closed_handling_description" required></textarea></div>
				<div class="clear"></div>
			</div>
		</div>

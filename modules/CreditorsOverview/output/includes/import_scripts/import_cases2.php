...
if($creditorData) {
	...
	if(!function_exists("customer_local_update")){
		function customer_local_update($customer_list, $creditorData, $updateOnly = false) {
			...
			foreach($customer_list as $customer) {
				...
				$regNr = preg_replace('/[^0-9]+/', '', $customer['OrganizationNumber']);
        ...
        function sync_transactions($invoicesTransactions, $creditorData, $moduleID, $api, $sync_customer=true) {
            ...
            foreach($groupedTransactions as $customerId => $invoicesTransactions) {
                ...
                foreach($invoicesTransactions as $invoicesTransaction) {
					...
					$projectCode = 0;
					if(isset($invoicesTransaction['dimensions']['Dimension']['Type'])) {
						if($invoicesTransaction['dimensions']['Dimension']['Type'] == "Project") {
							$projectCode = $invoicesTransaction['dimensions']['Dimension']['Value'];
							$project_name = $invoicesTransaction['dimensions']['Dimension']['Name'];
						}
					} else {
						foreach($invoicesTransaction['dimensions']['Dimension'] as $dimension){
							if($dimension['Type'] == "Project") {
								$projectCode = $dimension['Value'];
								$project_name = $dimension['Name'];
							}
						}
					}
    ...
    if($creditorData['integration_module'] == "Integration24SevenOffice"){
        ...
        // } else {
			...
			$s_sql = "SELECT * FROM integration24sevenoffice_session WHERE username = '".$o_main->db->escape_str($username)."' AND creditor_id = '".$o_main->db->escape_str($creditorData['id'])."' AND IFNULL(DATE_ADD(IFNULL(failed,'0000-00-00'), INTERVAL 30 MINUTE), CURDATE()) < NOW() ORDER BY created DESC";
	            ...
	            if($api->error == "") {
					...
					if($data['ShowOpenEntries']) {
						...
						do {
							...
							do {
								$connect_tries++;
								$invoicesTransactionsApi = $api->get_transactions($transactionData, false, $show_error);
						...
						do {
							$connect_tries++;
							$invoicesTransactions = $api->get_transactions($transactionData, false, $show_error);
					...
					if($connectedSuccessfully){
							...
							if($connectedSuccessfully) {
								...
								foreach($transactions_with_cases as $invoice) {
									...
									if($case){
										...
										$invoice_payments_prep = ($o_query ? $o_query->result_array() : array());

										if(count($invoice_payments_prep) > 0) {
											//ignore payment with  comment resetFee
											$invoice_payments = array();									
											foreach($invoice_payments_prep as $invoice_payment){												
												$commentArray = explode("_",$fee_transaction['comment']);
												if($commentArray[0] != "resetFee") {
												...
												if(count($credited_transactions) > 0) {
													...
													foreach($fee_transactions as $fee_transaction) {
														...
														if (file_exists($hook_file)) {
															...
															if (is_callable($run_hook)) {
																...
																$o_query = $o_main->db->query($s_sql, array($creditorData['id'], 'reset interest started 1 '.$fee_transaction['id'], $creditor_syncing_id));
																...
																if($hook_result['result']){
																	...
																	$s_sql = "INSERT INTO creditor_syncing_log SET created = NOW(), creditor_id = ?, log = ?, type = 1, creditor_syncing_id = ?, number_of_tries = ?";
																	$o_query = $o_main->db->query($s_sql, array($creditorData['id'], 'reset interest failed: '.json_encode($hook_result), $creditor_syncing_id, $connect_tries));
																}
															}
														}
													}
													...
													if($connectedSuccessfully) {
														if($creditedAmount > 0) {
															...
															$s_sql = "UPDATE collecting_cases SET due_date = '".$o_main->db->escape_str($newDueDate)."', create_letter=0, collecting_cases_process_step_id = 0, updated=NOW(), updatedBy = 'import-CREDITED due date'  WHERE id = '".$o_main->db->escape_str($case['id'])."'";
															$o_query = $o_main->db->query($s_sql);
									...
									if($connectedSuccessfully) {
											...
											if($case) {
												if(!in_array($case['id'], $cases_checked)) {
													...
													if($invoice['open'] == 0) {
														...
														if($local_transaction['collecting_company_case_id'] == 0){
												...
												foreach($fee_transactions as $fee_transaction) {
													...
													if($transactionType == "interest"){														
														...
														if (file_exists($hook_file)) {
															...
															if (is_callable($run_hook)) {
																$s_sql = "INSERT INTO creditor_syncing_log SET created = NOW(), creditor_id = ?, log = ?, type = 1, creditor_syncing_id = ?";
																$o_query = $o_main->db->query($s_sql, array($creditorData['id'], 'reset interest started 2 '.$fee_transaction['id'], $creditor_syncing_id));
												...
												if($calleble_count > 0){
													...
													if($totalInterest > 0) {
														...
														if (file_exists($hook_file)) {
															...
															if (is_callable($run_hook)) {
																$s_sql = "INSERT INTO creditor_syncing_log SET created = NOW(), creditor_id = ?, log = ?, type = 1, creditor_syncing_id = ?";
																$o_query = $o_main->db->query($s_sql, array($creditorData['id'], 'new interest started from rest claim', $creditor_syncing_id));
																...
																if($hook_result['result']){
																	...
																	$s_sql = "INSERT INTO creditor_syncing_log SET created = NOW(), creditor_id = ?, log = ?, type = 1, creditor_syncing_id = ?, number_of_tries = ?";
																	$o_query = $o_main->db->query($s_sql, array($creditorData['id'], 'new interest finished from rest claim', $creditor_syncing_id, $connect_tries));
																} else {
																	// var_dump("deleteError".$hook_result['error']);
																	$s_sql = "INSERT INTO creditor_syncing_log SET created = NOW(), creditor_id = ?, log = ?, type = 1, creditor_syncing_id = ?, number_of_tries = ?";
																	$o_query = $o_main->db->query($s_sql, array($creditorData['id'], 'new interest failed from rest claim: '.json_encode($hook_result), $creditor_syncing_id, $connect_tries));
																}
															}
														}
													}
									    ...
									    foreach($createRestForCasesUpdated as $caseToGenerate) {
											...
											if(!$rest_letter) {
												...
												$s_sql = "UPDATE collecting_cases SET due_date = '".$o_main->db->escape_str($newDueDate)."' WHERE id = '".$o_main->db->escape_str($caseToGenerate)."'";
												$o_query = $o_main->db->query($s_sql);
												if($o_query) {
													$sql = "UPDATE creditor_transactions SET to_be_reordered = 1 WHERE collectingcase_id = ? AND creditor_id = ?";
													$o_query = $o_main->db->query($sql, array($caseToGenerate, $creditorData['id']));
												}
										        ...
										        if(count($result['errors']) > 0){
													...
													$failedMsg = ' error creating rest note';
										        } else {
										...
										$failedMsg = "Failed To Connect to sync transactions . Reached maximum number of tries ".$api->error;
									}
								} else {
									echo $formText_FailedToConnect_output."<br/>";
									$connection_error = true;
									$failedMsg = "Failed To Connect to reset fees . Reached maximum number of tries ".$api->error;
								}
							} else {
								echo $formText_FailedToConnect_output."<br/>";
								$connection_error = true;
								$failedMsg = "Failed To Connect Creating links . Reached maximum number of tries ".$api->error;
							}
		                }
		                if(!$fromProcessCases) {
		                ...
		                $sql = "UPDATE creditor SET lastImportedDate = ?, sync_status = 0, check_type_no = 1, autosyncing_not_working_date = '0000-00-00'  WHERE id = ?";
						...
						echo $formText_FailedToConnect_output."<br/>";
						$connection_error = true;
						$failedMsg = "Failed To Connect while trying to get transactions. Reached maximum number of tries ".$api->error.json_encode($transactionData);
					}
	            } else {
					$s_sql = "UPDATE creditor SET sync_status = 0 WHERE id = ?";
					$o_query = $o_main->db->query($s_sql, array($creditorData['id']));
					
					$s_sql = "UPDATE integration24sevenoffice_session SET failed = NOW() WHERE creditor_id = ? AND session_id = ?";
					$o_query = $o_main->db->query($s_sql, array($creditorData['id'], $api->sessionId));
		...
		// 	echo $formText_FailedToConnect_output."<br/>";
		// 	$connection_error = true;
		// 	$failedMsg = "Critical error with exception. ";
		// }
    }
}
?>

...
class TcpdfCharts extends TCPDF
{
	...
	**/
	...
	function builVerticalGraph($data, $highestTotalAmount = 100, $tableW=200, $tableH=40, $gridTextPt=8,$showGrid=true, $onSameRow = false)
	{
		$MAXSW=200;	// Max string width
		// Retain my starting point
		$oldX = $this->GetX();
		...
		foreach($data as $d) $sums[] = array_sum($d['rows']);
		arsort($sums);
		foreach($sums as $idx=>$d) $sorted_data[] = $data[$idx];

		$final_data = $data;
		$GR = $highestTotalAmount; 
		//$GR = $final_data[0]['rows'][0];
		$OW = $tableW/count($final_data)-4;
		...
		if ($showGrid)
		{
			...
			$this->setFillColor(255,255,255);
			$this->SetLineStyle(array('width' => .5, 'cap' => 'square', 'join' => 'bevel', 'dash' => "3,3", 'color' => array(0, 0, 0))); 
			$this->cell($tableW,$GFS,'',0,0,'R');

			$this->SetX($oldX);
			$this->SetY($oldY);
			$currentY = $this->GetY();
			for($x = 100;$x >= 0;$x--) if ($x % 25 == 0) 
			{
				$height_mod = $tableH / 4;
				// // The numbers at the bottom
				$this->setAlpha(.20);
				...
				$this->setAlpha(.70);
				$this->setX($oldX-15);
				$this->cell($oldX,$height_mod,round($GR / 100 * $x,1),'',0,'L',0, '', 0, false, 'C', 'M');
				$this->setX($oldX);
				$this->setAlpha(.20);
				// horizontal Lines going RIGHT
				$this->SetLineStyle(array('width' => .5, 'cap' => 'square', 'join' => 'bevel', 'dash' => "3,3", 'color' => array(192, 192, 192))); 
				$this->Line($oldX+$tableW,$currentY,$oldX,$currentY);
				$currentY = $this->GetY()+$height_mod;			
				$this->SetY($currentY);	
			}			
		...
		// $this->ln($tableH);
		$this->SetY($oldY+$tableH);
		$this->SetX($oldX);
		$this->setCellPaddings(0, 0, 0, 0);	
		
		foreach($final_data as $idx=>$d)
		{
			$this->cell(2,$tableH,'', 0,0,'',0, '', 0, false,'B','T');
			// For each of my row/datas
			foreach($d['rows'] as $ridx=>$v) 
			{
				// Width of the bar
				...
				if(empty($v['color'])){
					...
					$this->SetFillColorArray($v['color']);
				}
				if ($dch == 0) {
					$this->setFillColor(255,255,255);
				}
				$this->cell($dcw,$dch,'',1,0,'',1, '', 0, false,'B','T');
		...
		foreach($final_data as $idx=>$d)
		{
			...
			$this->cell(2,0,'', 0,0,'',0, '', 0, false,'B','T');
			$this->cell($OW,0,$this->stringReduce($d['text'],$MAXSW),0,0,'L', 0,'',1);
			$this->cell(2,0,'', 0,0,'',0, '', 0, false,'B','T');
		}
		$this->ln(6);
		$this->SetX($oldX);
		foreach($final_data as $idx=>$d)
		{
			...
			foreach($d['rows'] as $row){
				if(empty($row['color'])){
					...
					$this->SetFillColorArray($row['color']);
				}
				
				$this->cell(5,4,'',1,0,'',1, '', 0, false,'T','M');
				$this->cell(25,4,$this->stringReduce($row['label'],$MAXSW),0,0,'L', 0,'',1, false, 'T', 'M');
			}
			break;
		}
		$this->SetLineStyle(array('width' => .5, 'cap' => 'square', 'join' => 'bevel', 'dash' => 0, 'color' => array(0, 0, 0))); 
		$this->Line($oldX,$oldY+$tableH,$oldX,$oldY);
		$this->Line($oldX,$oldY+$tableH,$oldX+$tableW,$oldY+$tableH);

		if($onSameRow) {
			$this->SetY($oldY);
			$this->SetX($oldX+$tableW);
		}
		/***************************/
	}
	...
	**/
	function buildPieGraph($cdata,$r=50,$xc=null,$yc=null)
	{
		...
		if ($xc == null) $xc = $this->getPageWidth() / 4;
		if ($yc == null) $yc = $r + $this->y;

		$colors[] = $this->hex2rgb('e97132');
		$colors[] = $this->hex2rgb('196b24');
		...
		{	
			...
			$this->cell(5,4,'',1,0,'',1, '', 0, false,'T','M');
			$this->cell(40,4,$data['text'][$c],0,1,'L', 0,'',1, false, 'T', 'M');
		}

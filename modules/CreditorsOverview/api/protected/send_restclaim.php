...
if($invoice){
    ...
    if($caseData){
        ...
        if($type_no == "") {
            $v_return['error'] = ($fromMultiCreditorProcessing?$creditor['companyname']." ":'').$formText_FailedToRetreiveCustomerInvoiceTypeNo_output." ".$formText_PleaseContactSupport_output."<br/>";
        } else {
            $noFeeError3 = true;
            ...
            if($noFeeError3count == $calleble_count) {
                if($newInterestAdded){                    
                    ...
                    $collectingCaseToUpdate = $caseData['id'];
                    ...
                    $result = generate_pdf($caseData['id'], 1);
                    if(count($result['errors']) > 0){
                        foreach($result['errors'] as $error){
                            $v_return['error'][]= $formText_LetterFailedToBeCreatedForCase_output." ".$caseData['id']." ".$error."</br>";
                        }
                    } else {

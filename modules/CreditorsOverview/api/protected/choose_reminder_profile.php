...
if($choose_reminder_profile != "") {
    if(isset($case_id) && $case_id > 0)
    {
        ...
        if($o_query && $o_query->num_rows() == 1) {
            ...
            reminder_profile_id = ?
            WHERE id = ?";
            $o_main->db->query($s_sql, array($username, $choose_reminder_profile, $case_id));
        }
        $v_return['status'] = 1;
    } if(isset($transaction_id) && $transaction_id > 0)
    {
        ...
        if($o_query && $o_query->num_rows() == 1) {
            ...
            reminder_profile_id = ?
            WHERE id = ?";
            $o_main->db->query($s_sql, array($username, $choose_reminder_profile, $transaction_id));
        }
        $v_return['status'] = 1;
    } else {

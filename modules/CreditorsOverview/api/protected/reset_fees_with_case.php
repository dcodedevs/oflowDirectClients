            ...
            if($creditor['sync_status'] != 1){
                ...
                foreach($selected_main_transactions as $selected_main_transaction) {
                    ...
                    if($collecting_case){
                        ...
                        $s_sql = "SELECT * FROM creditor_transactions WHERE system_type='InvoiceCustomer' AND link_id = ? AND creditor_id = ? AND (collectingcase_id is null OR collectingcase_id = 0) AND (comment LIKE '%reminderFee_%' OR comment LIKE '%interest_%')";
                        ...
                        if(count($selected_transactions) > 0) {                            
                            ...
                            if (file_exists($hook_file)) {
                                ...
                                if (is_callable($run_hook)) {
                                    ...
                                    $o_query = $o_main->db->query($s_sql, array($creditor['id'], 'full reset fee started with case: '.$transactionType." ".json_encode($reset_amounts), $creditor_syncing_id));
                                    ...
                                    if($hook_result['result']) {
                                        ...
                                        $s_sql = "INSERT INTO creditor_syncing_log SET created = NOW(), creditor_id = ?, log = ?, type = 2, creditor_syncing_id = ?";
                                        $o_query = $o_main->db->query($s_sql, array($creditor['id'], 'full  reset fee finished '.$transactionType, $creditor_syncing_id));
                                    } else {
                                        // var_dump("deleteError".$hook_result['error']);
                                        $s_sql = "INSERT INTO creditor_syncing_log SET created = NOW(), creditor_id = ?, log = ?, type = 2, creditor_syncing_id = ?";
                                        $o_query = $o_main->db->query($s_sql, array($creditor['id'], 'full reset fee failed: '.json_encode($hook_result), $creditor_syncing_id));
                                    }
                                }
                                unset($run_hook);
                            }                         
                ...
                if(is_file(__DIR__."/../../output/includes/import_scripts/import_cases2.php")){
                    ...
                    $o_query = $o_main->db->query($s_sql, array($creditor['id']));
                    echo $validTransactionCount." ".$formText_CasesReset_output;
                    $v_return['status'] = 1;
                } else {

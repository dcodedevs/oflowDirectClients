...
if($moduleAccesslevel > 10)
{
	...
	<div class="addPriceList btnStyle">
		<div class="plusTextBox active">
			<div class="text"><?php echo $formText_AddPriceList_Output; ?></div>
		</div>
		<div class="clear"></div>
	</div>
	
	<div class="exportIncome2 btnStyle">
		<div class="plusTextBox active">
			<div class="text"><?php echo $formText_ExportIncome_Output; ?></div>
		</div>
		<div class="clear"></div>
	...
	<?php if($variables->developeraccess > 5) { ?>
	<div class="showUsageInfo btnStyle">
		<div class="plusTextBox active">
			<div class="text"><?php echo $formText_ShowUsageList_Output; ?></div>
		</div>
		<div class="clear"></div>
	</div>
	<?php }?>
...
$(".exportIncome2").off('click').on('click', function(e){
	e.preventDefault();
	var data = {
		creditor_id: 0
	};
	ajaxCall({module_file: 'export_income_2', module_folder: 'output_creditor'}, data, function(json) {
		$('#popupeditboxcontent').html('');
		$('#popupeditboxcontent').html(json.html);
		out_popup = $('#popupeditbox').bPopup(out_popup_options);
		$("#popupeditbox:not(.opened)").remove();
...
$(".addPriceList").on('click', function(e){
	e.preventDefault();
	var data = {};
    ajaxCall('add_price_list', data, function(obj) {
        $('#popupeditboxcontent').html('');
...
$(".showUsageInfo").off("click").on("click", function(e){
	e.preventDefault();
	var data = {
		creditor_id: 0
	};

<?php
$relationField = $relationModule = $s_relation_module = "";
...
if(is_file($s_relation_file))
{
	...
	foreach($prerelations as $prep)
	{
		...
		if($spPrep[2] == $buttonRelationModule)
		{
			...
			if($prerelationsarray[13]==1) $s_relation_module = "&relation_module_id=".$moduleID;
		}
	}
}
if(isset($ID) && $ID > 0 && $buttonlisttype == "inputform")
{
	$edit_link = $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&".(isset($_GET['moduleID'])?"moduleID=".$_GET['moduleID']."&":'')."module=".$buttonSubmodule."&includefile=edit&submodule=".$relationModule."&relationID=".$ID."&relationfield=".$relationField.($include_sublist?"&subcontent=1":"").($buttonContentStatus!=""?"&content_status=".$buttonContentStatus:"").$s_relation_module;
} else {
	$edit_link = $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&".(isset($_GET['moduleID'])?"moduleID=".$_GET['moduleID']."&":'')."module=".$buttonSubmodule."&includefile=edit&submodule=".$submodule.(isset($_GET['relationID'])?"&relationID=".$_GET['relationID']."&relationfield=".$_GET['relationfield']:"").((isset($include_sublist)&&$include_sublist)?"&subcontent=1":"").($buttonContentStatus!=""?"&content_status=".$buttonContentStatus:"").$s_relation_module;
}
$ui_id_counter++;

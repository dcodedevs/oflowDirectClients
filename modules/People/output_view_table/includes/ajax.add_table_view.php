<?php
$tableViewerId = $_POST['tableViewerId'] ? $o_main->db->escape_str($_POST['tableViewerId']) : 0;
$action = $_POST['action'] ? $_POST['action'] : '';
$status = $_POST['status'] ? $_POST['status'] : 0;

$sql = "SELECT * FROM table_viewer  WHERE id = ?";
$o_query = $o_main->db->query($sql, array($tableViewerId));
	...
	if(isset($_POST['output_form_submit'])) {
		if($_POST['table_name'] != ""){
	        if ($table_viewer) {
	            ...
	            $sql = "INSERT INTO table_viewer SET
	            created = now(),
...
<div class="popupform popupform-<?php echo $processId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output_view_table&inc_obj=ajax&inc_act=add_table_view";?>" method="post">
		<input type="hidden" name="fwajax" value="1">
		<input type="hidden" name="fw_nocss" value="1">
		<input type="hidden" name="output_form_submit" value="1">
		<input type="hidden" name="tableViewerId" value="<?php echo $tableViewerId;?>">
        <input type="hidden" name="redirect_url" value="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output_view_table&inc_obj=details&cid=".$tableViewerId; ?>">
		<div class="inner">
    		<div class="line">
        		<div class="lineTitle"><?php echo $formText_TableName_Output; ?></div>
        		<div class="lineInput">
                    <input type="text" class="popupforminput botspace" autocomplete="off" name="table_name" value="<?php echo $table_viewer['table_name']; ?>">
                </div>
        		<div class="clear"></div>
    		</div>
		</div>

...
{
	$s_sql = "SELECT * FROM people_files WHERE id = '".$o_main->db->escape_str($_POST['cid'])."'";
	...
	$s_sql = "SELECT * FROM contactperson WHERE id = '".$o_main->db->escape_str($v_files['peopleId'])."'";
	$o_query = $o_main->db->query($s_sql);
	if($o_query && $o_query->num_rows()>0)
...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
			...
			if($v_files['id'] > 0 && '' != $s_filename && is_file(BASEPATH.$s_filename))
			{
				...
				if(is_file($s_signant_file))
				{
					...
					if(count($v_signers)>0)
					{
						...
						if(isset($v_response['status']) && 1 == $v_response['status'])
						{
							$o_main->db->query("UPDATE people_files SET signant_id = '".$o_main->db->escape_str($v_response['signant_id'])."' WHERE id = '".$o_main->db->escape_str($v_files['id'])."'");
						} else {
		...
		$fw_redirect_url = $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=details&cid=".$v_files['peopleId'];
		return;
	}
}
?>
...
<div class="popupform-contactperson-clone" style="display:none;">
	<div class="line">
		...
		<div class="lineInput">
			...
			{
				$b_valid = ('' != $v_row['name'] && '' != $v_row['email']);
				?><option value="<?php echo $v_row['name'].'[::]'.$v_row['email'].'[::]'.$v_row['mobile'].'[::]'.$v_row['priority'];?>"<?php echo (!$b_valid?' disabled':'');?>><?php echo $v_row['name'].' ('.$v_row['email'].')';?></option><?php
			}
			?>
			</select>
			<a href="#" onClick="$(this).closest('.line').remove(); return false;"><span class="glyphicon glyphicon-trash"></span></a>

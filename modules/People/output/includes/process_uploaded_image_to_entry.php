...
function process_uploaded_image_to_entry ($o_main, $uploaded_file_id, $img_item, $entry_id) {
	...
	var_dump($module_dir."/input/settings/fields/".$content_table."fields.php");
	...
	$o_query = $o_main->db->query($s_sql, array($entry_id));
	if($o_query && $o_query->num_rows() > 0){
		$entry_data = $o_query->row_array();
	}

    if ($entry_data[$content_field]) {
        $images = json_decode($entry_data[$content_field]);
    } else {
        $images = array();
    }
	...
	$sql = "UPDATE $content_table SET $content_field = ? WHERE id = ?";

	$o_main->db->query($sql, array($images, $entry_id));

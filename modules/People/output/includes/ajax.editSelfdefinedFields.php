...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		...
		if(isset($_POST['editResource']) && $_POST['editResource'] > 0) {
			...
			type= ?
			WHERE id = ?";
			$o_main->db->query($s_sql, array($variables->loggID, $_POST['resourceName'], $_POST['type'], $_POST['editResource']));
			...
			name= ?,
			type= ?";
			$o_main->db->query($s_sql, array($moduleID, $variables->loggID, $_POST['resourceName'], $_POST['type']));
			$fw_return_data = $o_main->db->insert_id();
			$fieldId = $fw_return_data;
		} else {
			$s_sql = "DELETE people_selfdefined_fields  FROM people_selfdefined_fields WHERE people_selfdefined_fields.id = ?";
...
<div class="popupform">
	...
	<div class="resourceList" data-action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editSelfdefinedFields";?>"
	data-action2="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editSelfdefinedFields";?>">
	<?php
			...
			<div class="resourceRow">
				...
				<div class="column typeColumn">
					...
					switch($resource['type']) {
						case 0:
							echo $formText_Text_output;
						break;
						case 1:
							echo $formText_Number_output;
						break;
					}
					...
					<div class="editInnerRow">
						...
						<div class="inputBlock">
							<select name="type" class="resourceType" id="resourceType<?php echo $resource['id']?>">
								<option value="0" <?php if($resource['type'] == 0) echo 'selected';?>><?php echo $formText_Text_output;?></option>
								<option value="1" <?php if($resource['type'] == 1) echo 'selected';?>><?php echo $formText_Number_output;?></option>
							</select>
						</div>
					</div>
					<div class="actionRow">
		...
		<div class="editInnerRow">
			...
			<div class="inputBlock">
				<select name="type" class="resourceType" id="resourceType0">
					<option value="0"><?php echo $formText_Text_output;?></option>
					<option value="1"><?php echo $formText_Number_output;?></option>
				</select>
			</div>
		</div>
		<div class="actionRow">
		...
		$("[data-resource-save-id").unbind("click").on('click', function(e){
	        ...
	        if(resourceName != ""){
				...
				$.ajax({
					...
					url: action,
					cache: false,
					data: 'fwajax=1&fw_nocss=1&editResource=' + resourceID + '&output_form_submit=1&type='+resourceType+'&resourceName='+resourceName,

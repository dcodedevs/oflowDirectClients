...
if($moduleAccesslevel > 10)
{
	if($_POST['output_delete'] == 1){
		if($_GET['cid'] > 0){
			$s_sql = "DELETE FROM people_followup
			WHERE id = ?";
	...
	if(isset($_POST['output_form_submit']))
	{
		...
		if($_POST['date'] != "") {
			...
			$fw_error_msg[] = $formText_DateMissing_output;
			return;
		}
		if($_POST['subject'] == "") {
			$fw_error_msg[] = $formText_SubjectMissing_output;
			return;
		}

		if(isset($_POST['cid']) && $_POST['cid'] > 0) {
			$s_sql = "UPDATE people_followup SET
			updated = now(),
			updatedBy = ?,
			moduleID = ?,
			date = ?,
			...
			peopleId = ?
			WHERE id = ?";
			$o_main->db->query($s_sql, array($variables->loggID, $moduleID, $start_date, $_POST['subject'], $_POST['text'], $_POST['peopleId'], $_POST['cid']));
			$fw_return_data = $_POST['cid'];
		} else {
            $s_sql = "INSERT INTO people_followup SET
			updated = now(),
			updatedBy = ?,
			moduleID = ?,
			date = ?,
			...
			text = ?,
			peopleId = ?";
			$o_main->db->query($s_sql, array($variables->loggID, $moduleID,  $start_date, $_POST['subject'], $_POST['text'], $_POST['peopleId']));
...
if(isset($_POST['cid']) && $_POST['cid'] > 0) {
	$s_sql = "SELECT * FROM people_followup WHERE id = ?";
...
<div class="popupform popupform-<?php echo $groupId;?>">
    ...
    .$_GET['folderfile']."&folder=".$_GET['folder']."&inc_obj=ajax&inc_act=".$_GET['inc_act'];?>" method="post">
        ...
        <div class="popupformTitle"><?php  echo $formText_AddEditFollowup_output;   ?></div>

        <div class="inner">
            <div class="line">
				<div class="lineTitle"><?php echo $formText_Date_Output;?></div>
				<div class="lineInput">
                    <input type="text" name="date" class="popupforminput botspace datepicker" autocomplete="off" required value="<?php if($employementData['date'] != "" && $employementData['date'] != "0000-00-00") { echo date("d.m.Y", strtotime($employementData['date'])); }?>"/>
				</div>
				<div class="clear"></div>
			</div>
            ...
            <div class="line">
				...
				<div class="lineInput">
                    <input type="text" name="subject" class="popupforminput botspace" autocomplete="off" required value="<?php echo $employementData['subject'];?>"/>
				</div>
				<div class="clear"></div>
			</div>
            <div class="line">
				...
				<div class="lineInput">
					<textarea name="text" class="popupforminput botspace" autocomplete="off"><?php echo $employementData['text'];?></textarea>
				</div>
				<div class="clear"></div>
			</div>
            <div class="line">

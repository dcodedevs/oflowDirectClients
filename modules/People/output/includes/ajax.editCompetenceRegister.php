...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		if(isset($_POST['editResource']) && $_POST['editResource'] > 0) {
			$s_sql = "UPDATE people_competence_register SET
			updated = now(),
			updatedBy= ?,
			name= ?
			WHERE id = ?";
			...
			$s_sql = "DELETE people_competence_register  FROM people_competence_register WHERE people_competence_register.id = ?";
			$o_main->db->query($s_sql, array($_POST['deleteResource']));
			$fw_return_data = $_POST['deleteResource'];
		} else {
			$s_sql = "INSERT INTO people_competence_register SET
			id=NULL,
			moduleID = ?,
			created = now(),
			createdBy= ?,
...
<div class="popupform">
	...
	<div class="popupformTitle"><?php echo $formText_AddEditCompetenceRegister_output;?></div>
	<div class="errorMessage"></div>
	<div class="resourceList" data-action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editCompetenceRegister";?>"
	data-action2="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editCompetenceRegister";?>">
	<?php

	$resources = array();
	$s_sql = "SELECT * FROM people_competence_register WHERE content_status < 2 ORDER BY sortnr";
			...
			<?php if(!$resource['disabledForEditing']) { ?>
				...
				<div class="editRow">
					...
					<div class="editInnerRow">
						<label><?php echo $formText_CompetenceName_output?></label>
	...
	<div class="addNew">
		<div class="plusTextBox active fw_text_link_color">
			<div class="text">+ <?php echo $formText_AddCompetenceRegister_output; ?></div>
		</div>
		<div class="clear"></div>
	</div>

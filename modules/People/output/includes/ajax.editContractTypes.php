...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		if(isset($_POST['editResource']) && $_POST['editResource'] > 0) {
			$s_sql = "UPDATE people_contract_types SET
			updated = now(),
			updatedBy= ?,
			name= ?,
			description= ?
			WHERE id = ?";
			$o_main->db->query($s_sql, array($variables->loggID, $_POST['resourceName'], $_POST['resourceDescription'], $_POST['editResource']));
			$fw_return_data = $_POST['editResource'];
			$fieldId = $fw_return_data;
		} else if(isset($_POST['deleteResource']) && intval($_POST['deleteResource']) > 0) {
			$s_sql = "DELETE people_contract_types  FROM people_contract_types WHERE people_contract_types.id = ?";
			$o_main->db->query($s_sql, array($_POST['deleteResource']));
			$fw_return_data = $_POST['deleteResource'];
		} else {
			$s_sql = "INSERT INTO people_contract_types SET
			id=NULL,
			moduleID = ?,
			created = now(),
			createdBy= ?,
			name= ?,
			description= ?";
			$o_main->db->query($s_sql, array($moduleID, $variables->loggID, $_POST['resourceName'], $_POST['resourceDescription']));
...
<div class="popupform">
	...
	<div class="popupformTitle"><?php echo $formText_AddEditContractTypes_output;?></div>
	<div class="errorMessage"></div>
	<div class="resourceList" data-action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editContractTypes";?>"
	data-action2="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editContractTypes";?>">
	<?php

	$resources = array();
	$s_sql = "SELECT * FROM people_contract_types WHERE content_status < 2 ORDER BY sortnr";
		...
		<div class="resourceRowSortable" id="sort_<?php echo $resource['id']?>">
			<div class="resourceRow">
				...
				<div class="column typeColumn">
					<div class="columnWrapper"><?php echo nl2br($resource['description'])?></div>
				</div>
			...
			<?php if(!$resource['disabledForEditing']) { ?>
				...
				<div class="editRow">
					...
					<div class="editInnerRow">
						<label><?php echo $formText_ContractTypeName_output?></label>
						<div class="inputBlock">
							<input type="text" name="resourceName" id="resource<?php echo $resource['id']?>" value="<?php echo $resource['name']?>" autocomplete="off"/>
						</div>
					</div>
					<div class="editInnerRow">
						<label><?php echo $formText_ContractTypeDescription_output;?></label>
						<div class="inputBlock">
							<textarea name="description" id="resourceDescription<?php echo $resource['id']?>" autocomplete="off"><?php echo $resource['description'];?></textarea>
						</div>
					</div>
					<div class="actionRow">
	...
	<div class="newResource editRow">
		...
		<div class="editInnerRow">
			<label><?php echo $formText_ContractTypeDescription_output;?></label>
			<div class="inputBlock">
				<textarea name="description" id="resourceDescription0" autocomplete="off"></textarea>
			</div>
		</div>
		<div class="actionRow">
	...
	<div class="addNew">
		<div class="plusTextBox active fw_text_link_color">
			<div class="text">+ <?php echo $formText_AddContractType_output; ?></div>
		</div>
		<div class="clear"></div>
	</div>

...
{
	$edit_link = $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&".(isset($_GET['moduleID'])?"moduleID=".$_GET['moduleID']."&":'')."module=".$buttonSubmodule."&includefile=../buttontypes/AddAccessForCompanyToEditProfiles/import&submodule=".$relationModule."&relationID=".$ID."&relationfield=".$relationField.($include_sublist?"&subcontent=1":"").($buttonContentStatus!=""?"&content_status=".$buttonContentStatus:"");
} else {
	$edit_link = $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&".(isset($_GET['moduleID'])?"moduleID=".$_GET['moduleID']."&":'')."module=".$buttonSubmodule."&includefile=../buttontypes/AddAccessForCompanyToEditProfiles/import&submodule=".$submodule.(isset($_GET['relationID'])?"&relationID=".$_GET['relationID']."&relationfield=".$_GET['relationfield']:"").($include_sublist?"&subcontent=1":"").($buttonContentStatus!=""?"&content_status=".$buttonContentStatus:"");
}
$ui_id_counter++;

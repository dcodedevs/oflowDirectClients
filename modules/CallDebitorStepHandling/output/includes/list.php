...
$sql = "SELECT * FROM collecting_company_cases_continuing_process_steps WHERE appear_in_call_debitor_step_handling = 1";
...
if(count($legal_handling_steps) > 0) {	
	$sql = "SELECT p.*, creditor.companyname as creditorName FROM collecting_company_cases p
	JOIN creditor ON creditor.id = p.creditor_id
	...
	WHERE nextContinuingStep.id IN ('".implode(",", $legal_handling_steps)."')";
	$o_query = $o_main->db->query($sql);
		...
		<div class="p_content">
			<div class="p_pageContent">
				<div class="p_pageContent_title">
					<?php echo $formText_CasesRequiringDebitorCallHandling_output." (".$legal_count.")";?>
				</div>
				<div class="gtable">

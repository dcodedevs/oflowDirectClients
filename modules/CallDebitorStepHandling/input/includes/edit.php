	...
	<div id="roundFields" class="edit_field_container">
		...
		if($activateSeo == "1")
		{
			...
			foreach($fieldsStructure['seotitle'][6] as $key => $value)
			{
				...
				if('' == $obj_ui_id) continue;
				?><div>
					<div class="oneinput sys-seo-label"><b><?php echo $formText_seoTitle_input;?></b><?php echo ($key!='' ? ' '.$languageName[$key] : '');?></div>
					<div class="onefield sys-seo-input"><input type="text" id="<?php echo $obj_ui_id;?>" name="<?php echo $submodule.'seotitle'.$key;?>" value="<?php echo htmlspecialchars($value);?>" maxlength="70"> <i><?php echo $formText_length_input;?> <span class="counter"><?php echo strlen($value);?></span> <?php echo $formText_of_input;?> <span class="total">70</span></i></div>
				</div><?php
			...
			foreach($fieldsStructure['seodescription'][6] as $key => $value)
			{
				...
				?><div>
					...
					<div class="onefield sys-seo-input"><input type="text" id="<?php echo $obj_ui_id;?>" name="<?php echo $submodule.'seodescription'.$key;?>" value="<?php echo htmlspecialchars($value);?>" maxlength="300"> <i><?php echo $formText_length_input;?> <span class="counter"><?php echo strlen($value);?></span> <?php echo $formText_of_input;?> <span class="total">300</span></i></div>
				</div><?php
				...
				if($seoUrlEditType == 2)
				{
					...
					?><div>
						...
						<div class="onefield sys-seo-input">
							<input type="hidden" id="<?php echo $obj_ui_id;?>fullurledit" name="<?php echo $submodule.'seourl'.$key;?>fullurledit" value="1">
							...
							<input type="text" id="<?php echo $obj_ui_id;?>" name="<?php echo $submodule.'seourl'.$key;?>" value="<?php echo htmlspecialchars($value);?>" data-url-name-field-uid="<?php echo $fieldsStructure[$seoUrlField]['ui_id'.$key];?>">
						</div>
					</div><?php
					...
					?><div>
						...
						<div class="onefield sys-seo-input input-group">
							...
							<input type="hidden" id="<?php echo $obj_ui_id;?>lang" name="<?php echo $submodule.'seourl'.$key;?>lang" value="<?php echo htmlspecialchars($v_seo_url_data['lang'][$key]);?>">
							<input type="hidden" id="<?php echo $obj_ui_id;?>menu" name="<?php echo $submodule.'seourl'.$key;?>menu" value="<?php echo htmlspecialchars($v_seo_url_data['menu'][$key]);?>">
							<input type="text" id="<?php echo $obj_ui_id;?>content" name="<?php echo $submodule.'seourl'.$key;?>content" class="form-control input-sm" value="<?php echo htmlspecialchars($v_seo_url_data['content'][$key]);?>">
							<input type="hidden" id="<?php echo $obj_ui_id;?>" name="<?php echo $submodule.'seourl'.$key;?>" value="<?php echo htmlspecialchars($value);?>" data-url-name-field-uid="<?php echo $fieldsStructure[$seoUrlField]['ui_id'.$key];?>">
						</div>
					</div><?php
				} else {
					// Url not editable
					?><input type="hidden" id="<?php echo $obj_ui_id;?>lang" name="<?php echo $submodule.'seourl'.$key;?>lang" value="<?php echo htmlspecialchars($v_seo_url_data['lang'][$key]);?>">
					<input type="hidden" id="<?php echo $obj_ui_id;?>menu" name="<?php echo $submodule.'seourl'.$key;?>menu" value="<?php echo htmlspecialchars($v_seo_url_data['menu'][$key]);?>">
					<input type="hidden" id="<?php echo $obj_ui_id;?>content" name="<?php echo $submodule.'seourl'.$key;?>content" value="<?php echo htmlspecialchars($v_seo_url_data['content'][$key]);?>">
					<input type="hidden" id="<?php echo $obj_ui_id;?>" name="<?php echo $submodule.'seourl'.$key;?>" value="<?php echo htmlspecialchars($value);?>" data-url-name-field-uid="<?php echo $fieldsStructure[$seoUrlField]['ui_id'.$key];?>"><?php
				}
				...
				?><div>
					...
					<div class="onefield sys-seo-input">
						<div class="sys-seo-preview">
							...
							<div class="sys-seo-link" data-href="<?php echo rtrim(($o_main->accountinfo['domain']!="" ? 'http://'.$o_main->accountinfo['domain'] : $languagedir),'/');?>/"><?php echo rtrim(($o_main->accountinfo['domain']!="" ? $o_main->accountinfo['domain'] : 'www.mydomain.com'),'/');?>/<span id="<?php echo $fieldsStructure['seourl']['ui_id'.$key];?>preview"><?php echo $value;?></span></div>
			...
			if($activateSeoKeywords == "1")
			{
				foreach($fieldsStructure['seokeywords'][6] as $key => $value)
				{
					...
					?><div>
						...
						<div class="onefield sys-seo-input"><input type="text" id="seokeywords<?php echo $key;?>" name="<?php echo $submodule.'seokeywords'.$key;?>" value="<?php echo htmlspecialchars($value);?>"></div>
					</div><?php
				}
			}
				...
				foreach($seoAutoFill as $item)
				{
						...
						if('' != $item[0] && '' != $item[1])
						{
							...
							$('#<?php echo $item[1];?>').on('change',function() {
								...
								if(0 < $ID)
								{
									if(1 > $seoUrlEditType)
									{
										?>$('#<?php echo $item[0].(2 > $seoUrlEditType ? 'content':'');?>').val($(this).val()).trigger('change');<?php
									}
								} else if(0 == $ID && 0 < $seoUrlEditType) { ?>
								if(!$('#<?php echo $item[0].(2 > $seoUrlEditType ? 'content':'');?>').is('.edited'))
									$('#<?php echo $item[0].(2 > $seoUrlEditType ? 'content':'');?>').val($(this).val()).trigger('change');
								<?php } else if(0 == $ID) { ?>
								$('#<?php echo $item[0].(2 > $seoUrlEditType ? 'content':'');?>').val($(this).val()).trigger('change');
								<?php } ?>
							});
							...
							$('#<?php echo $item[0].($seoUrlEditType < 2 ? 'content':'');?>').on('change',function() {
								...
								if(!$(this).is('.edited')) $(this).addClass('edited')
							});
							if($('#<?php echo $item[0];?>').val()=="") $('#<?php echo $item[0].($seoUrlEditType < 2 ? 'content':'');?>').val($('#<?php echo $item[1];?>').val()).trigger('change');
							<?php
						}
					} else if($item[3] == 2 and $mergeHeaderAndMenulevelInTitle=="1") //seo title
					{
						if('' != $item[0] && '' != $item[1])
						{
							...
							if($('#<?php echo $item[0];?>').val()=="") $('#<?php echo $item[0];?>').val($('#<?php echo $item[1];?>').val()).trigger('change');
							<?php
						}
					} else { // other fields
						if('' != $item[0] && '' != $item[1])
			...
			function sys_updateSeoUrl(seoUrlField, languageID)
			{
				...
				<?php } ?>
				var seo_url_value = $(seoUrlField+'<?php echo ($seoUrlEditType < 2 ? 'content':'');?>').val();
				<?php if(empty($ID) && 2 == $seoUrlEditType) { ?>
				if(!$(seoUrlField).is('.edited')) seo_url_value = '';
				<?php } ?>
				$.ajax({
					type: 'POST',
					dataType: 'json',
					...
					data: {
						id: '<?php echo $ID;?>',
						table: '<?php echo $submodule;?>',
						menulevelID: _menulevelID,
						url_name_field_data: $('#'+$(seoUrlField).data('url-name-field-uid')).val(),
						data: seo_url_value,
						languageID: languageID,
						allowEmpty: '<?php echo ($enableEmptySeoUrl==1?1:0);?>',
						seoUrlEditType: '<?php echo $seoUrlEditType;?>',
						seoUrlMenuSplitter: '<?php echo $seoUrlMenuSplitter;?>'
					},
					success: function(data) {
					...
					fw_info_message_add("error", "<?php echo $formText_ErrorOccuredHandlingSeoData_Input;?>", true);
					fw_loading_end();
					fw_editing_instance = false;
				});
			}
		...
		<?php
		$ob_created_updated = "";
		if(isset($showUpdatedCreatedBy) && 1 == $showUpdatedCreatedBy)
		{
			ob_start();
			$subData = array();
		...
		if(!isset($_GET['no_buttons']))
		{
			if(!isset($_GET['subcontent']))
			{
				...
				if($fieldsStructure['content_status'][6]['all'] == 3)
				{
					...
					if(isset($inputButtonSave) && 1 == $inputButtonSave && 10 <= $access)
					{
					...
					if(isset($showDuplicate) && 1 == $showDuplicate && 10 <= $access)
					{
					...
					if(isset($showSave2button) && 1 == $showSave2button && 10 <= $access)
					{
		...
		ob_start();
		if(isset($showPrint) && 1 == $showPrint)
		{

...
if($variables->developeraccess == 20)
{
	...
	foreach($tableorder as  $tablename=>$key)
	{	
		
		//echo "tableorder = ".$tablename. "key =) ".$key;
		...
		<?php 
		if($o_main->multi_acc){
			$s_sql = "SELECT * FROM ".$tablename." WHERE account_id = '".$o_main->db->escape_str($o_main->account_id)."'";
			$o_query = $o_main->db->query($s_sql);
			$table_entries = $o_query ? $o_query->num_rows() : 0; ?>
		<td><a class="empty-confirm-btn" data-name="<?php echo $tablename;?>" href="<?php echo $extradir."/input/includes/emptytable.php?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&emptyTable=".$tablename."&includefile=chooseToUpdate";?>"><?php echo $formText_EmptyTable_input;?> (<?php echo $table_entries?>) </a></td>
		<?php } ?>
		</tr>
		<?php
	}
	...
	<script type="text/javascript">
		$(function(){
			$(".empty-confirm-btn").off("click").on("click", function(e){
				var this_el = $(this);	
				e.preventDefault();
				bootbox.confirm('<?php echo $formText_ConfirmDelete_input; ?>', function (result) {
					if (result) {

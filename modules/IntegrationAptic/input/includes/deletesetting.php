...
if(isset($_GET['deleteField']))
{
	...
	$v_field_configs = sys_get_field_config($file, $account_absolute_path, $databases);

	sanitize_escape($_GET['deleteField'], 'string', $deleteFieldName);
	$deleteField = $v_field_configs[$deleteFieldName];
	...
	$makeString = "";
	foreach($v_field_configs as $nut)
	{
	...
	unset($v_field_configs[$deleteFieldName]);
...
if(isset($_GET['deleteButtonField']))
{
	...
	sanitize_escape($_GET['buttontype'], 'string', $buttontype);
	sanitize_escape($_GET['deleteButtonField'], 'string', $deleteFieldName);
	$v_field_configs = sys_get_button_config("/modules/$module/input/settings/buttonconfig/".$submodule.$buttontype.".php", $account_absolute_path);

	$makeString = "";
	foreach($v_field_configs as $key => $nut)
	{
	...
	ftp_file_put_content("/modules/$module/input/settings/buttonconfig/".$submodule.$buttontype.".php","<?php\n{$outString}\n?>");
}
...
{
	sanitize_escape($_GET['deleteTable'], 'string', $deleteTable);

...
if(isset($_POST['send']))
{
	...
	$alterData->init($_POST['databasename'], $submodule."".$_POST['databasename'], '$formText_'.$field_language_variable."_accounts", $_POST['databasetable'], $_POST['fieldtype'], $extra, $_POST['default'], $_POST['hiddenfield'], $_POST['readonlyfield'],$_POST['extravalue'],$_POST['updatefield'],$_POST['insertfield'],$_POST['mandatoryfield'],$_POST['duplicatefield'],$_POST['fieldWidth'], '', $_POST['allownullfield']);
	...
	if(count($error_msg)==0)
	{
		...
		$s_allow_null = ' NOT NULL';
		if($alterData->allownull) {
			$s_allow_null = ' NULL';
		}
		if($new == 0 && $_POST['fieldtype'] == $_POST['oldfieldtype'] &&  $_POST['allownullfield'] == $_POST['oldallownullfield'] && !in_array($alterData->type, $v_mandatory_update_types))
		{
			//no updates in DB
		...
		} else if($new == 0 && ($_POST['fieldtype'] != $_POST['oldfieldtype'] || $_POST['allownullfield'] != $_POST['oldallownullfield'] || in_array($alterData->type, $v_mandatory_update_types))) {
			...
			{
				$o_main->db->simple_query("ALTER TABLE ".$alterData->database." CHANGE ".$alterData->sqlname." ".$alterData->sqlname." ".$textType.$s_allow_null.$s_field_comment);
			}
			...
			if(!$o_main->db->field_exists($alterData->sqlname, $alterData->database))
			{
				$o_main->db->simple_query("ALTER TABLE ".$alterData->database." ADD COLUMN ".$alterData->sqlname." ".$textType.$s_allow_null.$s_field_comment);
			}
		}
		$makeString = "";
		foreach($v_field_config as $nut)
		{
			...
			$makeString .= $nut->sqlname."¤".$nut->formname."¤".$nut->htmlname."¤".$nut->database."¤".$nut->type."¤".$nut->extra."¤".$nut->default."¤¤¤".$nut->hidden."¤".$nut->readonly."¤".$nut->extravalue."¤".$nut->update."¤".$nut->insert."¤".$nut->mandatory."¤".$nut->duplicate."¤".$nut->fieldwidth."¤".$nut->input_desc."¤".$nut->allownull."¤";
		}
...
if(isset($_GET['editOrder']))
{
	...
	<div id="blocks_fields">
		...
		{
			if(is_array($block) && array_key_exists('sys_name', $block))
			{
				...
				$counter++;
			} else if(is_array($block) && array_key_exists('sys_columns', $block))
			{
				// group
	...
	$o_query= $o_main->db->query("SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE, COLUMN_DEFAULT
	FROM INFORMATION_SCHEMA.COLUMNS
	WHERE table_name = ?", array($v_field_config[$fieldWork]->database));
	$v_field_database_configs = $o_query ? $o_query->result_array() : array();
	if(count($v_field_database_configs) > 0){
		foreach($v_field_database_configs as $v_field_database_config){
			if($v_field_database_config['COLUMN_NAME'] == $v_field_config[$fieldWork]->sqlname) {
				if($v_field_database_config['IS_NULLABLE'] == 'YES'){
					$v_field_config[$fieldWork]->allownull = 1;
				}
			}
		}
	}
	...
	<input type="hidden" name="languageID" value="<?php echo $choosenInputLang;?>" />
	<input type="hidden" name="extraimagedir" value="<?php echo $extraimagedir;?>" />
	<input type="hidden" name="oldallownullfield" value="<?php echo $v_field_config[$fieldWork]->allownull;?>" />
	...
	<tr><td class="fieldname"><?php echo $formText_AllowNull_input;?></td><td>
		<select name="allownullfield">
		<option value="0"<?php echo ($v_field_config[$fieldWork]->allownull==0?" selected":"");?>><?php echo $formText_no_input;?></option>
		<option value="1"<?php echo ($v_field_config[$fieldWork]->allownull==1?" selected":"");?>><?php echo $formText_yes_input;?></option>
		</select>
	</td></tr>

...
$focus_w_limit = 560;
$focus_h_limit = 460;
if(FALSE !== strpos($field[11], 'ema_templ_conf:'))
{
	list($tmp, $s_cnf_field, $field[11]) = explode(":", $field[11], 3);
	$s_sql = "SELECT cnf.`".$s_cnf_field."` AS extra_value FROM `email_marketing_app` AS ema
	JOIN emailtemplate_newsletter_accountconfig AS cnf ON cnf.id = ema.template_accountconfig_id
	WHERE ema.email_template_id = '".$_GET['parentID']."'";
	$o_query = $o_main->db->query($s_sql);
	$v_ext_config = ($o_query && $o_query->num_rows()>0) ? $o_query->row_array() : array();
	if('' != $v_ext_config['extra_value']) $field[11] = $v_ext_config['extra_value'];
}
...
$show_alt_text = (!isset($o_main->app_id) || 0 == (int)$o_main->app_id);
?>
...
if($field[6][$langID] != "")
{
	...
	foreach($data as $obj)
	{
		...
		$popup_img = ($o_main->activate_cdn?'':$extradomaindirroot).$popup_img;		
		if($o_main->activate_cdn){
			$v_token = array(
				'upload_id' => $upload_id,
		...
		<div class="item row">
			<div class="col-md-1 col-xs-2">
				<div class="thumbnail"><a rel="gal_<?php echo $field_ui_id."_".$i;?>" class="<?php echo $field_ui_id;?>_fancy script" href="<?php echo $popup_img.(strpos($popup_img,'uploads/protected/')!==false?($o_main->multi_acc?$s_protected_cdn:$s_protected_local):'');?>"><img class="ptr" src="<?php echo $thumb_img_link.(strpos($thumb_img_link,'uploads/protected/')!==false?($o_main->multi_acc?$s_protected_cdn:$s_protected_local):'');?>"></a></div>
			</div>
			<div class="col-md-9 col-xs-8">
				...
				foreach($image as $x=>$item)
				{
					...
					{
						list($w,$h,$t,$r) = getimagesize(($o_main->activate_cdn?$item:ACCOUNT_PATH."/".rawurldecode($item)));
						if($h>$w)
						{
							$ini_h = $h;
...
$(function () {
	...
	$('#<?php echo $field_ui_id;?>_upload').fileupload({
		...
		url: "<?php echo $o_main->cdn_api_url;?>?param_name=<?php echo $field_ui_id;?>_files&fieldextra=<?php echo $fieldtype?>",
		<?php } else { ?>
		...
		add: function (e, data) {
			...
			<?php if($o_main->activate_cdn) { ?>
			$.getJSON("<?php echo $extradir;?>/input/fieldtypes/<?php echo $field[4];?>/get_next_id.php?accountname=<?php echo $_GET['accountname'];?>&companyID=<?php echo $_GET['companyID'];?>&caID=<?php echo $_GET['caID'];?>", function (result) {
			<?php } ?>
				...
				if(file.error) {
					...
					if($('#<?php echo $field_ui_id;?>_files .items .item:not(.deleted):not(.abortable)').length >= <?php echo $field_ui_id;?>limit)
					{
						...
						foreach($resize_codes as $resize_code)
						{
							...
							?>oTextCol.append('<input type="hidden" class="<?php echo ($tmp[2]=="c"?"handle":"");?>" name="<?php echo $field[1].$ending;?>_img' + <?php echo $field_ui_id;?>counter+ '[]" value="process|' + file.upload_id + '|<?php echo $resize_code;?>|' + file.width + '|' + file.height + '|' + file.url + '" data-device="<?php echo $tmp[4];?>"<?php echo ($o_main->activate_cdn?' data-cdn-token="\' + file.cdn_token + \'"':'');?>/>');<?php
	...
	$('#<?php echo $field_ui_id;?>_files').on('show_handle', function(){
		handle_<?php echo $field_ui_id;?>();
	});
		...
		if(!<?php echo $field_ui_id;?>handler)
		{
			...
			if(handle.length > 0)
			{
				...
				if(option[0] == 'process' && size[2] == 'c')
				{
					...
					var url_prefix = '';
					if(option[5].substr(0, 8) != "https://" && option[5].substr(0, 7) != "http://")
					{
						url_prefix = '<?php echo ($o_main->activate_cdn?'':$extradomaindirroot);?>';
					}
					...
					$<?php echo $field_ui_id;?>image.attr('src','').attr('src', url_prefix + option[5] + '?caID=<?php echo $_GET['caID'];?>&uid=' + option[1] + '<?php echo ($o_main->activate_cdn?"&cdn_token=' + $(".$field_ui_id."handle).data(\"cdn-token\") + '":"&username=".$_COOKIE['username']."&sessionID=".$_COOKIE['sessionID']."&companyID=".$_GET['companyID']."&server_id=".array_shift(explode('.', $_SERVER['HTTP_HOST'])) );?>&_=' + Math.random()).off('load').on('load', function(){
						$(this).cropper({
							strict:false,
							zoomable:false,
							rotatable:false,

...
$error_msg = is_array($fw_session['error_msg']) ? $fw_session['error_msg'] : json_decode($fw_session['error_msg'],true);
...
if($access >= 100 /*all permissions*/ or ($access > 0 and in_array($includefile, $permissionsRead)) or ($access >=10 and in_array($includefile, $permissionsWrite)) )
{
	...
	if(is_array($error_msg) && sizeof($error_msg)>0)
	{
		$print = "";
		...
		$s_sql = "UPDATE session_framework SET error_msg = '' WHERE companyaccessID = '".$o_main->db->escape_str($_GET['caID'])."' AND session = '".$o_main->db->escape_str($_COOKIE['sessionID'])."' AND username = '".$o_main->db->escape_str($_COOKIE['username'])."'".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"");
	...
	if($findBase = opendir(__DIR__."/settings/tables"))
	{
		while($s_file = readdir($findBase))
		{
			...
			if($v_file[0] != "" && $v_file[1] == "php" && (!isset($v_file[2]) || $v_file[2] != "LCK"))
			{
				...
				$l_table_access = intval(isset($vars['moduleTableAccesslevel']) ? $vars['moduleTableAccesslevel'] : 0);
				$l_table_access_admin = intval(isset($vars['moduleTableAdminAccess']) ? $vars['moduleTableAdminAccess'] : 0);
				$l_table_access_sysadmin = intval(isset($vars['moduleTableSystemAdminAccess']) ? $vars['moduleTableSystemAdminAccess'] : 0);
				...
				else if(isset($vars['moduleMainTable']) && "1" == $vars['moduleMainTable'] && 
	...
	if((!isset($moduleCol1MaxWidth) || $moduleCol1MaxWidth == '') && isset($precolumn2ListMaxWidth)) $moduleCol1MaxWidth = $precolumn2ListMaxWidth;
	if((!isset($moduleCol1MinWidth) || $moduleCol1MinWidth == '') && isset($precolumn2ListMinWidth)) $moduleCol1MinWidth = $precolumn2ListMinWidth;
	if((!isset($moduleCol2MaxWidth) || $moduleCol2MaxWidth == '') && isset($preDetailpageMaxWidth)) $moduleCol2MaxWidth = $preDetailpageMaxWidth;
	if((!isset($moduleCol2MinWidth) || $moduleCol2MinWidth == '') && isset($preDetailpageMinWidth)) $moduleCol2MinWidth = $preDetailpageMinWidth;
		...
		if($moduletype == 0)
		{
			?><div class="<?php echo ($fw_column == 1 ? 'module_list':'module_content');?>"><?php
				if($fw_column == 1)
				{
					...
					<div style="padding: 10px 20px;"><?php echo $ob_module_content;?></div><?php
				} else {

<?php
if(isset($o_main->check_table_fields)) return;
$o_main->check_table_fields = TRUE;
...
if($o_main->db->table_exists('accountinfo_external_table_view'))
{
	...
	foreach($o_query->result_array() as $v_row)
	{
		$v_sys_db_check[$v_row['table_name']] = 1;
...
foreach($table_names as $s_table)
{
	if(strlen($s_table))
	{
		...
		if(!$o_main->db->table_exists($s_table))
		{
			...
			if(isset($table_structure[$s_table]))
			{
				foreach($table_structure[$s_table] as $fieldFromFile)
				{
					...
					if($fieldFromFile['id'] == 'origId' || $fieldFromFile['id'] == 'account_id' || $fieldFromFile['id'] == 'app_id'
						|| ($multilanguage == 1 && $fieldFromFile['id'] == $single_language_table.'ID'))
					{
			...
			if(isset($table_structure[$s_table]))
			{
				foreach($table_structure[$s_table] as $fieldFromFile)
				{
					if(in_array($fieldFromFile['id'], $table_fields_from_db))
					{
						...
						if(($fieldFromFile['id'] == 'origId' || $fieldFromFile['id'] == 'account_id' || $fieldFromFile['id'] == 'app_id'
							|| ($multilanguage == 1 && $fieldFromFile['id'] == $single_language_table.'ID'))
						&& !array_key_exists($fieldFromFile['id'].'Idx', $v_db_indexes))
						{
							$s_sql_alter = 'ALTER TABLE '.$o_main->db_escape_name($s_table).' ADD INDEX '.$o_main->db_escape_name($fieldFromFile['id'].'Idx').' ('.$o_main->db_escape_name($fieldFromFile['id']).')';
		...
		if(isset($v_index_structure[$s_table]))
		{
			...
			{
				if('' == $o_main->db_escape_name($v_index[0]))
				{
					$error_msg['error_'.count($error_msg)] = '[DBCheck] Index name not specified in table '.$s_table.' with keys: '.implode(',', $v_index[3]);
					continue;
				}
				if($o_main->multi_acc && ('_basisconfig' != substr($s_table,-12)))
                {
                    $v_index[3][] = 'account_id';
                }
				if($o_main->multi_acc && ('_basisconfig' == substr($table,-12)))
                {
                    $v_index[3][] = 'app_id';
                }

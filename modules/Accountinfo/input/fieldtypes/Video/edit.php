...
if($access>=10)
{
	...
	<div id="<?php echo $field_ui_id;?>_tab_youtube" class="<?php echo $youtubeActive?>">
		<input <?php echo $field_attributes;?> id="<?php echo $field_ui_id;?>" type="text" name="<?php echo $field[1].$ending;?>" value="<?php if(!is_array($dataInit[0])) echo $dataInit[0];?>" />
	</div>
		...
		$(function () {
			...
			$('#<?php echo $field_ui_id;?>_upload').fileupload({
				...
				add: function (e, data) {
					...
					$.getJSON("<?php echo $extradir;?>/input/fieldtypes/<?php echo $field[4];?>/get_next_id_file.php", function (result) {
		...
		<div class="imageWrapper">
			...
			$(function () {
				...
				$('#<?php echo $field_ui_id;?>_imageupload').fileupload({
			        <?php if($b_activate_cdn) { ?>
					url: "<?php echo $variables->fw_session['content_server_api_url'];?>?param_name=<?php echo $field_ui_id;?>_images&fieldextra=<?php echo $fieldtype?>",
					<?php } else { ?>
					...
					<?php if($b_activate_cdn) { ?>
					add: function (e, data) {
						$.getJSON("<?php echo $extradir;?>/input/fieldtypes/<?php echo $field[4];?>/get_next_id_image.php", function (result) {
							data.formData = result;
							data.submit();
						});
					},
						...
						$.each(data.result.<?php echo $field_ui_id;?>_images, function (index, file) {
							if(file.error) {
								...
								if($('#<?php echo $field_ui_id;?>_images .item:not(.deleted)').length >= <?php echo $field_ui_id;?>limit)
								{
									...
									var oThumbImg = $('<img/>').attr('src', '<?php echo ($b_activate_cdn?'':$extradomaindirroot);?>' + file.thumbUrl + '?username=<?php echo $_COOKIE['username'];?>&sessionID=<?php echo $_COOKIE['sessionID'];?>&companyID=<?php echo $_GET['companyID'];?>&caID=<?php echo $_GET['caID'];?>&uid=' + file.upload_id + '&server_id=<?php echo array_shift(explode('.', $_SERVER['HTTP_HOST']));?>');
				...
				function handle_<?php echo $field_ui_id;?>()
				{
					...
					if(handle.length > 0)
					{
						...
						if(option[0] == 'process' && size[2] == 'c')
						{
							...
							$<?php echo $field_ui_id;?>image.attr('src', '<?php echo ($b_activate_cdn?'':$extradomaindirroot);?>' + option[5] + '?username=<?php echo $_COOKIE['username'];?>&sessionID=<?php echo $_COOKIE['sessionID'];?>&companyID=<?php echo $_GET['companyID'];?>&caID=<?php echo $_GET['caID'];?>&uid=' + option[1] + '&_=' + Math.random()).cropper({
								strict:false,
								zoomable:false,
								rotatable:false,
	...
	<script type="text/javascript">
		$(".video_settings_edit").unbind("click").bind("click", function(){
		...
		$("#<?php echo $field_ui_id;?>_tab_changer .youtube_tab").unbind("click").bind("click", function(){
		...
		$("#<?php echo $field_ui_id;?>_tab_changer .file_tab").unbind("click").bind("click", function(){

...
{
	$s_sql = "SELECT c.".$options[1]." cid, c.*, cc.*".$extraSelect." FROM ".$options[0]." c LEFT OUTER JOIN ".$options[0]."content cc ON c.id = cc.".$options[0]."ID AND cc.languageID = ".$o_main->db->escape($s_default_output_language).$extraJoin." WHERE c.content_status < 2";
	
} else {
	$s_sql = "SELECT c.".$options[1]." cid, c.*".$extraSelect." FROM ".$options[0]." c".$extraJoin." WHERE c.content_status < 2";
}
$s_sql_search = "";

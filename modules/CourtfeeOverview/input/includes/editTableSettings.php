...
$useVars = array(
	...
	"overwriteFrameworkContentmaxWidth" => "18",
...
<form action="<?php echo $extradir."/input/includes/editTableSettings.php?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&submodule=".$submodule;?>" method="post">
	...
	foreach($useVars as $key => $value)
	{
		switch($value)
		{
			...
			case 20:
				?><div class="tableSettingsRow config-<?php echo $key;?>">
					...
					<div class="tableSettingsContent">
						...
						<div class="alert alert-warning"><b>Warning!</b> Be aware that URL update functionality for <b>existing content</b> will not work corectly (could also cause existing URL lose) when changing <b>seoUrlEditType</b> from <b>Full url edit</b> to other or oposite.</div>
					</div>
				</div><?php
				break;
			case 21:
			...
			case 25:
				$v_options = array(0=>"Normal", 1=>"Use full width and hide menu");
				?>
				<div class="tableSettingsRow config-<?php echo $key;?>">
					<div class="tableSettingsLabel"><?php echo str_replace("pre","",$key);?></div>
					<div class="tableSettingsContent">
		...
		<?php foreach($value as $key2=>$value2){
			switch($value2){
				...
				case 100:
					...
					$(".group-<?php echo $key?> .fieldGroupWidth input").off("change").on('change', function(){
				...
				case 104:
					if($addBtn){
						...
						<script type="text/javascript">
						$(".group-<?php echo $key?> .fieldsetAddBtn").off("click").on("click", function(){

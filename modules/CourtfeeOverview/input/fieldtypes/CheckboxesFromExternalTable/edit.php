...
if(!$o_main->db->table_exists($options[3]))
{
	$b_table_created = $o_main->db->simple_query("CREATE TABLE `{$options[3]}` (
		...
		".($o_main->multi_acc?"`account_id` INT(11) NOT NULL,":"")."
		INDEX `Relation` (`{$options[4]}`, `{$options[5]}`, `contentTable`".($o_main->multi_acc?", `account_id`":"").")
	)");
	if(!$b_table_created)
	{
...
if($o_main->multi_acc && !$o_main->db->field_exists('account_id', $options[3]))
{
	$o_main->db->query("ALTER TABLE ".$options[3]." ADD COLUMN account_id INT NOT NULL DEFAULT 0");
}
...
{
	$s_sql = 'SELECT c.'.$options[1].' col0, cc.'.$options[2].' col1, r.'.$options[4].' col2, c.content_status FROM '.$options[0].' c
	JOIN '.$options[0].'content cc ON cc.'.$options[0].'ID = c.id AND cc.languageID = '.$o_main->db->escape($s_default_output_language).'
	LEFT JOIN '.$options[3].' r on r.'.$options[4].' = '.$o_main->db->escape($ID).' AND c.'.$options[1].' = r.'.$options[5].' AND r.contentTable = '.$o_main->db->escape($tablename).($o_main->multi_acc?" AND r.account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"");
} else {
	$s_sql = 'SELECT c.'.$options[1].' col0, c.'.$options[2].' col1, r.'.$options[4].' col2, c.content_status FROM '.$options[0].' c LEFT JOIN '.$options[3].' r ON r.'.$options[4].' = '.$o_main->db->escape($ID).' AND c.'.$options[1].' = r.'.$options[5].' AND r.contentTable = '.$o_main->db->escape($tablename).($o_main->multi_acc?" AND r.account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"");
}

$recursive = false;
$s_sql_where = ($o_main->multi_acc?" WHERE c.account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"");
$v_param = array();
if(isset($options[6]) and strlen($options[6])>0)
{
	...
	if(isset($options[7]) and strlen($options[7])>0)
	{
		...
		$s_sql_where .= (''!=$s_sql_where?' AND':' WHERE').' c.'.$options[6].' IN ?';
	} else {
		$s_sql_where .= (''!=$s_sql_where?' AND':' WHERE').' c.'.$options[6].' = 0';
	}
}
$s_sql_where .= (''!=$s_sql_where?' AND':' WHERE')." c.content_status < 2";
	...
	foreach($o_query->result_array() as $row)
	{
		if($field[9] == 1)
		{
			...
			?><div>
				...
				<label><?php echo htmlentities($row['col1'].($row['content_status']==1 ? " - [inactive]":""));?></label><?php
				if($recursive) $x = print_checkboxes_from_external_table(1, $s_sql, $options, $row['col0'], $field, $ending, $access);
			?></div><?php
		}
	}

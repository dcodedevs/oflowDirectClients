<?php
$claimline_id = $_POST['claimline_id'] ? $o_main->db->escape_str($_POST['claimline_id']) : 0;
$action = $_POST['action'] ? $_POST['action'] : '';
$status = $_POST['status'] ? $_POST['status'] : 0;

$sql = "SELECT * FROM collecting_company_cases_claim_lines WHERE id = $claimline_id";
...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($claimline_id) {
            ...
            $sql = "UPDATE collecting_company_cases_claim_lines SET
            updated = now(),
            updatedBy='".$variables->loggID."',
			court_fee_released_date='".date("Y-m-d", strtotime($o_main->db->escape_str($court_fee_released_date)))."'
            WHERE id = $claimline_id";
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_release_date";?>" method="post">
		<input type="hidden" name="fwajax" value="1">
		<input type="hidden" name="fw_nocss" value="1">
		<input type="hidden" name="output_form_submit" value="1">
		<input type="hidden" name="claimline_id" value="<?php echo $claimline_id;?>">
		...
		<div class="inner">
    		...
    		<div class="line">
        		<div class="lineTitle"><?php echo $formText_ReleaseDate_Output; ?></div>
        		<div class="lineInput">
                    <input type="text" class="popupforminput botspace datefield" autocomplete="off" name="court_fee_released_date" value="<?php if($projectData['court_fee_released_date'] != "0000-00-00" && $projectData['court_fee_released_date'] != "") echo date("d.m.Y", strtotime($projectData['court_fee_released_date'])); ?>" >
                </div>
        		<div class="clear"></div>
    		</div>

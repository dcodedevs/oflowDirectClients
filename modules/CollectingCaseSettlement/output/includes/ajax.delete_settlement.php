...
if($settlement_id > 0){
	...
	if($last_settlement['id'] == $settlement_id) {
	    $s_sql = "DELETE FROM collectingcompany_settlement WHERE id = '".$o_main->db->escape_str($settlement_id)."'";
	    $o_query = $o_main->db->query($s_sql);
	    if($o_query) {
	        $s_sql = "UPDATE collecting_cases_payments SET settlement_id = null WHERE settlement_id = '".$o_main->db->escape_str($settlement_id)."'";
	        $o_query = $o_main->db->query($s_sql);
	    }
	}
}

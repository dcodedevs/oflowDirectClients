...
{
	$sql = "select c.{$options[1]} cid, c.*, cc.* $extraSelect from {$options[0]} c LEFT OUTER JOIN {$options[0]}content cc ON c.id = cc.{$options[0]}ID AND cc.languageID = ".$o_main->db->escape($s_default_output_language)." $extraJoin WHERE c.content_status < 2 ORDER BY c.sortnr";
	
} else {
	$sql = "select c.{$options[1]} cid, c.* $extraSelect from {$options[0]} c $extraJoin WHERE c.content_status < 2 ORDER BY c.sortnr";
}

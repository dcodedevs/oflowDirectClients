...
if(isset($_POST['id'], $_POST['table'], $_POST['moduleid'], $_POST['time']))
{
	...
	{
		?><div class="<?php echo $_POST['field_ui_id'];?>_report"><span class="name"><?php echo $user['receiver'];?></span><span class="mobile"><?php echo $user['receiver_mobile'];?></span><span class="performed"><?php echo ($user['status']>0 ? $user['perform'] : '');?></span><img src="<?php echo $_POST['dir'];?>images/status<?php echo $user['status'];?>.png" border="0" title="<?php echo ($user['status'] == 3 ? $formText_errorOccured_fieldtype.': '.str_replace('"',"'",$user['status_msg']) : '');?>" <?php echo ($user['status']==3 ? 'onClick="javascript:alert($(this).attr(\'title\'));"' : '');?> /> <?php echo ($user['status'] == 1 ? $formText_Sending_fieldtype : ($user['status'] == 2 ? $formText_SuccessfullySent_fieldtype : $formText_errorOccured_fieldtype));?></div><?php
	}

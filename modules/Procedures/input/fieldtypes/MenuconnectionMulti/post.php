...
foreach($databases as $fieldbasetable)
{
	...
	{
		if($fields[$fieldnums][4] == 'MenuconnectionMulti')
		{
			$o_main->db->query("DELETE pageIDcontent FROM pageIDcontent INNER JOIN pageID ON pageID.id = pageIDcontent.pageIDID WHERE pageID.contentID = ? AND pageID.contentTable = ? AND pageID.deleted = 1", array($fieldbasetable->ID, $fieldbasetable->name));
			$o_main->db->query("DELETE pageID, pageIDcontent FROM pageID LEFT JOIN pageIDcontent ON pageID.id = pageIDcontent.pageIDID WHERE pageID.contentID = 0");
			$o_main->db->query("UPDATE pageID SET deleted = 1 WHERE contentID = ? AND contentTable = ?", array($fieldbasetable->ID, $fieldbasetable->name));
			foreach($fields[$fieldnums][30]['all'] as $levelID)
			{
				$o_query = $o_main->db->query('SELECT id FROM pageID WHERE contentID = ? AND contentTable = ? AND deleted = 1', array($fieldbasetable->ID, $fieldbasetable->name));
				if($o_query && $o_query->num_rows()>0)
				{
					$v_old_row = $o_query->row_array();
					// var_dump($v_old_row);
					$o_main->db->query("UPDATE pageID SET menulevelID = ?, deleted = 0 WHERE id = ?", array($levelID, $v_old_row['id']));
				} else {
					// var_dump("1");
					$o_main->db->query("INSERT INTO pageID(contentID, contentTable, menulevelID) VALUES(?, ?, ?)", array($fieldbasetable->ID, $fieldbasetable->name, $levelID));
				}
			}
		}
	}
...
if(isset($curField))
{
	...
	if($curField[3]==$s_signle_table) {
		$modRewriteSql = "select {$s_signle_table}.{$curField[0]} url_rewrite_name, {$s_signle_table}.seotitle, {$s_signle_table}.seodescription, pageID.menulevelID, menulevelcontent.levelname, ".$o_main->db->escape($s_default_output_language)." languageID, pageID.id pageID from pageID join {$s_signle_table} on {$s_signle_table}.id = pageID.contentID AND pageID.contentTable = ".$o_main->db->escape($s_signle_table)." left outer join menulevelcontent on pageID.menulevelID = menulevelcontent.menulevelID AND menulevelcontent.languageID = ".$o_main->db->escape($s_default_output_language)." WHERE {$s_signle_table}.id = ".$o_main->db->escape($basetable->ID)." AND pageID.deleted != 1";
		$s_seo_field_cleanup_sql = "update {$s_signle_table} set seourl='', seotitle='', seodescription = '' where id = ".$o_main->db->escape($basetable->ID);
	} else {
		$modRewriteSql = "select {$s_signle_table}content.{$curField[0]} url_rewrite_name, {$s_signle_table}content.seotitle, {$s_signle_table}content.seodescription, pageID.menulevelID, menulevelcontent.levelname, {$s_signle_table}content.languageID, pageID.id pageID from pageID join {$s_signle_table}content on {$s_signle_table}content.{$s_signle_table}ID = pageID.contentID AND pageID.contentTable = ".$o_main->db->escape($s_signle_table)." left outer join menulevelcontent on pageID.menulevelID = menulevelcontent.menulevelID AND {$s_signle_table}content.languageID = menulevelcontent.languageID WHERE {$s_signle_table}content.{$s_signle_table}ID = ".$o_main->db->escape($basetable->ID)." AND pageID.deleted != 1";
		$s_seo_field_cleanup_sql = "update {$s_signle_table}content set seourl='', seotitle='', seodescription = '' where {$s_signle_table}ID = ".$o_main->db->escape($basetable->ID);
	}
	//print $modRewriteSql;
	...
	foreach($o_query->result_array() as $modRewriteRow)
	{
		...
		$o_check = $o_main->db->query($s_sql, array($modRewriteName.$rand, $basetable->ID, $s_signle_table));
		...
		if($o_check && $o_check->num_rows()>0)
		{
			$o_main->db->query("update pageIDcontent set urlrewrite = ?, lang_url_part = ?, menu_url_part = ?, content_url_part = ? where pageIDID = ? and languageID = ?", array($modRewriteName.$rand, $s_lang_url_part, $s_menu_url_part, $s_content_url_part, $modRewriteRow['pageID'], $modRewriteRow['languageID']));
		} else {
			$o_main->db->query("insert into pageIDcontent (pageIDID, languageID, urlrewrite, lang_url_part, menu_url_part, content_url_part) values(?, ?, ?, ?, ?, ?)", array($modRewriteRow['pageID'], $modRewriteRow['languageID'], $modRewriteName.$rand, $s_lang_url_part, $s_menu_url_part, $s_content_url_part));
		}

...
foreach($databases as $fieldbasetable)
{
	foreach($fieldbasetable->fieldNums as $fieldnums)
	{
		if($fields[$fieldnums][4] == 'MenuconnectionAdv')
		{
			...
			{
				$o_query = $o_main->db->query('SELECT id FROM pageID WHERE contentID = ? AND contentTable = ?', array($fieldbasetable->ID, $fieldbasetable->name));
				if($o_query && $o_query->num_rows()>0)
				{
					$v_old_row = $o_query->row_array();
					$o_main->db->query("UPDATE pageID SET menulevelID = ? WHERE id = ?", array($levelID, $v_old_row['id']));
				} else {

...
?><div class="fieldname"><?php echo $formText_chooseMenu_fieldtype;?></div>
<select <?php echo $field_attributes;?> id="<?php echo $field_ui_id."level-1";?>" onchange="javascript:fill_menulevel_<?php echo $field_ui_id;?>('<?php echo $field_ui_id."level"; ?>','-1');" <?php echo ($field[10]==1||$access<10?" disabled":"");?>><?php
...
for($t = 0; $t < $level; $t++)
{
	...
	<td class="menudropdown">
		...
		foreach($o_query->result_array() as $row)
		{
			?><option value="<?php echo $row['cid'];?>"<?php echo ($parents[$t]==$row['cid']?" selected":"");?>><?php echo $row[$menu_settings['prefieldInList']].($row['content_status']==1 ? " - [inactive]" : ""); ?></option><?php
		}
		?></select>
	</td><?php
}	

...
$o_query = $o_main->db->query('SELECT id FROM pageID WHERE contentID = ? AND contentTable = ?', array($basetable->ID, $s_signle_table));
if($o_query && $o_query->num_rows()>0)
{
	...
	$o_main->db->query("INSERT INTO pageID(contentID, contentTable, menulevelID) VALUES(?, ?, ?)", array($basetable->ID, $s_signle_table, $fields[$nums][6]['all']));
}
...
if(isset($curField))
{
	...
	$o_check = $o_main->db->query("select id from ".$s_signle_table." WHERE id = ? AND disable_menuconnection = 1", array($basetable->ID));
	...
	$o_query = $o_main->db->query($modRewriteSql, array($basetable->ID, $s_signle_table));
	if($o_query && $o_query->num_rows()>0)
	foreach($o_query->result_array() as $modRewriteRow)
	{
	...
	if($b_disable_menuconnection)
	{
		$o_main->db->query("UPDATE pageID SET deleted = 1 WHERE contentID = ? AND contentTable = ?", array($basetable->ID, $s_signle_table));
	}
}
?>
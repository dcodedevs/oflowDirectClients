	...
	for($x = 0; $x < sizeof($prerelations); $x++)
	{
		...
		if($showMe[$prerelationsarray[2]] != 1)
		{
			if($prerelationsarray[4] == 1)
			{
				...
				$relationvalue = array();
				$o_query = $o_main->db->query('SELECT '.$submodule.'.* FROM '.$submodule.' WHERE '.$submodule.'.id = ?', array($_GET['ID']));
				...
				$prelistButtonCreate = $preinputformName = $preexpandSublist = "";
				$childmodule = array();
				...
				if(isset($prerelationsarray[10])) $preperPage = $prerelationsarray[10];
				...
				if($preorderByField != '0') $orderbyfield = " ORDER BY ".$relationstable.".".$o_main->db_escape_name($preorderByField);
				...
				if($ob_buttons!="")
				{
					?><div class="btn btn-sm dropdown"><a id="<?php echo "sl_btn_".$prerelationsarray[1]."_".$prerelationsarray[2]."_".$rand_id;?>" class="script" data-target="#" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon-menu-hamburger glyphicon-white"></span></a><ul class="dropdown-menu" role="menu" aria-labelledby="<?php echo "sl_btn_".$prerelationsarray[1]."_".$prerelationsarray[2]."_".$rand_id;?>"><?php echo $ob_buttons;?></ul></div><?php
				}
				$ob_buttons = ob_get_clean();
				...
				.subcontent-list.sublist<?php echo $x?> .col-md-1 {
					width: 10%;
					padding: 0;
				}
									...
									?><div class="input_list_item row" data-group="input_list_item"><?php
												...
												foreach($finList as $key=>$finList)
												{
													...
													if(is_file(ACCOUNT_PATH."/modules/".$prerelationsarray[1]."/input/fieldtypes/".$linkersField[$key][4]."/customlistdisplay.php"))
													{
														...
														if(isset(${$invoiceName}))
														{
															...
															print '<a href="'.$_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&"."module=".$prerelationsarray[1]."&ID=".$content_id."&parentID=".$_GET['ID']."&subcontent=1&includefile=edit&submodule=".$prerelationsarray[2]."&relationID=".$relationvalue[$prerelationsarray[0]]."&relationfield=".$prerelationsarray[3].($s_content_status_filter!="" ? "&content_status=".$s_content_status_filter : "").'" class="optimize" data-target="#content_'.$prerelationsarray[1].'_'.$prerelationsarray[2].'_'.$content_id.'"> '.$value.'</a>';
										...
										if($item_name == "")
										{
											...
											foreach($linkers as $key => $finList)
											{
												...
												{
													?><div class="column column<?php echo $counter;?>"><?php
													if($counter==0) print '<a href="'.$_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&"."module=".$prerelationsarray[1]."&ID=".$content_id."&parentID=".$_GET['ID']."&subcontent=1&includefile=edit&submodule=".$prerelationsarray[2]."&relationID=".$relationvalue[$prerelationsarray[0]]."&relationfield=".$prerelationsarray[3].($s_content_status_filter!="" ? "&content_status=".$s_content_status_filter : "").'" class="optimize" data-target="#content_'.$prerelationsarray[1].'_'.$prerelationsarray[2].'_'.$content_id.'"> '.$item_name.'</a>';
													?></div><?php
													$counter++;
												}
												?></div><?php
										...
										<div class="list_buttons popup_button_box col-md-1 dropdown">
											<a id="<?php echo "sl_btn_".$prerelationsarray[1]."_".$prerelationsarray[2]."_".$content_id;?>" class="script" data-target="#" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">

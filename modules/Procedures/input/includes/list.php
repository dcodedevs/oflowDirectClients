...
if(isset($_GET['search']) && $_GET['search'] != '')
{
	...
	if($searchFieldName != '')
	{
		...
		$o_query = $o_main->db->query('SHOW INDEX FROM '.$soker->mainTable.' WHERE Column_name = ?', array($searchFieldName));
		if($o_query && $o_query->num_rows()>0)
		{
			$o_main->db->simple_query('CREATE FULLTEXT INDEX '.$searchFieldName.'FullIdx ON '.$soker->mainTable.'('.$searchFieldName.')');
		}
	} else {
		foreach($fields as $felt)
		{
			...
			{
				$soker->searchConditions[] = array($searchword,'LIKE',$felt[3],$felt[0]);
			}
		}
	}
...
<div class="list_content">
				...
				$(".list_content .searchField").keyup(function(){
					...
					delay(function(){   
						...
						$.ajax({
							...
							data: {soker: '<?php echo json_encode($soker); ?>',filter1: filter1, filter2:filter2, filter3: filter3, search: searchText, searchFieldName: "<?php echo $searchFieldName?>", searchType: "<?php echo $searchType?>", 
							orderByField: "<?php echo $orderByField?>",	orderByDesc: "<?php echo $orderByDesc?>", perPage: "<?php echo $perPage?>", choosenListInputLang: "<?php echo $choosenListInputLang;?>", 
								...
								selfurl: "<?php echo $_SERVER['PHP_SELF']?>", fields: '<?php echo json_encode($fields)?>',
	...
	if($showFilterByMenulevelField == 1 && $filterModuleID != null)
	{
		...
		WHERE menulevelcontent.languageID = ".$o_main->db->escape($s_default_output_language)." AND menulevel.content_status < 2 
		AND menulevel.level = 0 AND menulevel.moduleID = ".$o_main->db->escape($filterModuleID)." ORDER BY menulevel.sortnr";
		...
		if($o_query && $menulevels = $o_query->result_array())
		{
			...
			?>
						...
						<div class="selectDiv">
							<div class="selectDivWrapper">
								<select name="filter" class="filterBy">
									...
									<option value="<?=$menulevel['id']?>" <?php if($menulevel['id'] == $_SESSION['filter1']) echo 'selected';?>><?=$menulevel['levelname']?></option>
									<?php } ?>
								</select>
				...
				<script type="text/javascript">
					function bindFilter()
					{
						$(".list_content .menu_filter .filterBy").unbind("change").bind("change", function(){
							var cur_level = $(this).closest('.menu_filter').data('level');
							menu_filter_update(1, cur_level, cur_level);
						})
					}
					function menu_filter_update_list(filter1, filter2, filter3)
					{
						var saveFilter = true;
						...
						$.ajax({
							...
							data: {
								soker: '<?=json_encode($soker); ?>', filter1: filter1, filter2: filter2, filter3: filter3,
								searchFieldName: "<?=$searchFieldName?>", searchType: "<?=$searchType?>", orderByField: "<?=$orderByField?>",
								orderByDesc: "<?=$orderByDesc?>", perPage: "<?=$perPage?>", choosenListInputLang: "<?=$choosenListInputLang;?>", 
								...
								selfurl: "<?=$_SERVER['PHP_SELF']?>", fields: '<?=json_encode($fields)?>', saveFilter: saveFilter
							},
							success: function(data){
								var object = jQuery.parseJSON(data);
								$(".list_content .listItems").html(object.html);
					...
					function menu_filter_update(counter, from_level, to_level)
					{
						if($(".list_content .menu_filter."+counter).length == 0)
						{
							...
							var filter2 = $(".list_content .menu_filter.2 .filterBy option:selected").val();
							var filter3 = $(".list_content .menu_filter.3 .filterBy option:selected").val();
							menu_filter_update_list(filter1, filter2, filter3);
							bindFilter();
							return;
						}
					...
					{
						?>
						menu_filter_update_list(<?php echo intval($_SESSION['filter1']).', '.intval($_SESSION['filter2']).', '.intval($_SESSION['filter3']);?>);
	...
	<div class="input_list_item head">
		...
		foreach($linkers as $key => $outlink)
		{
			foreach($outlink as $key => $outlink)
			{
				...
				?><div class="column t<?php echo $data_columns;?><?php echo ($counter>0?" next":"");?>"><a href="<?php echo $linkUse;?>" _class="optimize"><?php echo (isset($linkersField[$key][2]) ? $linkersField[$key][2] : '[no_label]');?></a></div><?php
				$counter++;
			}
		}
		?></div>
	...
	<div class="listItems">
		...
		if($o_get_content && $o_get_content->num_rows()>0)
		{
			foreach($o_get_content->result_array() as $writeContent)
			{
				...
				?><div class="input_list_item activate<?php echo ($content_id==$_GET['ID'] ? ' active' : '')." ".$v_content_status_class[$writeContent['content_status']];?>" data-group="input_list_item">
					<div class="input_list_data"><?php
					ob_start();
					$setCounter = 1;
					$item_name = "";
						...
						foreach($finList as $key => $finList)
						{
							...
							if(is_file(__DIR__."/../fieldtypes/".$linkersField[$key][4]."/customlistdisplay.php"))
							{
								...
								else if(isset(${$varJson}))
								{
									...
									print '<a href="'.$editLink.'" class="optimize">'.$value.'</a>';
									...
									if($access>=10 and isset(${$varName}) && ${$varName}["type"] == "checkbox")
									{
										...
										print "<a href=\"".$editLink."\" class=\"optimize\">";
									}
									print (isset(${$varName}) ? ${$varName}[$writeContent[$finList]] : $writeContent[$finList]);
									print "</a>"; 
					...
					if($item_name == "")
					{
						...
						foreach($linkers as $key => $finList)
						{
							...
							{
								?><div class="column column<?php echo $counter;?> <?php echo ($counter>0?" next":"");?>"><?php
								if($counter==0) print '<a href="'.$editLink.'" class="optimize">'.$item_name.'</a>';
								?></div><?php
								$counter++;
							}
							?>
					...
					<div class="list_buttons popup_button_box dropdown">
						...
						{
							?><li><a href="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&ID=".$content_id."&includefile=sendEmail&submodule=".$submodule.(isset($_GET['relationID']) ? "&relationID=".$_GET['relationID']."&parentID=".$_GET['relationID'] : "").(isset($_GET['relationfield']) ? "&relationfield=".$_GET['relationfield'] : "").(isset($_GET["content_status"]) ? "&content_status=".$_GET['content_status'] : "");?>" class="optimize" role="menuitem"><?php echo $formText_sendEmail_list;?></a></li><?php
						}
						
						if($showSendSms == 1 and $access >= 10)
						{
							?><li><a href="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&ID=".$content_id."&includefile=sendSms&submodule=".$submodule.(isset($_GET['relationID']) ? "&relationID=".$_GET['relationID']."&parentID=".$_GET['relationID'] : "").(isset($_GET['relationfield']) ? "&relationfield=".$_GET['relationfield'] : "").(isset($_GET["content_status"]) ? "&content_status=".$_GET['content_status'] : "");?>" class="optimize" role="menuitem"><?php echo $formText_sendSms_list;?></a></li><?php
						}
						
						if($showSendPdf == 1 and $access >= 10)
						{
							?><li><a href="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&ID=".$content_id."&includefile=sendPdf&submodule=".$submodule.(isset($_GET['relationID']) ? "&relationID=".$_GET['relationID']."&parentID=".$_GET['relationID'] : "").(isset($_GET['relationfield']) ? "&relationfield=".$_GET['relationfield'] : "").(isset($_GET["content_status"]) ? "&content_status=".$_GET['content_status'] : "");?>" class="optimize" role="menuitem"><?php echo $formText_sendPdf_list;?></a></li><?php
						}
...
function bindScroll(){
	...
	$scrollbar.unbind("move").bind("move", function() {
		...
		if(!loadingData && positionCurrent >= positionEnd) {
			...
			$.ajax({
				...
				orderByField: "<?php echo $orderByField?>",	orderByDesc: "<?php echo $orderByDesc?>", perPage: "<?php echo $perPage?>", choosenListInputLang: "<?php echo $choosenListInputLang?>", 
					...
					listSet3FieldVariables: '<?php echo json_encode($listSet3FieldVariables); ?>',listSet4FieldVariables: '<?php echo json_encode($listSet4FieldVariables); ?>',
					module: "<?php echo $module?>", submodule: "<?php echo $submodule?>",
					selfurl: "<?php echo $_SERVER['PHP_SELF']?>", accountPath: "<?php echo ACCOUNT_PATH?>", fields: '<?php echo json_encode($fields)?>',

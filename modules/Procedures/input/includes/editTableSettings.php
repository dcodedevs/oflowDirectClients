...
$ignoreMe = array("donotdeleteever","mysqlTableName","prechildmodule","preinputformName","preinputformDescription","preparentmodule","preinputLevel","thisLevel","numberLevels","tableordernr");
$useVars = array(
			"List in column 2" => "-1",
...
<form action="<?php echo $extradir."/input/includes/editTableSettings.php?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&submodule=".$submodule;?>" method="post">
	...
	foreach($useVars as $key => $value)
	{
		switch($value)
		{
			case -2:
			?>
				<div class="tableSettingsRow"><h4><?php echo str_replace("pre","",$key);?></h4></div>
			<?php
				break;
			case -1:
			?>				
				<div class="tableSettingsRow"><h2><?php echo str_replace("pre","",$key);?></h2></div>
			<?php
				break;
			case 0:
			?>
			...
			case 16:
				?>
				<div class="tableSettingsRow orderTypeRow">
					<div class="tableSettingsLabel"><?php echo str_replace("pre","",$key);?></div>
					<div class="tableSettingsContent">
						<select style="width:400px;" name="<?php echo $key; ?>" class="form-control input-sm">
				...
				$(".orderTypeRow select").change(function(){
					if($(this).val() == 0){
						$(".orderRow select").prop("disabled", true).hide();
						$(".orderRow input").prop("disabled", false).show();
					}else{
						$(".orderRow select").prop("disabled", false).show();
						$(".orderRow input").prop("disabled", true).hide();

					}
				})					
				$(".orderTypeRow select").change();
			...
			case 17:
				?>
				<div class="tableSettingsRow orderRow">
					<div class="tableSettingsLabel"><?php echo str_replace("pre","",$key);?></div>
					<div class="tableSettingsContent">
						<select style="width:400px;" name="<?php echo $key; ?>" class="form-control input-sm">
							<option value=""><?php echo $formText_none_input;?></option>
	...
	function fieldListSettingReturn($key, $value, $variableVal, $fields, $formText_none_input, $addBtn, $formText_Add_input){
		...
		<?php foreach($value as $key2=>$value2){
			switch($value2){
				case -100:
					?>
					<div class="tableSettingsRow"><h4><?php echo str_replace("pre","",$key2);?></h4></div>
					<?php
					break;
				case 100:
					?>
				...
				case 102:
					?>
					<div class="tableSettingsRow fieldGroup">
						<div class="tableSettingsLabel"><?php echo str_replace("pre","",$key2);?></div>
						<div class="tableSettingsContent">
				...
				case 103:
					?>
					<div class="tableSettingsRow fieldGroupWidth">
						<div class="tableSettingsLabel"><?php echo str_replace("pre","",$key2);?></div>
						<div class="tableSettingsContent">
							<div class="inlineBlock">

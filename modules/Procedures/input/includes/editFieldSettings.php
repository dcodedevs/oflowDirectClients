...
if(isset($_POST['send']))
{
	...
	{
		if($new == 0 &&  $_POST['fieldtype'] ==  $_POST['oldfieldtype']
		&& ($alterData->type != 'Decimal') )
		{
			//no updates in DB
			$v_field_config[$fieldWork] = $alterData;
		} else if($new == 0 && ($_POST['fieldtype'] != $_POST['oldfieldtype'] || $alterData->type == 'Decimal') ) {
			//alter field in DB
			$v_field_config[$fieldWork] = $alterData;
			$textType = "";
			...
			$o_main->db->simple_query("ALTER TABLE ".$alterData->database." CHANGE ".$alterData->sqlname." ".$alterData->sqlname." ".$textType);
		} else {
			...
			$o_main->db->simple_query("ALTER TABLE ".$alterData->database." ADD COLUMN ".$alterData->sqlname." ".$textType);
		} 
		...
		foreach($v_field_config as $nut)
		{
			...
			$makeString .= $nut->sqlname."¤".$nut->formname."¤".$nut->htmlname."¤".$nut->database."¤".$nut->type."¤".$nut->extra."¤".$nut->default."¤¤¤".$nut->hidden."¤".$nut->readonly."¤".$nut->extravalue."¤".$nut->update."¤".$nut->insert."¤".$nut->mandatory."¤".$nut->duplicate."¤".$nut->fieldwidth."¤";
		}
		//echo $makeString;
		//exit; 
...
if(isset($_POST['savefieldstructure']))
{
	...
	{
		if($content[$i] == '$' && (substr($content,$i,10)=='$formText_' || substr($content,$i,10)=='$formText_'))
		{
			$found=true;
			$content[$i-1] = " ";
		}
	...
	//print_r($content);print_r($_POST);exit;
...
if(isset($_GET['editOrder']))
{
	...
	$(function() {
		refresh_sortable()
		$("#edit_add_block").unbind("click").on("click", function() {
		...
		$("#edit_add_group").unbind("click").on("click", function() {
			fw_changes_made = true;
			fw_field_counter++;
	...
	function refresh_sortable()
	{
		...
		$("#sort_blocks li .title .buttons .btn").unbind("click").on("click", function(e) {
			e.preventDefault();
		...
		$("#sort_blocks .delete-field").unbind("click").on("click",function(e){
			e.preventDefault();
	...
	foreach($typeDirFiles as $i => $typeDirFile)
	{
		...
		{
			$selected = false;
			if($v_field_config[$fieldWork]->type==$typeDirFile || (!isset($v_field_config[$fieldWork]->type) && $i==0))
			{
				$selected = true;
				$info_idx = $i;
			}
			$field_info[] = /*$thisFieldType.' - '.*/$thisAboutInfo;

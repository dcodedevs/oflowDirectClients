...
if(count($db_languages_titles)>0)
{
	...
	if(isset($_GET['actionType']) and $_GET['actionType'] == 'saveInputLanguageChanges')
	{
		...
		foreach($output_folders as $output_dir)
		{
			...
			foreach($db_languages as $db_language)
			{
				...
				ftp_copy($lang_file_with_sufix,$lang_file_with_sufix_bckp);
	...
	//prints out form
	?>
	<form name="updateLanguages" id="updateLanguages" action="?<?php echo get_curent_GET_params(array("actionType","autoTranslate"));?>&actionType=saveInputLanguageChanges&selectedLanguage=<?php echo (isset($_GET['selectedLanguage']) ? $_GET['selectedLanguage'] : '');?>" method="POST">
	<input type="hidden" value="<?php echo (isset($_GET['choosenInputLang']) ? $_GET['choosenInputLang'] : '');?>" name="updateLang" />
	
	<div style="margin:10px;"><input type="button" onClick="javascript: document.location.href='?<?php echo get_curent_GET_params(array("autoTranslate","actionType"));?>&actionType=changeInputLanguage&autoTranslate=1';" value="<?php echo $formText_automaticallyTranslateEmpty_input;?>" /></div>

...
{	
	...
	$(_form).removeClass("saved").parent().find(".mandatory").removeClass("error").each(function(index, value){
		...
		{
			_return = false;
			fw_info_message_add("error", "<strong>" + $(this).addClass("error").closest(".edit_field").find(".fieldname").text() + "</strong> <?php echo $formText_isMandatory_input;?>");
		}
	});
...
<div class="edit_container">
	...
	unset($v_check['all']);
	if($b_multilanguage_table && count($v_check)>1)
	{
		$x=0;
		?>
		<div class="btn-group">
	...
	<div id="roundFields" class="edit_field_container">
		...
		if($activateSeo == "1")
		{
			...
			?>
			<style>
			.sys-seo { border:1px solid #999999; background-color:#f5f5f5; margin:15px 0; padding:10px 5px; }
			...
			<br clear="all">
			<div class="sys-seo ui-corner-all">
			<?php
			...
			foreach($fieldsStructure['seodescription'][6] as $key => $value)
			{
				...
				?><div>
					...
					<div class="onefield sys-seo-input"><input type="text" id="<?php echo $obj_ui_id;?>" name="<?php echo $submodule.'seodescription'.$key;?>" value="<?php echo $value;?>" maxlength="150"> <i><?php echo $formText_length_input;?> <span class="counter"><?php echo strlen($value);?></span> <?php echo $formText_of_input;?> <span class="total">150</span></i></div>
				</div><?php
					...
					} else if($item[3] == 2 and $mergeHeaderAndMenulevelInTitle=="1") //seo title
					{
						...
						$('#<?php echo $item[0];?>').on('change',function() {
							if($(this).attr('maxlength'))
							{
								...
								$(this).parent().find('span.counter').text($(this).val().length);
							}

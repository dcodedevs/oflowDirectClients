...
$(document).ready(function() {
  ...
  function fwaFileuploadInit() {
    ...
    $('#<?php echo $fwaFileuploadConfig['id'];?>_upload').unbind().fileupload({
      url: '<?php echo '../modules/'.$fwaFileuploadConfig['module_folder'].'/output/includes/fileupload8/ajax_upload.php?param_name='.$fwaFileuploadConfig['id'].'_files&file_type='.$fwaFileuploadConfig['upload_type']; ?>',
      dataType: 'json',
      formData: {
      ...
      progress: function (e, data) {
        ...
        $('[data-tmp-upload-id="' + btoa(encodeURIComponent(data.files[0].name)) + '"] .progress-fill').css('width', progress + '%');
        if(progress >= 100) $('[data-tmp-upload-id="' + btoa(encodeURIComponent(data.files[0].name)) + '"] .progress').addClass('progress-complete').closest('li').find('.name').prepend('<span class="glyphicon glyphicon-ok progress-complete-icon"></span>');
      },
  ...
  function fwaFileuploadAddFileToProgress (name, tmp_upload_id) {
    var html = '<li data-tmp-upload-id="' + btoa(encodeURIComponent(name)) + '">' +

...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		...
		if($_POST['editResource'] > 0) {
			$s_sql = "UPDATE company_product_set SET
			updated = now(),
			updatedBy= ?,
			name= ?
			WHERE id = ?";
			...
			$s_sql = "INSERT INTO company_product_set SET
			id=NULL,
			moduleID = ?,
			created = now(),
			createdBy= ?,
			...
			$s_sql = "DELETE company_product_set  FROM company_product_set WHERE company_product_set.id = ?";
...
<div class="popupform">
	...
	<div class="popupformTitle"><?php echo $formText_AddEditCompanyProductSets_output;?></div>
	<div class="errorMessage"></div>
	<div class="resourceList" data-action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editProductSets";?>"
	data-action2="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editProductSets";?>">
	<?php

	$resources = array();
	$s_sql = "SELECT * FROM company_product_set WHERE content_status < 2 ORDER BY sortnr";
			...
			<?php if(!$resource['disabledForEditing']) { ?>
				...
				<div class="editRow">
					...
					<div class="editInnerRow">
						<label><?php echo $formText_CompanyProductSetName_output?></label>
	...
	<div class="addNew">
		<div class="plusTextBox active">
			...
			<div class="text"><?php echo $formText_AddCompanyProductSetName_output; ?></div>
		</div>
		<div class="clear"></div>
	</div>

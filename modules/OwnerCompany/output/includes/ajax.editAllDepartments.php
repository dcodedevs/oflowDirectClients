...
if (isset($_POST['action']) && $_POST['action'] === 'openWithSync') {
    ...
    // // Empty table
    // $o_main->db->truncate('departmentforaccounting');

    // // Insert projects
    // foreach($projects_list as $project) {
    //     $o_main->db->insert('departmentforaccounting', array(
    //         'created' => date('Y-m-d H:i:s'),
    //         'createdBy' => $variables->loggID,
    //         'departmentnumber' => $project['code'],
...
function getProjects($o_main, $parentNumber = 0,  $search = '') {
    ...
    if($search != ""){
      $o_query = $o_main->db->query("SELECT * FROM departmentforaccounting WHERE (departmentnumber LIKE '%".$search."%' OR name LIKE '%".$search."%') ORDER BY departmentnumber");
    } else {
        if ($parentNumber) {
        ...
        foreach ($o_query->result_array() as $row) {
            ...
            if($search != ""){
                ...
                array_push($projects, array(
                    ...
                    'children' => getProjects($o_main, $row['departmentnumber'])
                ));
            }
        }
    }
...
function getProjectsListHtml($projects) {
    ...
    global $ownerCompanyBasisConfig;
    global $ownerCompanyAccountConfig;
    global $formText_AddNewDepartment_output;

	$b_activate_email = (isset($ownerCompanyAccountConfig->activate_email_for_department) && 1 == $ownerCompanyAccountConfig->activate_email_for_department);
    ob_start(); ?>

	<?php if($ownerCompanyBasisConfig->activateEditAllDepartment == 2) { ?>
		<span class="addDepartment"><?php echo $formText_AddNewDepartment_output;?></span>
	<?php }?>
            ...
            <li class="ep_project_list_item">
                <div class="ep_project_list_item_content">
                    <div class="ep_project_list_item_text">
						...
						<?php if($project['email'] != "" && $b_activate_email) echo " <span class='email_span'>".$project['email']."</span>";?>
                    </div>
                </div>
...
if ($ownercompany_accountconfig['lastDepartmentSyncDate']) {
    $lastSyncDate = date('H:i / d.m.Y', strtotime($ownercompany_accountconfig['lastDepartmentSyncDate']));
}
...
<div class="contactPersonSearch">
    ...
    <input type="text" placeholder="<?php echo $formText_DepartmentName_output;?>" class="contactPersonSearchInput" value="<?php echo $_POST['search']?>"/>
...
$(document).ready(function() {
	...
	$(".editDepartmentBtn").unbind("click").bind("click", function(){
		...
		ajaxCall('editDepartment', data, function(json) {

...
if($departmentId) {
    $sql = "SELECT * FROM projectforaccounting WHERE id = $departmentId";
...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($department) {
            ...
            $result = $o_main->db->query($sql, array($_POST['projectnumber'], $department['id']));
            ...
            if(count($departmentByNumber) == 0 ){
                ...
                projectnumber= ?
                WHERE id = ?";
                if($o_main->db->query($s_sql, array($variables->loggID, $_POST['name'], $_POST['projectnumber'], $department['id']))){
                ...
                $fw_error_msg = $formText_ProjectNumberAlreadyInUse_output;
            }
        } else {
            $sql = "SELECT * FROM projectforaccounting WHERE projectnumber = ?";
            $result = $o_main->db->query($sql, array($_POST['projectnumber']));
            $departmentByNumber = $result ? $result->row_array() : array();
            if(count($departmentByNumber) == 0 ){
                ...
                name= ?,
                projectnumber= ?";
                if($o_main->db->query($s_sql, array($variables->loggID, $_POST['name'], $_POST['projectnumber']))){
                    $fw_redirect_url = $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=details&cid=".$ownercompanyId;
                } else {
                    $fw_error_msg = $formText_ErrorUpdatingDatabase_output;
                }
	...
	if(isset($_POST['deleteDepartment']) && $departmentId > 0) {
        $s_sql = "DELETE FROM projectforaccounting WHERE id = ?";
...
<div class="popupform">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editAccountingProject";?>" method="post">
		...
		<div class="inner">
            <div class="line">
                ...
                <div class="lineInput">
                    <input type="text" class="popupforminput botspace" name="projectnumber" value="<?php echo $department['projectnumber']; ?>" required>
                </div>
                <div class="clear"></div>
            </div>
            <div class="line">

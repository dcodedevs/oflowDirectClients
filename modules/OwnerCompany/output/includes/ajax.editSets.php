...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($ownercompanyId) {
            ...
            WHERE id = ?";
            $o_main->db->query($s_sql, array($variables->loggID, $_POST['company_product_set_id'], $ownercompanyId));
...
<div class="popupform">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editSets";?>" method="post">
		...
		<div class="inner">
    		<div class="line">
        		<div class="lineTitle"><?php echo $formText_ArticleSet_output; ?></div>
        		<div class="lineInput">
                    <?php
                    $rows = array();
                    $s_sql = "SELECT * FROM company_product_set ORDER BY name ASC";
                    $o_query = $o_main->db->query($s_sql);
                    $rows = $o_query ? $o_query->result_array() : array();
                    ?>
                    <select name="company_product_set_id" class="popupforminput botspace" required>
                        <option value="0"><?php echo $default_label;?></option>
                        <?php foreach($rows as $row){  ?>
                        <option value="<?php echo $row['id']; ?>" <?php if($officeSpaceData->company_product_set_id == $row['id']) echo 'selected'; ?>><?php echo $row['name'];?></option>
                        <?php } ?>
                    </select>
                </div>
        		<div class="clear"></div>

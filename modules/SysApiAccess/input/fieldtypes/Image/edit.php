			...
			<div class="col-md-9 col-xs-8">
				...
				foreach($image as $x=>$item)
				{
					...
					if(strpos($tmp[3],"f")!==false && ($b_activate_cdn || is_file(ACCOUNT_PATH."/".rawurldecode($item))))
					{
						...
						if($x<$ini_x) $x = $ini_x;
						if($y<$ini_x) $y = $ini_x;
						?><input class="focus" type="hidden" name="<?php echo $field[1].$ending."_focus".$i;?>[]" value="<?php echo $focus[$focus_counter];?>" data-src="<?php echo ($b_is_local_image?$extradomaindirroot:'').$item.(strpos($item,'uploads/protected/')!==false?'?username='.$_COOKIE['username'].'&sessionID='.$_COOKIE['sessionID'].'&companyID='.$_GET['companyID'].'&caID='.$_GET['caID'].'&table='.$fields[0][3].'&field='.$field[0].'&ID='.$ID.'&languageID='.$variables->languageID.'&server_id='.array_shift(explode('.', $_SERVER['HTTP_HOST'])):'');?>" data-w="<?php echo $w;?>" data-h="<?php echo $h;?>" data-x="<?php echo $x-12;?>" data-y="<?php echo $y-12;?>" data-ratio="<?php echo $ratio;?>"><?php
						$focus_counter++;
					}
				}
				if($show_link)
...
$(function () {
	...
	function <?php echo $field_ui_id;?>update_btns()
	{
		...
		$('#<?php echo $field_ui_id;?>_all .select_all').on('click', function(e){
			e.preventDefault();
		...
		$('#<?php echo $field_ui_id;?>_files .item .filename input').on('change', function(){
			if($('#<?php echo $field_ui_id;?>_files .item .filename input:checked').length > 0)
			{
				$('#<?php echo $field_ui_id;?>_all .delete_selected, #<?php echo $field_ui_id;?>_all .select_all').removeClass('unselected');
			} else {
	...
	$('#<?php echo $field_ui_id;?>_upload').fileupload({
				...
				$.each(data.files, function (index, file){
					...
					var html = '<div class="item row abortable" data-tmp-upload-id="' + tmp_upload_id + '">' +
						...
						'<div class="col-md-9 col-xs-8">' +
							...
							'<input class="name" type="hidden" name="<?php echo $field[1].$ending."_name";?>[]" value="|' + tmp_upload_id + '|' + '|' + file.name + '">' +
							'<div class="progress">' +
		...
		stop: function(e) {
			$('#<?php echo $field_ui_id;?>_files .item.upload-abort').unbind('click').remove();
			<?php echo $field_ui_id;?>alert_shown = false;

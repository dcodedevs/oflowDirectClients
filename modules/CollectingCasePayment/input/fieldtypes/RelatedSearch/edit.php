...
if($options[6] != 1)
{
	...
	$s_sql_where = ' AND c.'.$options[1].' IN('.implode(',', $v_related_id).')';
}


if($o_main->db->table_exists($options[0].'content'))
{
	$sql = "SELECT c.".$options[1]." cid, c.*, cc.*".$s_sql_select." FROM ".$options[0]." c LEFT OUTER JOIN ".$options[0]."content cc ON c.id = cc.".$options[0]."ID AND cc.languageID = ".$o_main->db->escape($s_default_output_language).$s_sql_join." WHERE c.content_status < 2".$s_sql_where." ORDER BY ".($options[6] != 1 ? 'r':'c').".sortnr";
	
} else {
	$sql = "SELECT c.".$options[1]." cid, c.*".$s_sql_select." FROM ".$options[0]." c".$s_sql_join." WHERE c.content_status < 2".$s_sql_where." ORDER BY ".($options[6] != 1 ? 'r':'c').".sortnr";
}

...
function get_customer_list($o_main, $filter, $filters, $page=1, $perPage=100, $customer_id = null) {
    ...
    $sql = "SELECT cp.*, CONCAT_WS(' ', c.name, c.middlename, c.lastname) as collecting_case_name
             FROM collecting_cases_payments cp
             LEFT OUTER JOIN collecting_cases cc ON cc.id = cp.collecting_case_id
             LEFT OUTER JOIN customer c ON c.id = cc.debitor_id
            ".$sql_join."
            WHERE 1=1 ".$sql_where;
    if($customer_id != null){
        $list = array();
        $sql .= " GROUP BY cp.id ORDER BY cp.date DESC".$pager;
        ...
        if($page == 0 && $perPage == 0){
            ...
            $sql .= " ORDER BY cp.date DESC".$pager;
            $f_check_sql = $sql;

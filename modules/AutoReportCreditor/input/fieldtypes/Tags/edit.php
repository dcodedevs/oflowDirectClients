...
if($access >= 10)
{
	...
	function bind_<?php echo $field_ui_id;?>()
	{
		...
		$("a.tag_popup_item_add_<?php echo $field_ui_id;?>").off('click').on('click', function() {
			$("#<?php echo $field_ui_id;?>_tagid").val($(this).prev().find("input.tagid").val());
			$("a#show_add_popup_<?php echo $field_ui_id;?>").trigger('click');
			return false;
		});
		$("a.tag_popup_item_rename_<?php echo $field_ui_id;?>").off('click').on('click', function() {
			$.fancybox.showLoading();
			var _data = {
		...
		$("a.tag_popup_item_move_<?php echo $field_ui_id;?>").off('click').on('click', function() {
			$.fancybox.showLoading();
			var _data = {
				className: '<?php echo $field_ui_id;?>',
				selectedtagid: $(this).parent().find("input.tagid").val(),
		...
		$("a.tag_popup_item_merge_<?php echo $field_ui_id;?>").off('click').on('click', function() {
			$.fancybox.showLoading();
			var _data = {
				className: '<?php echo $field_ui_id;?>',
				selectedtagid: $(this).parent().find("input.tagid").val(),
		...
		$("a.tag_popup_item_delete_<?php echo $field_ui_id;?>").off('click').on('click', function() {
			$.fancybox.showLoading();
			var _data = {
				className: '<?php echo $field_ui_id;?>',
				selectedtagid: $(this).parent().find("input.tagid").val(),
...
<div class="tags_<?php echo $field_ui_id.($field[9]==1?" hide":"");?>">
	...
	<?php
	$i=0;
	$o_query = $o_main->db->query("select t.id, t.name, t.parentID from sys_tag t join sys_tagrelation tr on tr.tagID = t.id and tr.contentID = ? and tr.contentTable = ?".($o_main->multi_acc?" WHERE t.account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")." group by t.id order by tr.sortnr", array($ID, $field[3]));
	if($o_query && $o_query->num_rows()>0)

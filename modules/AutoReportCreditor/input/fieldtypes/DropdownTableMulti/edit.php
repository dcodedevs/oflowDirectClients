...
$contentSQL = "SELECT ".implode(",",$dataFields).", {$options[3]}.{$options[4]} FROM $dataTable LEFT OUTER JOIN {$options[3]} on $dataTable.{$options[1]} = {$options[3]}.{$options[5]} AND {$options[3]}.contentTable = '{$s_single_table}' AND {$options[3]}.{$options[4]} = ".$o_main->db->escape($ID)." $extraJoin";

...
foreach($_POST[$fields[$nums][1]] as $item)
{
	if(!in_array($item,$values))
	{
		...
		if($options[8]==1)
		{
			$sql = 'SELECT c.'.$options[1].' col0, c.'.$options[6].' col1 FROM '.$options[0].' c WHERE c.'.$options[1].' = ?';
...
{
	$o_main->db->query('INSERT INTO '.$options[3].'('.$options[4].','.$options[5].',contentTable) VALUES(?,?,?)', array($basetable->ID, $item, $tablename));
}

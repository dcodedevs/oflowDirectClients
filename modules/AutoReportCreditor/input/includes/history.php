<div class="history" style="padding:20px 10px;">
	...
	{
		$s_sql = "SELECT ".$submodule.".id cid, ".$submodule.".*, ".$submodule."content.* FROM ".$submodule." LEFT OUTER JOIN ".$submodule."content ON ".$submodule."content.".$submodule."ID = ".$submodule.".id WHERE ".$submodule.".id = ".$o_main->db->escape($ID)."".($o_main->multi_acc?" AND ".$submodule.".account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")." ORDER BY ".$submodule."content.languageID";
		$s_sql_history = "SELECT ".$submodule.".id cid, ".$submodule.".*, ".$submodule."content.* FROM ".$submodule." LEFT OUTER JOIN ".$submodule."content ON ".$submodule."content.".$submodule."ID = ".$submodule.".id WHERE ".$submodule.".origId = ".$o_main->db->escape($ID)."".($o_main->multi_acc?" AND ".$submodule.".account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")." ORDER BY ".$submodule.".id DESC, ".$submodule."content.languageID ASC";
	} else {
		$s_sql = "SELECT id cid, ".$submodule.".* FROM ".$submodule." WHERE ".$submodule.".id = ".$o_main->db->escape($ID)."".($o_main->multi_acc?" AND ".$submodule.".account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"");
		$s_sql_history = "SELECT id cid, ".$submodule.".* FROM ".$submodule." WHERE ".$submodule.".origId = ".$o_main->db->escape($ID)."".($o_main->multi_acc?" AND ".$submodule.".account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")." ORDER BY ".$submodule.".id DESC";
	}
	$origRow = array();

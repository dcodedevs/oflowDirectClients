...
<script type="text/javascript">
	...
	$(".employees .pagination a").unbind("click").bind("click", function(){
		var _data = { fwajax: 1, fw_nocss: 1, creditor:'<?php echo $_POST['creditor']?>', search: $("#employeesearch").val(), resultOnly: true, page: $(this).html()};
		$.ajax({
			cache: false,
			type: 'POST',
			dataType: 'json',
	...
	<?php if(!$resultOnly){ ?>
		$(".employeeSearch button").unbind("click").bind("click", function(){
			var _data = { fwajax: 1, fw_nocss: 1, creditor:'<?php echo $_POST['creditor']?>', search: $("#employeesearch").val(), resultOnly: true};
			$.ajax({
				cache: false,
				type: 'POST',
				dataType: 'json',

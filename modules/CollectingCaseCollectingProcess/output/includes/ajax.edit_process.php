...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
		...
		$sql = "SELECT * FROM collecting_cases_collecting_process ORDER BY sortnr DESC";
		$o_query = $o_main->db->query($sql);
		$maxData = $o_query ? $o_query->row_array() : array();
		$sortnr = intval($maxData['sortnr']) + 1;
        if ($processId) {
            $sql = "UPDATE collecting_cases_collecting_process SET
            updated = now(),
            updatedBy='".$variables->loggID."',
            name='".$o_main->db->escape_str($_POST['name'])."',
			available_for='3',
			with_warning='".$o_main->db->escape_str($_POST['with_warning'])."'
            WHERE id = $processId";
            ...
            $sql = "INSERT INTO collecting_cases_collecting_process SET
            created = now(),
...
if($action == "deleteProcess" && $processId) {
    $sql = "DELETE FROM collecting_cases_collecting_process
    WHERE id = $processId";
    $o_query = $o_main->db->query($sql);
}
...
<div class="popupform popupform-<?php echo $processId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_process";?>" method="post">
		...
		<div class="inner">
    		...
    		<div class="line">
        		<div class="lineTitle"><?php echo $formText_WithWarning_Output; ?></div>
        		<div class="lineInput">
                    <input type="checkbox" class="popupforminput botspace checkbox" autocomplete="off" name="with_warning" value="1" <?php if($processData['with_warning']) echo 'checked';?>>
                </div>
        		<div class="clear"></div>
    		</div>
		</div>

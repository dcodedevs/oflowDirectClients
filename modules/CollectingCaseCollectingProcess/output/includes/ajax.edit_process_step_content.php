...
<div class="popupform popupform-<?php echo $processId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_process_step_content";?>" method="post">
		...
		<div class="inner">
    		...
    		<div class="line">
        		...
        		<div class="lineInput">
					<select name="claim_type_2_article" autocomplete="off" >
						<option value=""><?php echo $formText_No_output;?></option>
					   <?php

					   foreach($articles as $article) {
						   ?>
						   <option value="<?php echo $article['id'];?>" <?php if($article['id'] == $processStepData['claim_type_2_article']) echo 'selected';?>><?php echo $article['article_name'];?></option>
						   <?php
					   }
					   ?>
				   </select>

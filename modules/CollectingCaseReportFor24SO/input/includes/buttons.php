...
if($access >= 10)
{
	...
	if(is_file(__DIR__."/../settings/buttonconfig/$buttonbase".$buttonlisttype.".php"))
	{
		...
		foreach($buttonlist as $buttonsArray)
		{
			...
			if($buttonsArray[5] == 1 && $buttonsArray[6] == $module)
				$buttonMode = 1;
			$buttonContentStatus = isset($buttonsArray[7]) ? $buttonsArray[7] : '';
	...
	{
		if(isset($showSendMail) && 1 == $showSendMail && 10 <= $access)
		{
		...
		if(isset($showSendSms) && 1 == $showSendSms && 10 <= $access)
		{
		...
		if(isset($showSendPdf) && 1 == $showSendPdf && 10 <= $access)
		{
		...
		if((!isset($orderManualOrByField) || $orderManualOrByField == '0') and $access >= 10 && !$jumpfirstpage)
		{
            if(isset($writeContent) && is_array($writeContent) && array_key_exists('menulevel',$writeContent)) 

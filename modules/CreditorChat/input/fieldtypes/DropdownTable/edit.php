...
$b_skipp_multi_acc = isset($options[7]) && 1 == $options[7];
if(!isset($options[4]) or $options[4]=="" or ($dataParentField!="" and ($dataParentLimit != 1 or ($dataParentLimit==1 and $_GET['level']>0))))
{
	?><select <?php echo $field_attributes;?> id="<?php echo $field_ui_id;?>" onChange="$('#<?php echo $field_ui_id;?>orig').val($(this).val());"<?php echo ($field[10]==1||$access<10?" disabled":"");?>><?php
		...
		$contentSQL = "SELECT ".implode(",",$select_fields)." FROM $dataTable $extra_join";
		$sqlWhere = (($o_main->multi_acc&&!$b_skipp_multi_acc)?" WHERE account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"");
		//TODO: ALI security_check - filter
		if($dataFilter!="") $sqlWhere .= (''!=$sqlWhere?" AND ":" WHERE ")."{$dataFilter}";
		if($dataParentField!="") $sqlWhere .= (''!=$sqlWhere?" AND ":" WHERE ")."$dataParentField = 0";
		$sqlWhere .= (''!=$sqlWhere?" AND ":" WHERE ")."$dataTable.content_status < 2";

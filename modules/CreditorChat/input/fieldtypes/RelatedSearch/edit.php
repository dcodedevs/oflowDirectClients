...
if($options[6] != 1)
{
	...
	{ 
		...
		$b_table_created = $o_main->db->query("CREATE TABLE `{$options[3]}` (
			...
			".($o_main->multi_acc?"`account_id` INT(11) NOT NULL DEFAULT 0,":"")."
	...
	$s_sql_join = " JOIN ".$options[3]." r on c.".$options[1]." = r.".$options[5]." and r.".$options[4]." = ".$o_main->db->escape($ID)." and r.contentTable = ".$o_main->db->escape($s_single_table)."".($o_main->multi_acc?" AND r.account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")."";
} else {

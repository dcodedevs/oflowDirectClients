...
{
	$s_sql = "SELECT $dataTable.id cid, $dataTable.{$dataID[0]}, $dataTable.moduleID, ".implode(",",$sqlSelect)." FROM $dataTable LEFT OUTER JOIN {$dataTable}content ON {$dataTable}content.{$dataTable}ID = $dataTable.id AND {$dataTable}content.languageID = ".$o_main->db->escape($s_default_output_language)." WHERE $dataTable.$dataID[0] = ".$o_main->db->escape($field[6][$langID]).$s_sql_where." AND $dataTable.content_status < 2 order by ".implode(",",$sqlOrder).";";
} else {
	$s_sql = "SELECT $dataTable.id cid, $dataTable.{$dataID[0]}, $dataTable.moduleID, ".implode(",",$sqlSelect)." FROM $dataTable WHERE $dataTable.$dataID[0] = ".$o_main->db->escape($field[6][$langID]).$s_sql_where." AND $dataTable.content_status < 2 order by ".implode(",",$sqlOrder).";";
}

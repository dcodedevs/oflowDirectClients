...
$contentSQL = "SELECT ".implode(",",$dataFields).", {$options[3]}.{$options[4]} FROM $dataTable
 LEFT OUTER JOIN {$options[3]} on {$options[3]}.{$options[4]} = ".$o_main->db->escape($ID)."
 AND $dataTable.{$options[1]} = {$options[3]}.{$options[5]}
 AND {$options[3]}.contentTable = '{$s_single_table}'".($o_main->multi_acc?" AND {$options[3]}.account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")."$extraJoin";
$sqlWhere = " WHERE ".($o_main->multi_acc?"$dataTable.account_id = '".$o_main->db->escape_str($o_main->account_id)."' AND ":"")."$dataTable.content_status < 2";

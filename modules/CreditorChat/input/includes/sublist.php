	...
	for($x = 0; $x < sizeof($prerelations); $x++)
	{
		...
		if($showMe[$prerelationsarray[2]] != 1)
		{
			if($prerelationsarray[4] == 1)
			{
				...
				$relationvalue = array();
				$o_query = $o_main->db->query("SELECT ".$submodule.".* FROM ".$submodule." WHERE ".$submodule.".id = ?".($o_main->multi_acc?('_basisconfig' == substr($submodule, -12)?" AND app_id = '".$o_main->db->escape_str($o_main->app_id)."'":" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'"):""), array($_GET['ID']));
				...
				$extraWhere = ($o_main->multi_acc?('_basisconfig' == substr($relationstable, -12)?" AND ".$relationstable.".app_id = '".$o_main->db->escape_str($o_main->app_id)."'":" AND ".$relationstable.".account_id = '".$o_main->db->escape_str($o_main->account_id)."'"):"");
				...
				if($module_multi_table)
				{
					...
					JOIN ".$submodule." ON ".$relationstable.".".$o_main->db_escape_name($prerelationsarray[3])." = ".$submodule.".".$o_main->db_escape_name($prerelationsarray[0]).($o_main->multi_acc?('_basisconfig' == substr($submodule, -12)?" AND ".$submodule.".app_id = '".$o_main->db->escape_str($o_main->app_id)."'":" AND ".$submodule.".account_id = '".$o_main->db->escape_str($o_main->account_id)."'"):"")."

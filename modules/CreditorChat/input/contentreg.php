...
require_once(__DIR__."/includes/class_Field.php");
require_once(__DIR__."/includes/fn_sys_get_field_config.php");
require_once(__DIR__."/includes/fn_sys_get_field_setting_text.php");
...
if($o_main->db->table_exists($submodule))
{
	...
	{
		$o_query = $o_main->db->query("SELECT createdBy FROM ".$submodule." WHERE id = '".$o_main->db->escape_str($ID)."'");
...
if(count($error_msg)>0)
{
	...
	if(!isset($_POST['no_success_msg']))
	{
		...
		$s_sql = "UPDATE session_framework SET error_msg = '".$o_main->db->escape_str(json_encode(array("info_1"=>$formText_ContentIsSaved_input)))."' WHERE companyaccessID = '".$o_main->db->escape_str($_GET['caID'])."' AND session = '".$o_main->db->escape_str($_COOKIE['sessionID'])."' AND username = '".$o_main->db->escape_str($_COOKIE['username'])."'".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"");
	...
	foreach($v_field_config as $nut)
	{
		...
		foreach($_POST['input_description'] as $index2=>$input_description) {
			if($index2 == $fieldCounter) {
				$nut->input_description = str_replace(array("\r\n", "\n", "\r"), '',nl2br($input_description));
				break;
			}
		}
		foreach($_POST['input_title'] as $index2=>$input_description) {
			if($index2 == $fieldCounter) {
				$nut->htmlname = str_replace(array("\r\n", "\n", "\r"), '',nl2br($input_description));
				break;
			}
		}
		$input_description="";
		if(isset($nut->input_description)){
			$input_description = $nut->input_description;
		}
		$makeString .= $nut->sqlname."¤".$nut->formname."¤".$nut->htmlname."¤".$nut->database."¤".$nut->type."¤".$nut->extra."¤".$nut->default."¤¤¤".$nut->hidden."¤".$nut->readonly."¤".$nut->extravalue."¤".$nut->update."¤".$nut->insert."¤".$nut->mandatory."¤".$nut->duplicate."¤".$nut->fieldwidth."¤".$input_description."¤";
		$fieldCounter++;
	}
	//echo $makeString;
	//exit;

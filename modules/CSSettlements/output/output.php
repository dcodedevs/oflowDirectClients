...
$o_query = $o_main->db->get('customerportalinvoices_accountconfig');
$module_accountinfo = $o_query ? $o_query->row_array() : array();
$topImage = json_decode($module_accountinfo['topImage']);
	...
	<div id="output-content-container">
		<?php if(count($topImage) > 0 && $s_group_id == 0){ ?>
			<div class="output-top-image fw_module_head_color">
				...
				<?php } ?>
				<?php if($module_accountinfo['activateTopImageTitle']) {?>
					<div class="output-top-image-text"><?=($v_module_main_tables[1][1]!=""?$v_module_main_tables[1][1]:$v_module_main_tables[1][0]);?></div>
				<?php } ?>
			</div>
		<?php } ?>
		<?php

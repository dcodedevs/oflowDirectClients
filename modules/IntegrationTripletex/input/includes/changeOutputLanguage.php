...
if(count($db_languages_titles)>0)
{
	...
	if($displayForm)
	{
		...
		<div style="margin:10px;"><input type="button" onClick="javascript: document.location.href='?<?=get_curent_GET_params(array("autoTranslate","actionType"));?>&actionType=changeOutputLanguage&autoTranslate=1';" value="<?=$formText_automaticallyTranslateEmpty_input;?>" /></div>
			...
			if(count($data_of_variables[$output_dir]))
			{
				...
				foreach($variable_ids as $var_id)
				{
					...
					<tr class="item">
						...
						$color = 0;
						$percent = '';
						$data = array('data'=>json_encode(array('action'=>'get_variables', 'type'=>'output', 'data'=>array($var_id,$lang))));
		...
		?>
		<tr><td colspan="3"><strong>Access elements</strong></td></tr>
		<?php
		foreach($output_folders as $output_dir)
		{
			// $accessElementsAllow = get_accesselement_variables($output_dir, "allow");

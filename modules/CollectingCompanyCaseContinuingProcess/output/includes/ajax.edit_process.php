...
$sql = "SELECT * FROM collecting_company_cases_continuing_process WHERE id = $processId";
...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
		...
		$sql = "SELECT * FROM collecting_company_cases_continuing_process ORDER BY sortnr DESC";
		$o_query = $o_main->db->query($sql);
		$maxData = $o_query ? $o_query->row_array() : array();
		$sortnr = intval($maxData['sortnr']) + 1;
        if ($processId) {
            $sql = "UPDATE collecting_company_cases_continuing_process SET
            updated = now(),
            ...
            $sql = "INSERT INTO collecting_company_cases_continuing_process SET
            created = now(),
...
if($action == "deleteProcess" && $processId) {
    $sql = "DELETE FROM collecting_company_cases_continuing_process
    WHERE id = $processId";
    $o_query = $o_main->db->query($sql);
}

...
// Version 1.9
// 1.9 - updated how language variables retreived for non central accounts
// ************
...
/*
** Include central language files (10.004)
**/
if($o_main->central_output_language)
{
	...
	if(1 == $_SESSION['show_label_edit'])
	{
		foreach($o_main->language_variables as $s_key => $s_value)
		{
			if(FALSE === strpos($s_value, 'class="fw-label-info'))
			{
				//${$s_key} .= '<span class="fw-label-info glyphicon glyphicon-info-sign"><div class="fw-label-info-container">Variable: '.$s_key.'</div></span>';
				${$s_key} .= ' '.$s_key;
			}
		}
	}
...
// $v_filtered_files = array();
// foreach($v_files as $v_file){
// 	if(stripos($v_file, "languagesOutput/") === false){
// 		$v_filtered_files[] = $v_file;
// 	}
// }
// $v_variables = get_language_variables($v_filtered_files);
// $v_variables_ids = array_keys($v_variables);



$v_all_variables_old = get_defined_vars();

$s_include = $s_module_absolute_path.'/'.$s_folder.'/languagesOutput/empty.php';
if(is_file($s_include)) include($s_include);

if(!function_exists("devide_by_uppercase")) include_once($s_module_absolute_path."/input/includes/fnctn_devide_by_upercase.php");
$v_all_variables_new = get_defined_vars();
$v_variables = array();
foreach($v_all_variables_new as $key=>$v_variable_new) {
	if(strpos($key, "formText_") === 0 && !isset($v_all_variables_old[$key])){			
		$formTextName=explode("_", $key);
		$v_variables[$key] = array('name'=>$formTextName[1], 'defaultValue'=>devide_by_uppercase($formTextName[1]));
	}
}
$v_variables_ids = array_keys($v_variables);

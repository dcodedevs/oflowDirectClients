...
if($field[6][$langID] == "" and $access >= 10)
{
	if($field[11] != "")
	{
		...
		if($o_main->multi_acc && !$o_main->db->field_exists('account_id', 'sys_sequence'))
		{
			$o_main->db->query("ALTER TABLE sys_sequence ADD COLUMN account_id INT NOT NULL DEFAULT 0");
		}
		$compFormat = $format[0]."[]".$format[2];
		$s_sql = "select num from sys_sequence where created < ADDDATE(NOW(), INTERVAL -1 HOUR) and tablefield = ? and seq_format = ? and seq_status = 1".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"");
		$o_row = (object) array();
		...
		if($o_row->num > 0) // if exists not used number
		{
			...
			{
				$sequence = $o_row->num;
				$o_main->db->query("update sys_sequence set created = now() where num = $sequence and tablefield = ? and seq_format = ? and seq_status = 1".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":""), array($field[1], $compFormat));
			} else {
				$o_main->db->query("insert into sys_sequence SET
				tablefield = '".$o_main->db->escape_str($field[1])."',
				seq_format = '".$o_main->db->escape_str($compFormat)."',
				num = '".$o_main->db->escape_str($sequence)."',
				created = NOW(),
				seq_status = 1".($o_main->multi_acc?",account_id = '".$o_main->db->escape_str($o_main->account_id)."'":""));
			}
		} else {
			// check next max sequence number
			$s_sql = "select max(num) num from sys_sequence where tablefield = ? and seq_format = ?".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"");
			$o_row = (object) array();
			$o_query = $o_main->db->query($s_sql, array($field[1], $compFormat));
			if($o_query && $o_query->num_rows()>0) $o_row = $o_query->row_array();
			if($o_row->num > 0) // if exists not used number
		...
		$o_main->db->query("update sys_sequence set seq_num = ? where num = ? and tablefield = ? and seq_format = ? and seq_status = 1".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":""), array($field[6][$langID], $sequence, $field[1], $compFormat));
	} else {
		// no format set
	}
}

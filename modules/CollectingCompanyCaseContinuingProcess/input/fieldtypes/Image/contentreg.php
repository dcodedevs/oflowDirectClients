...
if($fields[$fieldPos][9]==1)
{
	...
	if(isset($fw_session['content_server_api_url']) && '' != $fw_session['content_server_api_url'])
	{
						...
						} else if($img_obj[0] == "process")
						{
							if('http' == $img_obj[1])
							{
								...
								if($l_upload_id > 0)
								{
									...
									$s_upload_path = 'uploads/'.$l_upload_id.'/'.$s_upload_name;
									$s_upload_path_enc = 'uploads/'.rawurlencode($l_upload_id).'/'.rawurlencode($s_upload_name);
									...
									file_put_contents($account_path.'/'.$s_upload_path, file_get_contents(implode("/", $image_array)));
									...
									if(is_file($account_path.'/'.$s_upload_path) && 0 < $l_size)
									{
										...
										$v_direct_upload = array();
										$v_direct_upload['path'] = $s_upload_path;
										$v_direct_upload['real_path'] = $account_path.'/'.$s_upload_path;
										$v_direct_upload['upload_id'] = $l_upload_id;
										$v_direct_uploads[] =$v_direct_upload;
									} else {
		...
		} else if(count($v_direct_uploads) > 0){
			$images_array =  array();
			$s_response = APIconnectAccount("account_authenticate", $o_main->accountinfo['accountname'], $o_main->accountinfo['password']);
			$v_response = json_decode($s_response, TRUE);
			foreach($v_direct_uploads as $v_direct_upload){
				if(function_exists('curl_file_create'))
				{
					// php 5.5+
					$cFile = curl_file_create($v_direct_upload['real_path']);
				} else {
					$cFile = '@' . realpath($v_direct_upload['real_path']);
				}

				$v_upload['data'] = json_encode(array('action'=>'upload_file_direct'));
				$v_upload['upload_file'] = $cFile;
				$v_upload['upload_to'] = $v_direct_upload['path'];
				$v_upload['accountname'] = $o_main->accountinfo['accountname'];
				$v_upload['token'] = $v_response['token'];
				...
				if($s_response !== FALSE && $s_response != "")
				{
					...
					if(isset($v_response['status']) && 1 == $v_response['status'])
					{
						$upload_url = $v_response['upload_url'];
						$image = array();
						$image[0] = basename($upload_url);
						$image[1] = array($upload_url);
						
						$x = array();
						$image[2] = $x;
						$image[3] = "";
						$image[4] = $v_direct_upload['upload_id'];
						$image[5] = "";
						$x = array();
						$image[6] = $x;
						$images_array[] = $image;
						unlink($v_direct_upload['real_path']);
					}
					if(isset($v_response['errors']) && 0 < sizeof($v_response['errors']))
					{
						foreach($v_response['errors'] as $s_error) $error_msg["error_".count($error_msg)] = $s_error;
		...
		if(array_key_exists($fieldName."_name",$_POST))
		{
			...
			$quota_bytes = $fw_session['upload_quota'];

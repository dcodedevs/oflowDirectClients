<?php
$start = 0;
if($preorderByField != '') $orderbyfield = " ORDER BY ".$submodule.".".$o_main->db_escape_name($preorderByField);
...
if(isset($_GET['relation_module_id']))
{
	$extraWhere .= " AND ".$o_main->db_escape_name($_GET['sub_relationstable']).".moduleID = ".$o_main->db->escape($moduleID);
...
if($find_content){
	...
	foreach($find_content->result_array() as $writeContent)
	{
		...
		$editLink = $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&"."module=".$module."&ID=".$content_id."&parentID=".$_GET['sub_parentID']."&subcontent=1&includefile=edit&submodule=".$submodule."&relationID=".$_GET['sub_relationID']."&relationfield=".$_GET['sub_relationfield'].($_GET["content_status"]!="" ? "&content_status=".$_GET["content_status"] : "").(isset($_GET["relation_module_id"]) ? "&relation_module_id=".$moduleID : "");
		...
		?><div class="input_list_item row" data-group="input_list_item"><?php
					...
					foreach($finList as $key=>$finList)
					{
						...
						if(is_file(ACCOUNT_PATH."/modules/".$module."/input/fieldtypes/".$linkersField[$key][4]."/customlistdisplay.php"))
						{
							...
							if(isset(${$invoiceName}))
							{
								...
								echo '<a href="'.$_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&"."module=".$module."&ID=".$content_id."&parentID=".$_GET['sub_parentID']."&subcontent=1&includefile=edit&submodule=".$submodule."&relationID=".$_GET['sub_relationID']."&relationfield=".$_GET['sub_relationfield'].($_GET["content_status"]!="" ? "&content_status=".$_GET["content_status"] : "").(isset($_GET["relation_module_id"]) ? "&relation_module_id=".$moduleID : "").'" class="optimize" data-target="#content_'.$module.'_'.$submodule.'_'.$content_id.'"> '.htmlentities($value).'</a>';
			...
			<div class="list_buttons popup_button_box col-md-1 dropdown">
				...
				if($orderManualOrByField == '0' and $access >= 10){ ?><li><a role="menuitem" href="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&ID=".$content_id."&includefile=orderContent&submodule=".$submodule.(array_key_exists('menulevel',$writeContent) ? "&menulevel=".$writeContent['menulevel']:"")."&relationID=".$_GET['sub_relationID']."&relationfield=".$_GET['sub_relationfield']."&relationstable=".$_GET['sub_relationstable'].($_GET["content_status"]!="" ? "&content_status=".$_GET["content_status"] : "").(isset($_GET["relation_module_id"]) ? "&relation_module_id=".$moduleID : "")."&parenttable=".$submodule."&ordersql=".$ordersql;?>" class="optimize"><?php echo $formText_order_list;?></a></li><?php }
				
				if($prelistButtonDelete == 1 and $access >= 100){ ?><li><a role="menuitem" class="delete" data-name="<?php echo $item_name;?>" href="<?php echo $extradir."/input/delete.php?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&parentID=".$_GET['sub_parentID']."&deleteID=".$content_id."&deletemodule=".$submodule."&submodule=".$submodule."&relationID=".$_GET['sub_relationID']."&relationfield=".$_GET['sub_relationfield']."&relationstable=".$_GET['sub_relationstable'].($_GET["content_status"]!="" ? "&content_status=".$_GET["content_status"] : "").(isset($_GET["relation_module_id"]) ? "&relation_module_id=".$moduleID : "");?>"><?php echo $formText_delete_sublist;?></a></li><?php }

...
if($s_inc_obj != "ajax")
{
    ...
    $currentYear = date("Y");
    ?>
    <div class="output-category-block" id="claimtypes_block">
        <div class="output-title-different"><?php echo $formText_ClaimlineTypes_output;?></div>
        <div class="rs_box extra">
            <div class="output-content" id="claimtypes_info">
                <div class="loading"><div class="lds-ring"><div></div><div></div><div></div><div></div></div></div>
            </div>
        </div>
    </div>
    <style>
    #claimtypes_block .output-content .output-content-sub-title {
        font-size: 14px;
        font-weight: bold;
		padding: 0px 0px 5px;
	}
    ...
    <script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(json){
                setTimeout(function() {
                    $('#claimtypes_info').html(json.html);
                }, 500);
            }
        }).fail(function() {
        });

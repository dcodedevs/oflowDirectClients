...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		...
		{
			$s_sql = "SELECT * FROM collecting_cases_messages_debitor WHERE id = ?";
		    $o_query = $o_main->db->query($s_sql, array($_POST['cid']));
		    if($o_query && $o_query->num_rows() == 1) {
				$s_sql = "UPDATE collecting_cases_messages_debitor SET
				updated = now(),
				updatedBy= ?,
				text= ?
				WHERE id = ?";
			...
			$s_sql = "INSERT INTO collecting_cases_messages_debitor SET
			id=NULL,
			moduleID = ?,
			created = now(),
			createdBy= ?,
	...
	} else if(isset($_POST['output_delete']))
	{
		if(isset($_POST['cid']) && $_POST['cid'] > 0)
		{
			$s_sql = "DELETE FROM collecting_cases_messages_debitor WHERE id = ?";

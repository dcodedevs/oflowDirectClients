...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if($projectData) {
			$update_sql= "";
			if(isset($_POST['stopped_date'])) {
				if($_POST['stopped_date'] != ""){
					$update_sql = ", stopped_date = '".$o_main->db->escape_str(date("Y-m-d H:i:s", strtotime($_POST['stopped_date'])))."'";
				} else {
					$update_sql = ", stopped_date = '0000-00-00 00:00:00'";
				}
			}
			if(isset($_POST['reminder_process_started'])) {
				if($_POST['reminder_process_started'] != ""){
					$update_sql .= ", reminder_process_started = '".$o_main->db->escape_str(date("Y-m-d H:i:s", strtotime($_POST['reminder_process_started'])))."'";
				} else {
					$update_sql .= ", reminder_process_started = '0000-00-00 00:00:00'";
				}
			}
			if(isset($_POST['collecting_cases_process_step_id'])) {				
				$update_sql .= ", collecting_cases_process_step_id = '".$o_main->db->escape_str($_POST['collecting_cases_process_step_id'])."'";
			}
			$s_sql = "UPDATE collecting_cases SET
			updated = now(),
			updatedBy='".$o_main->db->escape_str($username)."'".$update_sql."
			WHERE id = '".$o_main->db->escape_str($projectData['id'])."'";
...
$o_query = $o_main->db->query($s_sql, array($projectData['reminder_profile_id']));
...
$s_sql = "SELECT * FROM collecting_cases_process_steps WHERE collecting_cases_process_id = '".$o_main->db->escape_str($reminderProcess['id'])."'  ORDER BY sortnr ASC";
$o_query = $o_main->db->query($s_sql);
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_case_status";?>" method="post">
		...
		<div class="inner">
			<div class="line">
				...
				<div class="lineInput">
					<input type="text" class="popupforminput botspace datepicker" name="stopped_date" value="<?php if($projectData['stopped_date'] != "" && $projectData['stopped_date'] != "0000-00-00 00:00:00") echo date("d.m.Y", strtotime($projectData['stopped_date']))?>"/>
				</div>
				<div class="clear"></div>
			</div>
			<div class="line">
				<div class="lineTitle"><?php echo $formText_ReminderProcessStartedDate_Output; ?></div>
				<div class="lineInput">
					<input type="text" class="popupforminput botspace datepicker" name="reminder_process_started" value="<?php if($projectData['reminder_process_started'] != "" && $projectData['reminder_process_started'] != "0000-00-00 00:00:00") echo date("d.m.Y", strtotime($projectData['reminder_process_started']))?>"/>
				</div>
				<div class="clear"></div>
			</div>
			<div class="line">
				...
				<div class="lineInput">
					<select name="collecting_cases_process_step_id" autocomplete="off" >
						...
						foreach($all_steps as $step){
							?>
							<option value="<?php echo $step['id'];?>" <?php if($step['id'] == $projectData['collecting_cases_process_step_id']) echo 'selected';?>><?php echo $step['name'];?></option>
							<?php
						}
						?>
					</select>

<?php
$letter_id = $_POST['letter_id'] ? $o_main->db->escape_str($_POST['letter_id']) : 0;
$action = $_POST['action'] ? $_POST['action'] : '';
$status = $_POST['status'] ? $_POST['status'] : 0;

$sql = "SELECT * FROM collecting_company_cases WHERE id = $letter_id";
...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($letter_id) {
            ...
            $sql = "UPDATE collecting_cases_claim_letter SET
            updated = now(),
            updatedBy='".$variables->loggID."',
			content_status = 2,
			delete_comment = '".$o_main->db->escape_str($_POST['delete_comment'])."'
            WHERE id = $letter_id";
			$o_query = $o_main->db->query($sql);
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=delete_claimletter";?>" method="post">
		...
		<div class="inner">
			<div class="deletedInfo"><?php echo $formText_LetterWillBeDeleted_output." "; echo $formText_YouCanLeaveCommentWhyItIsGettingDeleted_output;?></div>
			<div class="line">
				<div class="lineTitle"><?php echo $formText_Comment_Output; ?></div>
				<div class="lineInput">
					<textarea class="popupforminput botspace" name="delete_comment"></textarea>
				</div>
				<div class="clear"></div>
			</div>

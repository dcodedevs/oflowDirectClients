...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($caseId) {
			...
			due_date='".date("Y-m-d", strtotime($o_main->db->escape_str($_POST['due_date'])))."'
            WHERE id = $caseId";
            ...
            WHERE creditor_id = '".$o_main->db->escape_str($creditor['id'])."' AND collectingcase_id = '".$o_main->db->escape_str($caseId)."'";
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_due_date";?>" method="post">
		...
		<div class="inner">
    		...
    		<div class="line">
        		...
        		<div class="lineInput">
                    <input type="text" class="popupforminput botspace datefield" autocomplete="off" name="due_date" value="<?php if($projectData['due_date'] != "0000-00-00" && $projectData['due_date'] != "") echo date("d.m.Y", strtotime($projectData['due_date'])); ?>" >
                </div>
        		<div class="clear"></div>
    		</div>

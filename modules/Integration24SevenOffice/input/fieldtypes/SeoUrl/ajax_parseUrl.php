...
if($_POST['seoUrlEditType'] == 2)
{
	...
	$s_list_prefix = "list-";
	if(strtolower($langID) == 'no') $s_list_prefix = "liste-";
	...
	if($o_query && $o_query->num_rows()>0) $row = $o_query->row_array();
	$levelName = strtolower(str_replace(array_keys($char_map), $char_map, trim(get_menulevel_parrents($_POST['menulevelID'], $langID).$row['levelname'])));

...
if($search != ""){
	...
	WHERE customer.content_status < 2 AND customer.creditor_id > 0 AND  (customer.name LIKE '%".$search."%' OR customer.middlename LIKE '%".$search."%' OR customer.lastname LIKE '%".$search."%'".$search_sql.")  ".$group_by." ORDER BY customer.name ASC";
} else {
	$s_sql = "SELECT customer.* FROM customer
	JOIN cs_mainbook_transaction cmt ON cmt.debitor_id = customer.id
	WHERE customer.content_status < 2 AND customer.creditor_id > 0 ".$group_by." ORDER BY customer.name ASC";
}
...
<script type="text/javascript">
	$(".employees .tablescript").unbind("click").bind("click", function(e){
		...
		if(employeeID > 0){
			<?php if($_POST['indexNumber'] > 0) { ?>
				$(".transaction_list .transaction_row").eq('<?php echo $_POST['indexNumber'];?>').find(".debitorId").val(employeeID);
				$(".transaction_list .transaction_row").eq('<?php echo $_POST['indexNumber'];?>').find(".selectDebitor").html(employeeName);
				$("#popupeditbox2 .b-close").click();
			<?php } else { ?>
				$(".debitor_selector .debitorChanger").val(employeeID).change();
				$(".debitor_selector span.labelText").html(employeeName);
	...
	$(".employees .pagination a").unbind("click").bind("click", function(e){
		...
		$.ajax({
			...
			type: 'POST',
			dataType: 'json',
			url: '<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=get_debitors";?>',
			data: _data,

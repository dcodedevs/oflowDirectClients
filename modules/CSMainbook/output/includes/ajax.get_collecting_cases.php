	...
	<div class="resultOnlyWrapper">
		<table class="table table-striped table-condensed">
			...
			<?php foreach($employees as $employee) { 				
				...
				collecting_company_case_id = '".$o_main->db->escape_str($employee['id'])."'
			...
			<tr>
				<td>
					<a href="#" class="tablescript" data-has-court-protected-lines="<?php if(count($courtFeeProtectedLines) > 0){ echo '1';} else { echo 0;}?>" data-employeeid="<?php echo $employee['id']?>" data-employeename="<?php echo $employee['id']." ".$employee['debitorName'];?>"><?php echo $employee['id']." ". $employee['debitorName'];?></a>
				</td
			</tr>
			<?php } ?>
			</tbody>
...
<script type="text/javascript">
	$(".employees .tablescript").unbind("click").bind("click", function(){
		...
		if(employeeID > 0){
			var protectedLines = $(this).data("has-court-protected-lines");
			$(".output-form2 #collectingCaseId").val(employeeID).data("has-court-protected-lines", protectedLines);

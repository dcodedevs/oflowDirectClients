...
$v_collecting_system_settings = $o_query ? $o_query->row_array() : array();

if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if(isset($v_collecting_system_settings['id'])) {
            $s_sql = "UPDATE collecting_system_settings SET
            updated = now(),
            updatedBy='".$o_main->db->escape_str($variables->loggID)."',
			accounting_close_last_date='".date("Y-m-d", strtotime($o_main->db->escape_str($_POST['accounting_close_last_date'])))."'
            WHERE id = '".$o_main->db->escape_str($v_collecting_system_settings['id'])."'";
...
<div class="popupform popupform-<?php echo $caseId;?>">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=".$_GET['folderfile']."&folder=".$_GET['folder']."&inc_obj=".$s_inc_obj."&inc_act=".$s_inc_act."";?>" method="post">
		...
		<input type="hidden" name="redirect_url" value="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output"; ?>">
		<div class="inner">

    		<div class="line">
        		<div class="lineTitle"><?php echo $formText_AccountingCloseLastDate_Output; ?></div>
        		<div class="lineInput">
                    <input type="text" class="popupforminput botspace datefield" autocomplete="off" name="accounting_close_last_date" value="<?php if($v_collecting_system_settings['accounting_close_last_date'] != "0000-00-00" && $v_collecting_system_settings['accounting_close_last_date'] != "") echo date("d.m.Y", strtotime($v_collecting_system_settings['accounting_close_last_date'])); ?>" >
                </div>
        		<div class="clear"></div>
    		</div>
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data.redirect_url !== undefined)
                    {
						...
						$(".popupform-<?php echo $caseId;?> #popup-validate-message").html("<?php echo $formText_ErrorOccurredSavingContent_Output;?>", true);

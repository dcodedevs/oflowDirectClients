...
$o_main->db->query("CREATE TABLE IF NOT EXISTS ".$rel_table." (".$rel_dropdown_field." INT NOT NULL, ".$rel_content_field." INT NOT NULL, ".$rel_input_field." TEXT,".($o_main->multi_acc?"`account_id` INT(11) NOT NULL DEFAULT 0,":"")." INDEX rel_idx (".$rel_dropdown_field.", ".$rel_content_field."".($o_main->multi_acc?", `account_id`":"")."))");

if($o_main->multi_acc && !$o_main->db->field_exists('account_id', $rel_table))
{
	$o_main->db->query("ALTER TABLE ".$rel_table." ADD COLUMN account_id INT NOT NULL DEFAULT 0");
}
...
$o_links = $o_main->db->query("SELECT * FROM ".$rel_table." WHERE ".$rel_content_field." = ?".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")."", array($ID));

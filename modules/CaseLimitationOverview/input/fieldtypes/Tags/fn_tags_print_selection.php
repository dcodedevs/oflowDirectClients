...
{
	$o_main = get_instance();
	$o_query = $o_main->db->query("SELECT t.id, IF(tc.tagname<>'',tc.tagname,t.name) name FROM sys_tag t JOIN sys_tagcontent tc ON tc.sys_tagID = t.id AND tc.languageID = ? WHERE t.parentID = ?".($o_main->multi_acc?" AND t.account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")." ORDER BY t.sortnr", array($s_default_output_language, $parentID));

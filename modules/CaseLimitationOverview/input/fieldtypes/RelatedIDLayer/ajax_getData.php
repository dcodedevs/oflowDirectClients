...
{
	$sql = "SELECT $dataTable.id cid, $dataTable.{$dataID[0]}, $dataTable.moduleID, ".implode(",",$sqlSelect)." FROM $dataTable LEFT JOIN {$dataTable}content ON {$dataTable}content.{$dataTable}ID = $dataTable.id AND {$dataTable}content.languageID = ".$o_main->db->escape($s_default_output_language).($s_extra_where!="" ? " WHERE ".$s_extra_where : "");
} else {

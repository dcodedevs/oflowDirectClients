		...
		if(!<?php echo $field_ui_id;?>handler)
		{
			...
			if(handle.length > 0)
			{
				...
				if(option[0] == 'process' && size[2] == 'c')
				{
					...
					$<?php echo $field_ui_id;?>image.attr('src','').attr('src', '<?php echo ($o_main->activate_cdn?'':$extradomaindirroot);?>' + option[5] + '?caID=<?php echo $_GET['caID'];?>&uid=' + option[1] + '<?php echo ($o_main->activate_cdn?"&cdn_token=' + $(".$field_ui_id."handle).data(\"cdn-token\") + '":"&username=".$_COOKIE['username']."&sessionID=".$_COOKIE['sessionID']."&companyID=".$_GET['companyID']."&server_id=".array_shift(explode('.', $_SERVER['HTTP_HOST'])) );?>&_=' + Math.random()).off('load').on('load', function(){
						$(this).cropper({
							strict:false,
							zoomable:false,
							rotatable:false,

...
$s_list_link = $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=list&main_filter=case&list_filter=".$list_filter."&list_filter_main=".$list_filter_main."&responsibleperson_filter=".$responsibleperson_filter."&projecttype_filter=".
$projecttype_filter."&projectcategory_filter=".$projectcategory_filter."&search_filter=".$search_filter."&search_by=".$search_by;
...
$s_sql = "SELECT * FROM collecting_cases_payment_plan WHERE  id = '".$o_main->db->escape_str($cid)."' ORDER BY created DESC";
$o_query = $o_main->db->query($s_sql);
$paymentPlan = ($o_query ? $o_query->row_array() : array());
					...
					<div class="p_contentBlock_wrapper">
						...
						<div class="p_contentBlock">
						    <div class="projectDetails">
								<div class="caseLabel">
									<?php echo $formText_PaymentPlan_output;?>
								</div>


								<table class="paymentPlanTable table table-borderless">
									<tr>
										<td><?php echo $formText_CollectingCase_Output;?></td>
										<td><?php echo $caseData['id']." ".$debitor['name']." ".$debitor['middlename']." ".$debitor['lastname'];?></td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td><?php echo number_format($paymentPlan['monthly_payment'], 2, ",", " ");?></td>
									</tr>
								</table>
								<table class="paymentPlanTableBig table table-borderless">
									<tr>
										...
										<th><?php echo $formText_BalanceBeforePayment_Output; ?></th>
										<th><?php echo $formText_BalanceAfterPayment_Output; ?></th>
									</tr>
									<?php
									$amountToPay = $totalSumDue;
									...
									foreach($collecting_cases_payment_plan_lines as $collecting_cases_payment_plan_line) {
										if(intval($collecting_cases_payment_plan_line['status']) == 0){
											$needToBePayed = $amountToPay - $collecting_cases_payment_plan_line['amount_to_pay'];
										} else {
											$needToBePayed = $amountToPay - $collecting_cases_payment_plan_line['payed'];
										}
										?>
										<tr>
											...
											<td><?php echo number_format($needToBePayed, 2, ",", " "); ?></td>
										</tr>
										<?php
										$amountToPay = $needToBePayed;
									}
									?>
								</table>
				   	        </table>

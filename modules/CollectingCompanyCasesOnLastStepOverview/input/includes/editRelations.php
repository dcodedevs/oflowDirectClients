...
if(isset($_POST['send']))
{
	...
	ftp_file_put_content($savefile, str_replace('$formText_', '[%]formText_', file_get_contents($file)));
	include($file);
	...
	$newFile = $_POST['choosenparentfield']."¤".$moduleInfoSplit[2]."¤".$moduleInfoSplit[1]."¤".$_POST['choosenmodulefield']."¤1¤".$_POST['newbutton']."¤".$_POST['showbutton']."¤".$_POST['choosennamefield']."¤".$_POST['contentstatusfilter']."¤".$s_relation_lang_variable."¤".$_POST['items_per_page']."¤".$_POST['expand_list']."¤".$_POST['in_hidden_list']."¤".$_POST['relation_link_to_module_id']."¤".$_POST['close_opened_items_after_reload']."¤";
	...
	foreach($prerelations as $x => $rest)
	{
		$newFile .= '"'.$rest.'",';
	}
	...
	$newFile = str_replace('[%]formText_', '$formText_', $newFile);
...
if(isset($_GET['editorder']))
{
	...
	if(!isset($_GET['relationedit']) && !isset($_GET['editorder']))
	{
		...
		<?php
		$counter = 0;
		foreach($prerelations as $name => $value)
		{
		...
		$(function() {
			$("a.delete_relation").off("click").on("click",function(e){
				e.preventDefault();
		...
		<tr>
			<td><?php echo $formText_CloseOpenedItemsAfterReload_input;?></td>
			<td><input type="checkbox" name="close_opened_items_after_reload" value="1"<?php echo ($relationValue[14]==1 ? 'checked':'');?> /></td>
		</tr>
		<tr>

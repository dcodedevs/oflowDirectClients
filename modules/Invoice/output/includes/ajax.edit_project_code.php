...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		...
		{
			$o_main->db->query("UPDATE customer_collectingorder SET accountingProjectCode = '".$o_main->db->escape_str($_POST['accountingProjectCode'])."' WHERE id = '".$o_main->db->escape_str($v_collectingorder['id'])."'");
        }
...
<div class="popupform">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_project_code";?>" method="post">
		...
		<h4><?php echo $formText_EditProjectCode_Output;?></h4>
		<div class="inner">
    		<div class="line">
        		<div class="lineTitle"><?php echo $formText_ProjectCode_Output; ?></div>
        		<div class="lineInput">
                    <input type="text" class="popupforminput botspace" name="accountingProjectCode" value="<?php echo $v_collectingorder['accountingProjectCode']; ?>">
                </div>
        		<div class="clear"></div>
    		</div>
		</div>

...
function get_customer_list($o_main, $filter, $search_filter, $search_by, $page=1, $perPage=100, $customer_id = null) {
    ...
    if ($search_filter) {
        switch($search_by){
            case 1:
            $search_filter_sql_join = "";
            $search_filter_sql_where = " AND (gate.phone LIKE '%".$o_main->db->escape_like_str($search_filter)."%')";
            break;
            case 2:
            $search_filter_sql_join = "";
    ...
    if($filter == "active"){
        $with_orders_sql_join = " ";
        $with_orders_sql_where = " AND (gate.deleted is null OR gate.deleted = '0000-00-00 00:00:00')";
    } 
    ...
    $sql = "SELECT gate.*, c.name customerName, cp.name contactPersonName  
            FROM contactperson_gate_access gate
            LEFT OUTER JOIN contactperson cp ON cp.id = gate.contactpersonId
    ...
    if($page == 0 && $perPage == 0){
        ...
        $sql .= " ORDER BY gate.id DESC".$pager;
        $f_check_sql = $sql;

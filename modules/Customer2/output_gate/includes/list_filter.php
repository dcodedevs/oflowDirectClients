...
<div class="output-filter">
    <ul>  
        <li class="item<?php echo ($list_filter == 'active' ? ' active':'');?>">
            <a class="topFilterlink" data-listfilter="all" href="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output_gate&list_filter=active"; ?>">
                <span class="link_wrapper">
                    <?php echo $formText_ActiveGateAccesses_output;?>
                </span>
            </a>
        </li>      
        <li class="item<?php echo ($list_filter == 'all' ? ' active':'');?>">
            <a class="topFilterlink" data-listfilter="all" href="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output_gate&list_filter=all"; ?>">
                <span class="link_wrapper">
    ...
    <div class="p_tableFilter_right">
        <form class="searchFilterForm" id="searchFilterForm">
            <select name="searchBy" class="searchBy">
                <option value="1" <?php if($search_by == 1) echo 'selected';?>><?php echo $formText_PhoneNumber_output_gate;?></option>
                <option value="2" <?php if($search_by == 2) echo 'selected';?>><?php echo $formText_Customer_output_gate;?></option>
                <option value="3" <?php if($search_by == 3) echo 'selected';?>><?php echo $formText_ContactPerson_output_gate;?></option>
            </select>

...
if($s_inc_obj != "ajax")
{
    ...
    $currentYear = date("Y");
    ?>
    <div class="output-category-block" id="offers_block">
        <div class="output-title-different"><?php echo $formText_Offers_output;?></div>
        <div class="rs_box extra">
            <div class="output-content" id="offers_info">
                <div class="loading"><div class="lds-ring"><div></div><div></div><div></div><div></div></div></div>
            </div>
        </div>
    </div>
    <style>
    #offers_block .output-content .output-content-sub-title {
        font-size: 14px;
        font-weight: bold;
		padding: 0px 0px 5px;
	}
    ...
    <script type="text/javascript">
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            url: '<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&abortable=1&module=Customer2&folderfile=output_redirect&subfolder=".basename(__DIR__)."&folder=output_dashboard&inc_obj=ajax&inc_act=get_info"; ?>',
            data:  "fwajax=1&fw_nocss=1",
            success: function(json){
                setTimeout(function() {
                    $('#offers_info').html(json.html);
                }, 500);
            }
        }).fail(function() {
        });

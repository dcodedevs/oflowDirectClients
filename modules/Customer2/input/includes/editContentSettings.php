...
if(isset($_POST['send']))
{
	...
	if($access >= 10)
	{
		...
		header("Location: ".substr($_SERVER['HTTP_REFERER'],0,strpos($_SERVER['HTTP_REFERER'],"?"))."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$_GET['module']."&submodule=".$_GET['submodule']."");
	} else {
...
<form action="../modules/<?php echo $_GET['module']."/input/includes/".$_GET['includefile'].".php?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$_GET['module']."&submodule=".$_GET['submodule']."&includefile=".$_GET['includefile'];?>" method="post">
	...
	<input type="hidden" name="seodataID" value="<?php echo $seodataID; ?>" />
	<input type="hidden" name="moduleID" value="<?php echo $moduleID; ?>" />
	<input type="hidden" name="loginType" value="<?php echo ($access < 211?1:2) ?>" />
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr><td width="15%">Set as startpage</td><td width="85%" class="fieldholder"><input type="checkbox" name="startpage" value="1"<?php echo ((isset($startpage['startpage']) && 1 == $startpage['startpage']) ? ' checked': '');?> style="width:auto;" /></td></tr>
	<?php
	$o_query = $o_main->db->query('SELECT languageID, name FROM language WHERE outputlanguage = 1 ORDER BY defaultOutputlanguage DESC, sortnr ASC');
	if($o_query && $o_query->num_rows()>0)
	{
		...
		{
			?>
			<tr><td colspan="2"><strong><?php echo $writeLanguage['name']; ?></strong></td></tr>
			<tr><td>Title</td><td class="fieldholder"><input type="text" name="contentTitle_<?php echo $writeLanguage['languageID']; ?>" value="<?php echo (isset($contentTitle[$writeLanguage['languageID']]) ? $contentTitle[$writeLanguage['languageID']] : ''); ?>" /></td></tr>
			<tr><td valign="top">Description</td><td class="fieldholder"><textarea name="contentDescription_<?php echo $writeLanguage['languageID']; ?>"><?php echo (isset($contentDescription[$writeLanguage['languageID']]) ? $contentDescription[$writeLanguage['languageID']] : ''); ?></textarea></td></tr>
			<tr><td valign="top">Keywords</td><td class="fieldholder"><textarea name="contentKeywords_<?php echo $writeLanguage['languageID']; ?>"><?php echo (isset($contentKeywords[$writeLanguage['languageID']]) ? $contentKeywords[$writeLanguage['languageID']] : ''); ?></textarea></td></tr>
			<?php
		}
	}
	?>

...
$creditor_only = $v_data['params']['creditor_only'];
...
if($companyID != "" && $email != ""){
	...
	$creditor_join = "";
	if($creditor_only) {
		$creditor_join = " JOIN creditor_getynet_pay ON creditor_getynet_pay.customer_id = customer.id";
	}
	if($v_data['params']['useradmin']) {
		$s_sql = "SELECT customer.* FROM customer ".$creditor_join."
		WHERE 1=1 AND customer.content_status < 2 GROUP BY customer.id ORDER BY customer.name";
	} else {
		$s_sql = "SELECT customer.* FROM customer JOIN contactperson ON contactperson.customerId = customer.id ".$creditor_join." WHERE contactperson.email = ?  AND customer.content_status < 2 GROUP BY customer.id ORDER BY customer.name";
	}
	...
	foreach($customers as $customer) {
		$s_sql = "SELECT * FROM creditor WHERE creditor.customer_id = ?";

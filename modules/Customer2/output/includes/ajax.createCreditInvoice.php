...
if($o_query && $o_query->num_rows()>0) {
    ...
    if($createOrder){
        ...
        $o_query2 = $o_main->db->query($sql, array($collectingOrders[0]['contactpersonId'], $collectingOrders[0]['customerId'], $collectingOrders[0]['accountingProjectCode'], $collectingOrders[0]['department_for_accounting_code'], $collectingOrders[0]['ownercompanyId'],
        $collectingOrders[0]['reference'], $collectingOrders[0]['delivery_date'],$collectingOrders[0]['delivery_address_line_1'],$collectingOrders[0]['delivery_address_line_2'],$collectingOrders[0]['delivery_address_city'],
        ...
        foreach($collectingOrders as $collectingOrder){
            ...
            $o_query = $o_main->db->query($s_sql, array($collectingOrder['invoiceNumber']));
        	...
        	foreach($orders as $order){
        		...
        		$o_main->db->query("INSERT INTO orders SET moduleID = ?, createdBy = ?, created = NOW(), projectLeader = ?,
                    ...
                    array(0, $variables->loggID, $order['projectLeader'], $order['contactPerson'], $order['articleNumber'], $formText_CreditRegardingInvoiceNr.' '.$invoice['external_invoice_nr'].' - '.$order['articleName'], $order['describtion'],
                    '-1'*$order['amount'], $order['pricePerPiece'], $order['discountPercent'], ('-1'*$order['amount'])*$order['pricePerPiece']*(100-$order['discountPercent'])/100, $order['delieveryDate'], $order['expectedTimeuseMinutes'],
        		...
        		$s_message = "";
        		$s_action = "created credit invoice";
...
if($warningMessage != ""){
	...
	<div class="popupform">
		...
		<form class="output-form output-worker-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=createCreditInvoice";?>" method="post">

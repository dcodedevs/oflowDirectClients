...
$(document).ready(function() {
  ...
  $(document).on('click', '.' + instancePrefixClass +  ' .fwaFileupload_FilesList_Files .item .delete_stored', function(e){
      e.preventDefault();
   ...
   */
   <?php
   $fileuploadLink = '../modules/'.$fwaFileuploadConfig['module_folder'].'/output/includes/fileupload10/ajax_upload.php?param_name='.$fwaFileuploadConfig['id'].'_files&file_type='.$fwaFileuploadConfig['file_type'];
  ...
  function fwaFileuploadInit() {
    ...
    //'<?php echo '../modules/'.$fwaFileuploadConfig['module_folder'].'/output/includes/fileupload10/ajax_upload.php?param_name='.$fwaFileuploadConfig['id'].'_files'; ?>',
    ...
    $('#<?php echo $fwaFileuploadConfig['id'];?>_upload').unbind().fileupload({
      ...
      add: function (e, data) {
          $.getJSON("<?php echo '../modules/'.$fwaFileuploadConfig['module_folder'].'/output/includes/fileupload10/get_next_id.php?type='.$fwaFileuploadConfig['upload_type'].'&time='.time();?>", function (result) {
              ...
              result.content_module_id = '<?php echo $fwaFileuploadConfig['content_module_id'];?>';
              result.content_table = '<?php echo $fwaFileuploadConfig['content_table'];?>';
              result.content_field = '<?php echo $fwaFileuploadConfig['content_field'];?>';
              result.fileupload_session_id = fileupload_session_id;
        ...
        if(data.result.message){
            ...
            $.each(data.result.<?php echo $fwaFileuploadConfig['id'];?>_files, function (index, file) {
                if(file.error) {
                    ...
                    if($('.' + instancePrefixClass +  ' .fwaFileupload_FilesList_Files .item:not(.deleted):not(.loading)').length >= <?php echo $fwaFileuploadConfig['id'];?>limit)
                    {
                        ...
                        var oDiv = $('[data-tmp-upload-id="' + btoa(encodeURIComponent(file.name)) + '"]').eq(0);

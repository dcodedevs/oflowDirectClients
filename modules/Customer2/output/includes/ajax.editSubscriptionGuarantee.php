...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($subscribtionId) {
			if(intval($_POST['property_guarantee']) == 0){
				$_POST['property_guarantee_amountpledged'] = 0;
			}
			if(intval($_POST['property_guarantee_agreed']) == 0){
				$_POST['property_guarantee_agreed'] = 0;
			}
            $s_sql = "UPDATE subscriptionmulti SET
            updated = now(),
            updatedBy= ?,
			property_guarantee = ?,
			property_guarantee_amountpledged = ?,
			property_guarantee_agreed = ?
            WHERE id = ?";
            $o_main->db->query($s_sql, array($variables->loggID,
			$_POST['property_guarantee'],  $_POST['property_guarantee_amountpledged'],$_POST['property_guarantee_agreed'],
			 $subscribtionId));
...
<div class="popupform">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editSubscriptionGuarantee";?>" method="post">
		...
		<div class="inner">
			<div class="line ">
				<div class="lineTitle"><?php echo $formText_Guarantee_Output; ?></div>
				<div class="lineInput">
					<?php
					...
					?>
					<select name="property_guarantee">
						<option value=""><?php echo $formText_No_output;?></option>
						<?php foreach($guaranteeArray as $key => $value) {?>
							<option value="<?php echo $key+1;?>" <?php if($subscribtionData['property_guarantee'] == ($key+1)) echo 'selected'; ?>><?php echo $value;?></option>
						<?php } ?>
					</select>
				</div>
				<div class="clear"></div>
			...
			<div class="line ">
				<div class="lineTitle"><?php echo $formText_GuaranteeAmountPledged_Output; ?></div>
				<div class="lineInput">
					<input type="text" class="popupforminput botspace" name="property_guarantee_amountpledged" value="<?php echo $subscribtionData['property_guarantee_amountpledged']; ?>" autocomplete="off">
				</div>
				<div class="clear"></div>
			</div>
			<div class="line ">
				<div class="lineTitle"><?php echo $formText_GuaranteeAmountAgreed_Output; ?></div>
				<div class="lineInput">
					<input type="text" class="popupforminput botspace" name="property_guarantee_agreed" value="<?php echo $subscribtionData['property_guarantee_agreed']; ?>" autocomplete="off">
				</div>
				<div class="clear"></div>
			</div>

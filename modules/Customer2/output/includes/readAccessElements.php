...
if(isset($menuAccess[$s_current_module][4]['data'][0]['access_element'])){ $accessElements = $menuAccess[$s_current_module][4]['data'][0]['access_element']; } else { $accessElements = array(); }
...
if($variables->useradmin){
    ...
    foreach($allAccessElements as $singleAccessElement) {
        ${"accessElementRestrict_".$singleAccessElement['name']} = false;
    }
}
?>

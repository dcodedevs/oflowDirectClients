...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($subscribtionId) {
            ...
            updated = now(),
            updatedBy= ?,
			property_break_closes = ?,
			property_break_closes_text = ?
            WHERE id = ?";
            $o_main->db->query($s_sql, array($variables->loggID,
			$_POST['property_break_closes'],  $_POST['property_break_closes_text'],
			 $subscribtionId));
...
<div class="popupform">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editSubscriptionPropertyBreakCloses";?>" method="post">
		...
		<div class="inner">
			<div class="line ">
				...
				<div class="lineInput">
					...
					<input class="propertyOption" id="optionyes" type="radio" name="property_break_closes" value="1" <?php if($subscribtionData['property_break_closes'] == 1) echo 'checked'; ?>/>
					<label for="optionno"><?php echo $formText_No_output;?></label>
					<input class="propertyOption" id="optionno" type="radio" name="property_break_closes" value="0" <?php if($subscribtionData['property_break_closes'] == 0) echo 'checked'; ?>/>
				</div>
				<div class="clear"></div>
			</div>
			<div class="line propertyOptionTextWrapper" <?php if($subscribtionData['property_break_closes_text'] == 0) echo 'style="display: none;"';?> >
				<div class="lineTitle"><?php echo $formText_Text_Output; ?></div>
				<div class="lineInput">
					<textarea class="popupforminput botspace" name="property_break_closes_text"  autocomplete="off"><?php echo $subscribtionData['property_break_closes_text']; ?></textarea>
				</div>
				<div class="clear"></div>
			</div>

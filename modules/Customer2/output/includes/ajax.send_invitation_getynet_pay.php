				...
				if($v_row){
					...
					if(isset($response->error))
					{
						...
						$v_param = array(
							...
							'LANGUAGE_ID' => 'no'//$fw_session['accountlanguageID']
						);
						...
						if(isset($invitationresponse['status']) && 1 == $invitationresponse['status'])
						{
							...
							print_r($invitationresponse);
						}
					}

					if($isregistered == 1)
					{
						echo $formText_AccessGranted_Output." - ".$v_row['email']."</br>";
					} else {
						echo $formText_ErrorOccured_Output." - ".$v_row['email']."</br>";
					}
				}
			}
		} else {
			$isregistered = false;
			$s_error_msg .= "<br/>".$formText_AccessLevelWasNotSetInConfig_output;
		}
	} else {
		$s_error_msg .= "<br><br>".$formText_InvitationConfigurationError_Output;
	}
?>
<?php  echo $s_error_msg;?>
</div>
</div>

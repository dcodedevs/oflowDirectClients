...
<div id="out-batch-list">
	...
	$s_sql = "SELECT bi.* FROM batch_invoicing bi JOIN invoice i ON  i.batch_id = bi.id WHERE i.sentByEmail IS NULL OR i.sentByEmail = '' AND i.customerId = '".$customerId."'  GROUP BY bi.id ORDER BY bi.created DESC";
	...
	foreach($v_rows as $v_row)
	{
		?><tr>
			...
			<td><a target="_blank" href="<?php echo $extradir."/../BatchInvoicing/output/ajax.download.php?accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&ID=".$v_row["id"];?>"><?php echo $formText_Download_Output;?></a></td>
		</tr><?php
	}
	?>
	</tbody>

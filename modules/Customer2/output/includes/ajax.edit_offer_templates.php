...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		...
		if($_POST['editResource'] > 0) {
			$s_sql = "UPDATE offer_template SET
			updated = now(),
			updatedBy= ?,
			name=?,
			frontpage_headline= ?,
			...
			offer_end_text= ?
			WHERE id = ?";
			$o_main->db->query($s_sql, array($variables->loggID, $_POST['resourceName'],$_POST['frontpageHeadline'], $_POST['frontpageText'],$_POST['resourceHeadline'], $_POST['resourceText'], $_POST['resourceText2'], $_POST['editResource']));
			$fw_return_data = $_POST['editResource'];
			$fieldId = $fw_return_data;
		}
		else if(intval($_POST['deleteResource']) == 0) {
			$s_sql = "INSERT INTO offer_template SET
			id=NULL,
			moduleID = ?,
			created = now(),
			createdBy= ?,
			...
			offer_intro_text= ?,
			offer_end_text= ?";
			$o_main->db->query($s_sql, array($moduleID, $variables->loggID, $_POST['resourceName'],$_POST['frontpageHeadline'], $_POST['frontpageText'],$_POST['resourceHeadline'], $_POST['resourceText'], $_POST['resourceText2']));
			$fw_return_data = $o_main->db->insert_id();
			$fieldId = $fw_return_data;
		} else {
			$s_sql = "DELETE offer_template FROM offer_template WHERE offer_template.id = ?";
...
<div class="popupform">
	...
	<div class="popupformTitle"><?php echo $formText_AddEditOfferTemplates_output;?></div>
	...
	data-action2="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_offer_templates";?>">
	<?php

	$resources = array();
	$s_sql = "SELECT * FROM offer_template WHERE content_status < 2 ORDER BY sortnr";
	$o_query = $o_main->db->query($s_sql);
			...
			<?php if(!$resource['disabledForEditing']) { ?>
				...
				<div class="editRow">
					...
					<div class="editInnerRow">
						...
						<div class="inputBlock">
							<input type="text" name="resourceHeadline" id="resourceHeadline<?php echo $resource['id']?>" value="<?php echo $resource['offer_headline']?>" autocomplete="off"/>
						</div>
					</div>
					<?php if($v_customer_accountconfig['activateOfferFrontpage']) { ?>
	                    <div class="editInnerRow">
							<label><?php echo $formText_FrontpageText_Output?></label>
							<div class="inputBlock">
	                            <textarea name="frontpageText" class="ckeditor" id="frontpageText<?php echo $resource['id']?>"><?php echo $resource['frontpage_text'];?></textarea>
							</div>
						</div>
					<?php } ?>
                    <div class="editInnerRow">
						...
						<div class="inputBlock">
                            <textarea name="resourceText" class="ckeditor" id="resourceText<?php echo $resource['id']?>"><?php echo $resource['offer_intro_text'];?></textarea>
						</div>
					</div>
                    <div class="editInnerRow">
						<label><?php echo $formText_OfferEndText_Output?></label>
						<div class="inputBlock">
                            <textarea name="resourceText2" class="ckeditor" id="resourceText2<?php echo $resource['id']?>"><?php echo $resource['offer_end_text'];?></textarea>
						</div>
					</div>
					<div class="actionRow">
	...
	<div class="newResource editRow">
		...
		<?php if($v_customer_accountconfig['activateOfferFrontpage']) { ?>
			<div class="editInnerRow">
				<label><?php echo $formText_FrontpageText_Output?></label>
				<div class="inputBlock">
					<textarea name="frontpageText" class="ckeditor" id="frontpageText0"></textarea>
				</div>
			</div>
		<?php } ?>
        <div class="editInnerRow">
            <label><?php echo $formText_OfferIntroText_Output?></label>
            <div class="inputBlock">
                <textarea name="resourceText" class="ckeditor" id="resourceText0"></textarea>
            </div>
        </div>
        <div class="editInnerRow">
            <label><?php echo $formText_OfferEndText_Output?></label>
            <div class="inputBlock">
                <textarea name="resourceText2" class="ckeditor" id="resourceText20"></textarea>
            </div>
        </div>
		<div class="actionRow">
	...
	<div class="addNew">
		<div class="plusTextBox active">
			...
			<div class="text"><?php echo $formText_AddOfferTemplate_output; ?></div>
		</div>
		<div class="clear"></div>
	</div>
...
$('#popupeditbox2 textarea.ckeditor').each(function () {
	var $textarea = $(this);
	...
	function bindPopupActions(){
		...
		$("[data-resource-save-id").unbind("click").on('click', function(e){
			...
			var resourceID = $(this).data('resource-save-id'),
				...
				frontpageHeadline = $("#frontpageHeadline"+resourceID).val(),
				frontpageText = $("#frontpageText"+resourceID).val(),
				self = $(this);
	        ...
	        if(resourceName != ""){
				...
				var data = {
					...
					resourceHeadline: resourceHeadline,

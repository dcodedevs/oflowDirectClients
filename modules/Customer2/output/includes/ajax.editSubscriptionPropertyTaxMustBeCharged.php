...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($subscribtionId) {
            ...
            updated = now(),
            updatedBy= ?,
			property_tax_must_be_charged = ?
            WHERE id = ?";
            $o_main->db->query($s_sql, array($variables->loggID, $_POST['property_tax_must_be_charged'],
			 $subscribtionId));
...
<div class="popupform">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editSubscriptionPropertyTaxMustBeCharged";?>" method="post">
		...
		<div class="inner">
			<div class="line ">
				<div class="lineTitle"><?php echo $formText_PropertyTaxMustBeCharged_output; ?></div>
				<div class="lineInput">
					<label for="optionyes"><?php echo $formText_Yes_output;?></label>
					<input class="propertyOption" id="optionyes" type="radio" name="property_tax_must_be_charged" value="1" <?php if($subscribtionData['property_tax_must_be_charged'] == 1) echo 'checked'; ?>/>
					<label for="optionno"><?php echo $formText_No_output;?></label>
					<input class="propertyOption" id="optionno" type="radio" name="property_tax_must_be_charged" value="0" <?php if($subscribtionData['property_tax_must_be_charged'] == 0) echo 'checked'; ?>/>
				</div>
				<div class="clear"></div>
			</div>

...
if(isset($_POST['output_form_submit']))
{
    ...
    if ($_POST['imagesToProcess']) {
        ...
        $s_sql = "SELECT * FROM order_confirmation_attached_files";
        $o_query = $o_main->db->query($s_sql);
        $customer_file = $o_query ? $o_query->row_array() : array();
        if(!$customer_file){
            $s_sql = "INSERT INTO order_confirmation_attached_files SET created = NOW()";
            $o_query = $o_main->db->query($s_sql);
...
<div class="popupform">
    <form class="output-content-form main" action="<?php echo $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=attachFilesToConfirmationAll";?>" method="post">
    	<input type="hidden" name="fwajax" value="1">
    	<input type="hidden" name="fw_nocss" value="1">
    	<input type="hidden" name="output_form_submit" value="1">
        <?php
        $s_sql = "SELECT order_confirmation_attached_files.* FROM order_confirmation_attached_files
        ORDER BY id";
        $o_query = $o_main->db->query($s_sql);
                ...
                ?>
                    <tr>
                        ...
                        <td style="padding: 0 10px;" width="10%" style="text-align: right;">
                            ...
                            <button class="output-btn small output-delete-attachedfile-confirmation-all editBtnIcon" data-offer-id="<?php echo $offerFiles['id']; ?>" data-uid="<?php echo $file[4];?>">
        ...
        $fwaFileuploadConfig = array (
          ...
          'content_table' => 'order_confirmation_attached_files',
          'content_field' => 'file',
...
$(document).ready(function() {
    ...
    $(".output-delete-attachedfile-confirmation-all").off("click").on('click', function(e){
        e.preventDefault();
        var self = $(this);
        var data = {
            uid: $(this).data('uid'),
        ...
        bootbox.confirm('<?php echo $formText_ConfirmDeleteAttachedFile_output; ?>', function(result) {
            if (result) {
                ajaxCall('deleteAttachedFileConfirmationAll', data, function(json) {
                    var data = {
                    };
                    ajaxCall('attachFilesToConfirmationAll', data, function(json) {

...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($subscribtionId) {
			$property_administration_surcharge_nu_in_percent = str_replace(",", ".", $_POST['property_administration_surcharge_nu_in_percent']);
            $s_sql = "UPDATE subscriptionmulti SET
            updated = now(),
            updatedBy= ?,
			property_administration_surcharge_nu_in_percent = ?
            WHERE id = ?";
            $o_main->db->query($s_sql, array($variables->loggID, $property_administration_surcharge_nu_in_percent,
			 $subscribtionId));
...
<div class="popupform">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editSubscriptionPropertyAdministrationSurcharge";?>" method="post">
		...
		<div class="inner">
			<div class="line">
				<div class="lineTitle"><?php echo $formText_AdministrationSurchargeNuInPercent_output; ?></div>
				<div class="lineInput">
					<input class="popupforminput botspace" name="property_administration_surcharge_nu_in_percent" value="<?php echo number_format($subscribtionData['property_administration_surcharge_nu_in_percent'], 2, ",", ""); ?>"  autocomplete="off"/>
				</div>
				<div class="clear"></div>
			</div>

...
if ($uid && $collectingorderId) {
    ...
    if($o_query && $o_query->num_rows() > 0){
        ...
        $files = json_decode($entryData['files_attached_to_invoice']);
        $newFiles = array();
        ...
        $o_query = $o_main->db->query("UPDATE customer_collectingorder SET  files_attached_to_invoice = ? WHERE id = ?", array($newFilesJson, $collectingorderId));
    }
}
?>

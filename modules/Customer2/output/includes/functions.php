...
function get_customer_list($o_main, $filter, $city_filter, $search_filter,  $activecontract_filter, $selfdefinedfield_filter = null, $search_by, $page=1, $perPage=100, $customer_id = null, $s_custom_order_by = '') {
    ...
    $sql = "SELECT ".($b_get_count?($groupBySubscription?"COUNT(s.id)":"COUNT(c.id)")." AS cnt":"c.created, c.id customerId, c.name,c.middlename,c.lastname, c.publicRegisterId, c.shop_name,
            CONCAT(COALESCE(c.name, ''), ' ', COALESCE(c.middlename, ''), ' ', COALESCE(c.lastname, '')) as customerName, c.creditor_customer_id, c.paCity, c.paStreet, c.creditor_id".$customer_field_sql_select.$selfdefinedfield_sql_select.$with_subs_sql_select.$search_filter_sql_select)."
            FROM customer c
    ...
    if($customer_id != null) {
        ...
        if($groupBySubscription) {
            ...
            // $sql .= " GROUP BY c.id ".(''!=$s_custom_order_by ? $s_custom_order_by : "ORDER BY c.id ASC").$pager;
        }
        $sql .= $pager;
        ...
        if($b_get_count){
			...
			// 	$sql .= " GROUP BY c.id";
			// }
			// $sql .= (''!=$s_custom_order_by ? $s_custom_order_by : " ORDER BY c.id ASC").$pager;
            $sql .= $pager;
            $f_check_sql = $sql;

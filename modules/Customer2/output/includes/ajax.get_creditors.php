...
if($search != ""){
	$s_sql = "SELECT customer.*, creditor.id FROM customer
	LEFT JOIN creditor ON creditor.customer_id = customer.id
	WHERE creditor.content_status < 2 AND customer.name LIKE '%".$search."%' ORDER BY customer.name ASC";
} else {
	$s_sql = "SELECT customer.*, creditor.id FROM customer
	LEFT JOIN creditor ON creditor.customer_id = customer.id
	WHERE creditor.content_status < 2  ORDER BY customer.name ASC";
}
...
<script type="text/javascript">
	...
	$(".employees .pagination a").unbind("click").bind("click", function(){
		var _data = { fwajax: 1, fw_nocss: 1, owner:'<?php echo $_POST['owner']?>', search: $("#employeesearch").val(), resultOnly: true, page: $(this).html(), subtitueWorker: '<?php echo $_POST['subtitueWorker']?>', subtitueAll: '<?php echo $_POST['subtitueAll']?>'};
		$.ajax({
			cache: false,
			type: 'POST',
			dataType: 'json',
	...
	<?php if(!$resultOnly){ ?>
		$(".employeeSearch button").unbind("click").bind("click", function(){
			var _data = { fwajax: 1, fw_nocss: 1, owner:'<?php echo $_POST['owner']?>', search: $("#employeesearch").val(), resultOnly: true, workplanlineworkerId: '<?php echo $_POST['workplanlineworkerId']?>', subtitueWorker: '<?php echo $_POST['subtitueWorker']?>', subtitueAll: '<?php echo $_POST['subtitueAll']?>'};
			$.ajax({
				cache: false,
				type: 'POST',
				dataType: 'json',

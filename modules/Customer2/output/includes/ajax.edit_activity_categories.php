...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		if($_POST['editResource'] > 0) {
			$s_sql = "UPDATE customer_activity_types SET
			updated = now(),
			updatedBy= ?,
			name= ?
			WHERE id = ?";
			...
			$s_sql = "INSERT INTO customer_activity_types SET
			id=NULL,
			moduleID = ?,
			created = now(),
			createdBy= ?,
			...
			$s_sql = "SELECT * FROM customer_activity WHERE activity_type_id = ?";
			$o_query = $o_main->db->query($s_sql, array($_POST['deleteResource']));
		    $attached_activities = $o_query ? $o_query->result_array() : array();
			if(count($attached_activities) == 0) {
				$s_sql = "DELETE customer_activity_types  FROM customer_activity_types WHERE customer_activity_types.id = ?";
...
<div class="popupform">
	...
	<div class="popupformTitle"><?php echo $formText_AddEditActivityType_output;?></div>
	<div class="errorMessage"></div>
	<div class="resourceList" data-action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_activity_categories";?>"
	data-action2="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=edit_activity_categories";?>">
	<?php

	$resources = array();
	$s_sql = "SELECT * FROM customer_activity_types WHERE content_status < 2 ORDER BY sortnr";
			...
			<?php if(!$resource['disabledForEditing']) { ?>
				...
				<div class="editRow">
					...
					<div class="editInnerRow">
						<label><?php echo $formText_ActivityTypeName_output?></label>
	...
	<div class="addNew">
		<div class="plusTextBox active">
			...
			<div class="text"><?php echo $formText_AddActivityType_output; ?></div>
		</div>
		<div class="clear"></div>
	</div>
				...
				success: function(result){
					...
					if(parseInt(result.html) != 0){
						if(parseInt(result.html) == -1){
							$(".popupform .errorMessage").html("<?php echo $formText_CanNotDeleteTypeWithAttachedActivity_output;?>").show();
						} else {

...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($subscribtionId) {
            ...
            updated = now(),
            updatedBy= ?,
			property_separate_provision = ?
            WHERE id = ?";
            $o_main->db->query($s_sql, array($variables->loggID, $_POST['property_separate_provision'],
			 $subscribtionId));
...
<div class="popupform">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editSubscriptionPropertySeparateProvision";?>" method="post">
		...
		<div class="inner">
			<div class="line">
				<div class="lineTitle"><?php echo $formText_SeparateProvision_output; ?></div>
				<div class="lineInput">
					<textarea class="popupforminput botspace" name="property_separate_provision"  autocomplete="off"><?php echo $subscribtionData['property_separate_provision']; ?></textarea>
				</div>
				<div class="clear"></div>
			</div>

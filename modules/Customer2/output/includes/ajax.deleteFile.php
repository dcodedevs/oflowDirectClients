...
if ($projectFileId) {
    ...
    $s_sql = "SELECT * FROM subscriptionmulti_files WHERE id = ?";
    $o_query = $o_main->db->query($s_sql, array($projectFileId));
    if($o_query && $o_query->num_rows() > 0){
        $entryData = $o_query->row_array();
        if($entryData){
            $o_query = $o_main->db->query("DELETE subscriptionmulti_files FROM subscriptionmulti_files WHERE id = ?", array($projectFileId));
            if($o_query){
                $file_url = "";

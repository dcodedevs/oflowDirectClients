...
if($moduleAccesslevel > 10) {
	if(isset($_POST['output_form_submit'])) {
        if ($subscribtionId) {
            ...
            WHERE id = ?";
            $o_main->db->query($s_sql, array($variables->loggID, $_POST['seperateInvoiceFromSubscription'], $subscribtionId));
...
<div class="popupform">
	...
	<form class="output-form main" action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=editSubscriptionSeperateInvoice";?>" method="post">
		...
		<div class="inner">
            <div class="line renewalAppearanceDays">
                <div class="lineTitle"><?php echo $formText_SeperateInvoiceFromSubscription_Output; ?></div>
                <div class="lineInput">
                    <input type="checkbox" class="checkbox popupforminput botspace" name="seperateInvoiceFromSubscription" value="1" <?php if($subscribtionData['seperateInvoiceFromSubscription']) echo 'checked';?>>
                </div>
            </div>
		</div>

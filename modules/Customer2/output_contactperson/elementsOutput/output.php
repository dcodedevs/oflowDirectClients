...
$sql = "SELECT p.* FROM contactperson p WHERE (p.id is not null) AND type ? ";
...
if($data['status'] == 1){
	foreach($data['items'] as $item){
		...
		$isMember = false;
		$isAdmin = false;
		$data = json_decode(APIconnectorUser("group_user_get_list", $variables->loggID, $variables->sessionID, array('group_id'=>$item['id'])),true);
		if($data['status'] == 1){
			$members = $data['items'];
		...
		array_push($allGroupInfo, $item);
		if(($item['show_only_for_members'] && $isMember) || !$item['show_only_for_members']){
			if(($item['show_only_for_admins'] && $isAdmin) || !$item['show_only_for_admins']){

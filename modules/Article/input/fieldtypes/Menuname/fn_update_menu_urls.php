<?php
function update_menu_urls($modRewriteRow, $char_map, $s_list_prefix)
{
	$o_main = get_instance();
	...
	if($modRewriteRow['menulevelID'] > 0 && $modRewriteRow['id'] > 0)
	{
		...
		//Do not update records where menulevel not choosen for URL
		//if($modRewriteRow['lang_url_part'] == '') $b_handle_content = FALSE;
	...
	$modRewriteSql = "SELECT pageID.id, menulevelcontent.menulevelID, pageID.contentID, pageID.contentTable, menulevelcontent.languageID, pageIDcontent.urlrewrite, pageIDcontent.lang_url_part, pageIDcontent.menu_url_part, pageIDcontent.content_url_part, menulevelcontent.levelname FROM menulevelcontent JOIN menulevel AS menulevel_childs ON menulevel_childs.id = menulevelcontent.menulevelID LEFT JOIN pageID ON menulevelcontent.menulevelID = pageID.menulevelID LEFT JOIN pageIDcontent ON pageIDcontent.pageIDID = pageID.id AND menulevelcontent.languageID = pageIDcontent.languageID WHERE menulevel_childs.parentlevelID = ? AND menulevel_childs.parentlevelID > 0 ORDER BY pageID.id";
	$o_query = $o_main->db->query($modRewriteSql, array($modRewriteRow['menulevelID']));
	...
	foreach($o_query->result_array() as $modRewriteRow)
	{
		...
		update_menu_urls($modRewriteRow, $char_map, $s_list_prefix);
	}
}
?>
...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		...
		if($_POST['editResource'] > 0) {
			$s_sql = "UPDATE articlediscountmatrix SET
			updated = now(),
			updatedBy= ?,
			name= ?
			WHERE id = ?";
			...
			$s_sql = "INSERT INTO articlediscountmatrix SET
			id=NULL,
			moduleID = ?,
			created = now(),
			createdBy= ?,
			...
			$s_sql = "DELETE articlediscountmatrix FROM articlediscountmatrix WHERE articlediscountmatrix.id = ?";
...
<div class="popupform">
	...
	<div class="popupformTitle"><?php echo $formText_AddEditDiscountMatrix_output;?></div>
	<div class="errorMessage"></div>
	<div class="resourceList" data-action="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=add_discount_matrix";?>"
	data-action2="<?php print $_SERVER['PHP_SELF']."?pageID=".$_GET['pageID']."&accountname=".$_GET['accountname']."&companyID=".$_GET['companyID']."&caID=".$_GET['caID']."&module=".$module."&folderfile=output&folder=output&inc_obj=ajax&inc_act=add_discount_matrix";?>">
	<?php
	$resources = array();
	$s_sql = "SELECT * FROM articlediscountmatrix WHERE content_status < 2 ORDER BY name ASC";
	...
	foreach($resources as $resource){
		...
		<div class="resourceRowSortable" id="sort_<?php echo $resource['id']?>">
			<div class="resourceRow">
				...
				if($o_query && $o_query->num_rows()>0) {
				    $usedByCount = $o_query->num_rows();
				}
				?>
				<div class="column infoColumn">
					<div class="columnWrapper"><?php echo $formText_UsedBy_output." ".$usedByCount." ".$formText_Customer_output;?><span class="showArticles" data-resource-id="<?php echo $resource['id']?>" data-count="<?php echo $usedByCount;?>"><?php echo $formText_ShowCustomers_output;?></span></div>
				</div>
				<div class="clear"></div>
			</div>
			<?php if(!$resource['disabledForEditing']) { ?>
	...
	<div class="addNew">
		<div class="plusTextBox active">
			...
			<div class="text"><?php echo $formText_AddDiscountMatrix_output; ?></div>
		</div>
		<div class="clear"></div>
	</div>
			...
			$.ajax({
				...
				success: function(result){
					if(parseInt(result.html) > 0){
						ajaxCall('add_discount_matrix', {}, function(obj) {

...
if($moduleAccesslevel > 10)
{
	if(isset($_POST['output_form_submit']))
	{
		...
		if($_POST['editResource'] > 0) {
			$s_sql = "UPDATE articlepricematrix SET
			updated = now(),
			updatedBy= ?,
			name= ?
			WHERE id = ?";
			...
			$s_sql = "INSERT INTO articlepricematrix SET
			id=NULL,
			moduleID = ?,
			created = now(),
			createdBy= ?,
			...
			$s_sql = "DELETE articlepricematrix FROM articlepricematrix WHERE articlepricematrix.id = ?";
...
<div class="popupform">
	...
	<div class="popupformTitle"><?php echo $formText_AddEditPriceMatrix_output;?></div>
	...
	<?php
	$resources = array();
	$s_sql = "SELECT * FROM articlepricematrix WHERE content_status < 2 ORDER BY name ASC";
	...
	<div class="addNew">
		<div class="plusTextBox active">
			...
			<div class="text"><?php echo $formText_AddPriceMatrix_output; ?></div>
		</div>
		<div class="clear"></div>
	</div>
			...
			$.ajax({
				...
				success: function(result){
					if(parseInt(result.html) > 0){
						ajaxCall('add_price_matrix', {}, function(obj) {

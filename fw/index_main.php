...
if(!empty($_SERVER['HTTP_MOBILE_WEBVIEW_USERNAME']) && !empty($_SERVER['HTTP_MOBILE_WEBVIEW_SESSIONID']) && !empty($_SERVER['HTTP_MOBILE_WEBVIEW_CAID']))
{
...
if(isset($_GET['pageID']) && 100 == $_GET['pageID']) {
	$b_fw_getynet_app = true;
}
//-------------------------------------------------------------------
...
else if($_COOKIE['username'] != '' && $_COOKIE['sessionID'] && !isset($_GET['pageID']) && isset($_GET['cID']))
{
	...
	*/
	...
	/*
	** Two-factor authentication check and send code
	*/
	if(0 < $o_main->accountinfo['activate_2factor_authentification'])
	{
		$o_query = $o_main->db->query("SELECT * FROM session_framework WHERE companyaccessID = '".$o_main->db->escape_str($_GET['cID'])."' AND session = '".$o_main->db->escape_str($_COOKIE['sessionID'])."' AND username = '".$o_main->db->escape_str($_COOKIE['username'])."'".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")." LIMIT 1");
		$fw_session = $o_query ? $o_query->row_array() : array();
		$v_user_profile = json_decode($fw_session['user_profile'], TRUE);
		
		
		$o_query = $o_main->db->query("SELECT * FROM accountinfo_mobile_auth WHERE user_id = '".$o_main->db->escape_str($v_user_profile['id'])."' AND device_id = '".$o_main->db->escape_str($_COOKIE[$o_main->accountname.'_DUID'])."' AND expire >= NOW() AND object = 'fw'".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")." LIMIT 1");
		...
		if(0 === $l_valid)
		{
			...
			if($b_do_mobile_verification)
			{
				...
				$_SESSION['fw_verify_mobile_code'] = $fw_verify_mobile_code;
			...
			if(!empty($fw_verify_mobile_code['device_id']))
            {
...
if(isset($_COOKIE['username']))
{
	...
	if(!FW_AJAX_LESS && isset($v_accountinfo['activate_membersystem_subscription_login_check']) && 1 == $v_accountinfo['activate_membersystem_subscription_login_check'] && 4 == $variables->fw_session['accesslevel'])
	{
	...
	if(!isset($_SESSION['usersession']))
	{
		...
		$o_fw_user_session = $_SESSION['usersession'];
	}
	...
	if(!property_exists($response,'error')) 
	{
		if(!isset($_SESSION['usersession']))
		{
			$_SESSION['usersession'] = $o_fw_user_session;
		}
		...
		if(!FW_AJAX_LESS) include(__DIR__.'/account_fw/menu/refresh_user_session.php'); // This is old session reload!
	}
}
/*
...
if(0 < $o_main->accountinfo['activate_2factor_authentification'])
{
    ...
    $o_main->two_fa_valid = ($o_query && 0 < $o_query->num_rows());
	$o_main->show_two_fa = !$o_main->two_fa_valid;
}

/*
** Change developer mode
**/
if(!FW_AJAX_LESS && isset($_POST['fw_setdeveloperaccess']))
{
...
** Change language
**/
if(!FW_AJAX_LESS && isset($_POST['fw_set_account_language']) && '' != $_POST['fw_set_account_language'])
{
	$v_param = array(
...
if(!FW_AJAX_LESS)
{
	...
	$variables->fw_settings_basisconfig = array();
	if($o_main->db->table_exists('frameworksettings_basisconfig'))
	{
		$o_query = $o_main->db->get('frameworksettings_basisconfig');
		$variables->fw_settings_basisconfig = $o_query ? $o_query->row_array() : array();
	}
	$variables->fw_settings_accountconfig = array();
	if($o_main->db->table_exists('frameworksettings'))
	{

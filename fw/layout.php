...
$fw_activate_groups = isset($variables->accountinfo['activateGroups']) ? $variables->accountinfo['activateGroups'] : FALSE;
$fw_read_group_data_from_external_database = isset($variables->accountinfo['readGroupDataFromExternalDatabase']) ? $variables->accountinfo['readGroupDataFromExternalDatabase'] : FALSE;
$fw_groups_list = array();

// Account config lang "variables"
$selectGroupText = isset($variables->accountinfo['selectGroupText']) ? $variables->accountinfo['selectGroupText'] : FALSE;
$noAccessToAnyGroupText = isset($variables->accountinfo['noAccessToAnyGroupText']) ? $variables->accountinfo['noAccessToAnyGroupText'] : FALSE;
$changeGroupButtonText = isset($variables->accountinfo['changeGroupButtonText']) ? $variables->accountinfo['changeGroupButtonText'] : FALSE;
...
if(!$b_fw_account_upgrade_lock && 0 < (int)$o_main->accountinfo['activate_2factor_authentification'])
{
	$o_query = $o_main->db->query("SELECT * FROM accountinfo_mobile_auth WHERE user_id = '".$o_main->db->escape_str($variables->userID)."' AND device_id = '".$o_main->db->escape_str($_COOKIE[$o_main->accountname.'_DUID'])."' AND expire >= NOW() AND object = 'fw'".($o_main->multi_acc?" AND account_id = '".$o_main->db->escape_str($o_main->account_id)."'":"")." LIMIT 1");
	...
	if(0 === $l_valid)
	{
		...
		$b_do_mobile_verification = 1 == (int)$o_main->accountinfo['activate_2factor_authentification'];
		$b_do_google_verification = 3 == (int)$o_main->accountinfo['activate_2factor_authentification'];

		if(!isset($_POST['fw_verify_mobile_code']) || 'resend' == $_POST['fw_verify_mobile_id'])
		{
			...
			{
				$fw_verify_mobile_code = $_SESSION['fw_verify_mobile_code'] ? $_SESSION['fw_verify_mobile_code'] : array();
				if('resend' == $_POST['fw_verify_mobile_id'] || !array_key_exists('id', $fw_verify_mobile_code))
				{
					...
					$s_response = APIconnectorAccount($s_commands, $o_main->accountinfo['accountname'], $o_main->accountinfo['password'], $v_param);
					$_SESSION['fw_verify_mobile_code'] = $fw_verify_mobile_code = json_decode($s_response, true);
					setcookie($o_main->accountname.'_DUID', $fw_verify_mobile_code['device_id'], time()+60*60*24*365*2, '/', '.'.$host, true, true);
				}
			}/* else if($b_do_google_verification) {
                $fw_verify_mobile_code = array(
                    'status' => 21,
                );
				$s_response = APIconnectorUser('userprofileget', $_COOKIE['username'], $_COOKIE['sessionID']);
                $v_response = json_decode($s_response, true);
				if(isset($v_response['status']) && $v_response['status'] == 1)
				{
					$v_user = $v_response['data'];
					if(!empty($v_user['google_2fa'])) $fw_verify_mobile_code['status'] = 1;
				}
			} else {
				$s_commands = 'verify_mobile_send_code';
                $v_param = array(
            ...
            if($b_do_google_verification)
			{
				require(__DIR__.'/getynet_fw/verify_mobile/verify_google.php');
			} else {
				$s_commands = 'verify_mobile_check_code';
				$v_param = array(
					'ID' => $_POST['fw_verify_mobile_id'],
					'DEVICE_ID' => $_POST['fw_verify_device_id'],
					'CODE' => $_POST['fw_verify_mobile_code']
				);
				if($b_do_mobile_verification)
				{
					$v_param['MOBILE'] = $_POST['fw_verify_mobile'];
				} else {
					$v_param['EMAIL'] = $_POST['fw_verify_email'];
				}
				$s_response = APIconnectorAccount($s_commands, $o_main->accountinfo['accountname'], $o_main->accountinfo['password'], $v_param);
				$v_response = json_decode($s_response, true);
				if(isset($v_response['status']) && $v_response['status'] == 1)
				{
					$l_expire_interval = intval($o_main->accountinfo['2factor_auth_session_valid_for_days']);
					if(0 === $l_expire_interval) $l_expire_interval = 1;
					$o_main->db->query("INSERT INTO accountinfo_mobile_auth SET created = NOW(), createdBy = '".$o_main->db->escape_str($variables->loggID)."', user_id = '".$o_main->db->escape_str($variables->userID)."', device_id = '".$o_main->db->escape_str($_COOKIE[$o_main->accountname.'_DUID'])."', object = 'fw', expire = DATE_ADD(CONCAT(CURDATE(), ' 23:59:59'), INTERVAL ".$l_expire_interval." DAY)".($o_main->multi_acc?", account_id = '".$o_main->db->escape_str($o_main->account_id)."'":""));
					...
					if($b_do_mobile_verification)
					{
						...
						$fw_verify_mobile_code['email'] = $_POST['fw_verify_email'];
					}
				}
			}
		}
...
if(isset($_GET['fwajax']) && $_GET['fwajax'] == 1) $_POST['fwajax'] = 1;
...
?>
<body class="<?php echo $fw_getynet_class; if(isset($variables->fw_settings_basisconfig['alternative_design_seperate_menu_scroll']) && $variables->fw_settings_basisconfig['alternative_design_seperate_menu_scroll']) echo ' seperateMenuScroll';?>">
	<div id="fw_container" class="<?php echo $fw_getynet_class;?>">
		<?php if($acceptedTerms){ ?>
			...
			<div id="fw_account" class="<?php echo $fw_getynet_class;?>">
	            <?php if ((!isset($fw_selected_group) || !$fw_selected_group) && $fw_activate_groups): ?>
	            ...
	            <?php else: ?>
					...
					<div class="fw_col <?php if(!isset($variables->fw_settings_basisconfig['user_alternative_design']) || !$variables->fw_settings_basisconfig['user_alternative_design']) {?>tinyScrollbar<?php } ?> col0 <?php echo ($b_fw_getynet_app?' hide':'');?>">
	    			...
	    			<div class="fw_col <?php if(!isset($variables->fw_settings_basisconfig['user_alternative_design']) || !$variables->fw_settings_basisconfig['user_alternative_design']) {?> tinyScrollbar<?php } ?> col1<?php echo ($fw_columns==1?' end':'').($b_fw_getynet_app && $fw_columns==2 && $ob_column2!=''?' hide':'');?>"
	    			...
	    			<div class="fw_col <?php if(!isset($variables->fw_settings_basisconfig['user_alternative_design']) || !$variables->fw_settings_basisconfig['user_alternative_design']) {?> tinyScrollbar<?php } ?> col2<?php echo ($fw_columns==1 || $ob_column2==''?' hide':' end');?>"

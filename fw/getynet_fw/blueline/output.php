...
// Framework settings
$framework_settings_globalstyles = array();
...
// Helpcenter
$is_helpcenter_active = $variables->fw_settings_basisconfig && $variables->fw_settings_basisconfig['activateHelpButton'];
$b_help_page_active = $variables->fw_settings_basisconfig && 1 == $variables->fw_settings_basisconfig['activate_help_page'];
if($o_main->show_two_fa)
{
	$is_helpcenter_active = $b_help_page_active = $b_is_customer_chat_active = FALSE;
}

// Onboarding popup status check
$introArticle = false;
$fw_intro_force_read = false;
if($variables->fw_settings_basisconfig && $variables->fw_settings_basisconfig['activateReadIntroNewUsers']){
	$sql = "SELECT t.*
...
if($variables->fw_settings_basisconfig && $variables->fw_settings_basisconfig['framework_logo'] != ""){
...
//Hide chat config
$hideChatForAccount = isset($variables->fw_settings_basisconfig['hideChatForAccount']) ? $variables->fw_settings_basisconfig['hideChatForAccount'] : 0;
if(isset($variables->fw_settings_accountconfig['hideChatForAccount']) && $variables->fw_settings_accountconfig['hideChatForAccount'] > 0){
	$hideChatForAccount = $variables->fw_settings_accountconfig['hideChatForAccount'] - 1;
}
if($o_main->show_two_fa) $hideChatForAccount = 1;
?>
<!-- Wraper  -->
<div class="wraper<?php if($variables->fw_settings_basisconfig && $variables->fw_settings_basisconfig['user_alternative_design']) echo ' top_line_wrapper';?> fw_header_color">
	<?php if($variables->fw_settings_basisconfig && $variables->fw_settings_basisconfig['user_alternative_design']) { ?>
		...
		<?php
		if($variables->fw_session['developeraccessoriginal']>0 && !$o_main->show_two_fa)	{
	...
	<div class="fw_settings">
		<?php if($variables->fw_settings_basisconfig && $variables->fw_settings_basisconfig['user_alternative_design']) { ?>
			...
			<?php if(!$o_main->show_two_fa) require(__DIR__ . '/../modules/NotificationCenter/output/output.php'); ?>
			<?php if($hideChatForAccount != 1) { ?>
			<!--  Chat -->
...
$("#fw_menu_options").off("click").on('click',function(){
	$(".fw_logout_menu_dropdown").slideToggle();
})
$("#fw_change_mode_wrapper").off("click").on("click", function(){
	$(".fw_language_change").hide();
	$(".fw_mode_change").toggle();
})
$("#fw_change_language_wrapper").off("click").on("click", function(){
	$(".fw_mode_change").hide();
	...
	<script type="text/javascript">
		var onboardingPopup, onboardingPopupOptions={
			...
			onOpen: function(){
				...
				$(this).find('.b-close').off("click").on('click', function(){

...
if($variables->loggID != '')
{
	...
	if($o_query)
	{
		foreach($o_query->result_array() as $v_row)
		{
			...
			$s_virtual_module = '';
			if('' != trim($v_row['virtual_module_source']))
			{
	...
	{
		if($_GET['module']==37) $b_expand_admin_module = true;
		ob_start();

		if($variables->accountinfo['deactivate_useradministration'] > 0) {
			$variables->accountinfo_basisconfig['deactivate_useradministration'] = $variables->accountinfo['deactivate_useradministration'] - 1;
		}
		if(!$variables->accountinfo_basisconfig['deactivate_useradministration'] || $variables->developeraccess >= 10) {
			?>
			<div class="fw_menu_item fw_menu_color adm activate mod_UserManager<?php if($_GET['module']==37) echo " active";?>" data-group="fw_menu_item">
		...
		if($variables->fw_settings_accountconfig['activate_account_settings']) {
		...
		if($variables->fw_settings_basisconfig['activate_account_settings'] && $variables->fw_settings_basisconfig['account_settings_modulename'] != "" && $variables->useradmin) {
	...
	$v_user_groups = array();
	$hideGroupsFromMenu = $variables->fw_settings_basisconfig['hideGroupsFromMenu'];
	if($variables->fw_settings_accountconfig['hideGroupsFromMenu'] > 0){
		$hideGroupsFromMenu = $variables->fw_settings_accountconfig['hideGroupsFromMenu'] - 1;
	}
	if(!$hideGroupsFromMenu){
		$o_query = $o_main->db->query("SELECT * FROM contactperson_group WHERE group_type = ? ORDER BY name", array($people_contactperson_type));
	...
	if(intval($variables->accountinfo['allow_add_group']) > 0) {
		$variables->accountinfo_basisconfig['allow_add_group'] = intval($variables->accountinfo['allow_add_group']) - 1;
	}
	$variables->accountinfo_basisconfig['allow_add_group'] = intval($variables->accountinfo_basisconfig['allow_add_group']);

	$variables->accountinfo['allow_create_grouppage_only_for_admin'] = intval($variables->accountinfo['allow_create_grouppage_only_for_admin']);
	...
	if($s_buffer!='')
	{
		if($totalDepartmentsCount+$totalGroupsCount > 0) {
